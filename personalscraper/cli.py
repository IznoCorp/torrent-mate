...
@app.command()
def sort(dry_run: bool = typer.Option(False, "--dry-run", help="Preview without moving")):
...
@app.command()
def verify(dry_run: bool = typer.Option(False, "--dry-run", help="Preview without fixing")):
    """Verify and qualify scraped media before dispatch."""
    state["console"].print("[bold]verify[/bold] — not yet implemented (V4)")


@app.command()
def dispatch(dry_run: bool = typer.Option(False, "--dry-run", help="Preview without moving")):
    """Move media to storage disks."""
    state["console"].print("[bold]dispatch[/bold] — not yet implemented (V5)")


@app.command()
def run(dry_run: bool = typer.Option(False, "--dry-run", help="Preview full pipeline")):
