...
WHAT IS HELD HERE, and why it is not the arms. The five arms are exercised by
...
def test_the_skip_names_only_the_arms_that_need_the_parser() -> None:
    """The message names one arm, not five, and says the others ran.
    ...
    do. A skip covering all five would leave those resting on nothing.
    """
    source = GUARD.read_text(encoding="utf-8")
    assert 'NEEDS_THE_PARSER = ("correspondence",)' in source
