...
# THE FORWARDED-VALUE ARM'S TESTS DIED WITH THE ARM (b·7, 2026-09-14, ruling 87).
# `TestReadersOf` and `TestTheContract` held the two dead controls B-031 and the
# `gird` typo, against a handler shape — `store.write({ f: e.dataset.x })` — the
# engine no longer has anywhere. The guard's own header says what was lost and
# B-513 carries it; keeping the tests over a deleted arm would be two files
...
class TestTheTreeItself:
    ...
    # THE FORWARDER FLOOR DIED WITH ITS SUBJECT, AND SO DID THE ARM
    # (2026-09-14, b·7, rulings 78 and 87). The floor read « the engine still
    # forwards at least N attributes into the store » and was re-measured
    # downward at every phase that took one (5 → 4 → 3); the engine forwards
    # NONE now — `page`, `go` and `phase` were its last three and they are the
    # frame's verbs. The arm itself refused an empty corpus rather than printing
    # a green zero, which is the right refusal and left nothing to keep: it is
