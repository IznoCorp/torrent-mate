...
class TestFolderRenameBranches:
    ...
    def test_folder_rename_when_target_does_not_exist(self, scraper: Scraper, tmp_path: Path, movie_data: dict) -> None:
        ...
        # NAME-NEUTRAL Plex hint (operator directive): the folder name stays
        # the plain Kodi canonical — never a {tmdb-…} suffix — and the match
        # hint lives in the .plexmatch sidecar.
        canonical = tmp_path / "The Matrix (1999)"
        assert "{tmdb-" not in canonical.name
        assert (canonical / ".plexmatch").read_text(encoding="utf-8") == ("title: The Matrix\ntmdbid: 603\n")
