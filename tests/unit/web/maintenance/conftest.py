"""Shared fixtures for the maintenance-runner test suite.
...
def _neutralize_runner_sigterm_handler() -> Iterator[None]:
    ...
    pytest-xdist worker. The dedicated SIGTERM unit test nests its own capturing ``patch`` inside this
    one to still exercise the handler, so this fixture does not weaken that
    coverage — it only prevents the handler from surviving the test and killing
    the worker at shutdown.
