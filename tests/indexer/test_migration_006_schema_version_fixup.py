...
def test_migration_006_backfills_row_3_when_missing(tmp_path: Path) -> None:
    ...
    rows = conn.execute("SELECT version FROM schema_version ORDER BY version").fetchall()
    versions = [r[0] for r in rows]
    assert versions == [1, 2, 3, 4, 5, 6, 7, 8, 9], f"After 006-009, expected [1..9], got {versions}"

    user_version = conn.execute("PRAGMA user_version").fetchone()[0]
    assert user_version == 9, f"user_version must be 9, got {user_version}"
...
def test_migration_006_idempotent_on_fresh_db(tmp_path: Path) -> None:
    ...
    rows = conn.execute("SELECT version FROM schema_version ORDER BY version").fetchall()
    versions = [r[0] for r in rows]
    assert versions == [1, 2, 3, 4, 5, 6, 7, 8, 9], f"Fresh DB, expected [1..9], got {versions}"
