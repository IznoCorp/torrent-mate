...
| `LockedCapabilityUnresolved` | `personalscraper.api.metadata.registry._events` | `locked()` cannot bind the match's own provider                                                                                 | registry `locked` dispatch                                                                                                                                         |
...
| `ProviderCallCompleted`      | `personalscraper.api.metadata.registry._events` | `provider: str`, `latency_ms: float`, `ok: bool` — throttled ≤1/10s per transport (reg-health S6 web health projection)         | `HttpTransport._request_outer` after each call (success + failure paths)                                                                                           |
...
| `FilmAcquired`               | `personalscraper.acquire.events`                | `media_ref: MediaRef`, `title: str`, `followed_id: int`                                                                         | acquire/ — §5: a followed film reached the library and was auto-removed from the follows (detect)                                                                  |
...
| `GrabReswitched`             | `personalscraper.acquire.events`                | `media_ref: MediaRef`, `old_hash: str`, `reason: str` (`dead_swarm`/`broken`/`deadline`)                                        | acquire/ — emitted by `_reswitch.reswitch_stalled` when a dead-stalled grab is switched (reswitch #342); WS-relayed → `GrabReswitched` toast + wanted invalidation |
