...
`core/columns.load_columns`) which take a YAML _string_ and fail **loud** on any defect
...
(`app/wiring.py:189-230`), i.e. at _launch_ time, far from the edit.
...
org-webhook only — which _confirms_ KanbanMate's "no webhook, no n8n" polling design). A bespoke
...
- **`ColumnDef(key: str, name: str, column_class: str)`** — `column_class` is the **string**
  `"reactive"` | `"inert"`, mirroring `core.domain.ColumnClass` whose member _values_ are exactly
...
3. **Calling `load_transitions(transitions_yaml)` purely as a validation oracle** — its return
   value is _discarded_; we only care that it does **not** raise. A raw re-parse that the loader
...
load_transitions(X)` and likewise for columns — i.e. the _parsed_ configs are equivalent, not the
...
`decide()`'s **reactive interception** runs _before_ the whitelist in the real engine
...
| **V8**  | **Defaults coherence** — warn if an _uncommented_ `columns.yml` `defaults:` block disagrees with the authoritative `transitions.yml` `defaults:` (dead config). **Presence is detected on the RAW document** (`yaml.safe_load(columns_yaml).get("defaults")` is a dict) — NOT on `load_board_defaults`, which returns `BoardDefaults()` (3/10) for _both_ an absent block and a present 3/10 block (`core/columns.py:217-235`), so trusting its return would false-positive when the cap is raised in `transitions.yml` while `columns.yml`'s block stays commented (the shipped state, `assets/columns.yml.tmpl:26-28`) | `BoardDefaults` / `load_board_defaults` (`core/columns.py:139-235`); authority §10                                                                                                                                                                                                                                                         | **warning**                                               |
...
`CLONE_COLUMNS_RELPATH` directly. Instead the **`http/` entrypoint** (which _may_ import
...
- **Merge authority** — historically human-only; an **operator decision (2026-06-18)** added the
  autonomous Review→Merge stage (the `merge` profile + a sanctioned `Review → Merge` agent
  transition). V7 now carries a carve-out: a prompt-bearing transition into `Merge` is allowed
  **iff** it uses the `merge` profile (every OTHER profile is still rejected — no non-merge agent
  may be launched into `Merge`). The `merge` profile is the SOLE profile whose `perms.deny_list()`
  lifts `gh pr merge` (it stays banned for all others); force-push / history-rewrite / direct-main
  push stay banned even there, and `bypass*` is banned (V3). helm itself still performs **no** board
  mutation and **cannot** merge; the merge authority lives only in the sanctioned merge stage.
