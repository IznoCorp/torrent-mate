...
**R119 — `frontend/maquette/harness/priming.py`, 39 holds, full suite.** It intercepts two seams
from an init script — a wrapper installed after `load` arrives after the placeholder was computed —
and makes TWELVE cold loads, each a walk of its own. **The rule's own header enumerates them and this
...
one where the seasons land before the sheet, an ownership the layer answers as NULL — the
contract's third value — and a medium DELETED while its sheet is open.
...
**R117 — `frontend/maquette/harness/virtual.py`, 36 holds.** Its delete hold asserts the paging door
