...
`twice` arm and the size arm are GUARDS — `scripts/mutate.sh` cannot judge a guard (B-273) — so
...
- A guard that counts duplicates from a regex is only as good as the regex: the `twice` arm prints
