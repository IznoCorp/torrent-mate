...
> **PLAN CORRECTIONS — validated against code 2026-06-01 (follow THESE; the task bodies below drifted):**
>
> 1. **`library/models.py` is NOT only dataclasses.** It also defines I/O helpers `write_json` / `read_json` / `serialize_to_json` (`:573`/`:588`/+) used by `commands/library/analyze.py` (3×) and `commands/library/maintenance.py` (1×), and issue/priority/action string constants (`ISSUE_*` `:64-69`, `PRIORITY_*` `:215-217`, `ACTION_*`/`SKIP_*` `:460-464`, `_VALID_VALIDATION_STATUSES`). **Decision (fills the DESIGN §4.6 gap):** `write_json`/`read_json`/`serialize_to_json` → **`personalscraper/io_utils.py`** (the canonical I/O util module — already hosts `atomic_write_json`; layering-clean, avoids a maintenance→insights dep). The `ISSUE_*`/`PRIORITY_*`/`ACTION_*`/`SKIP_*` constants → move WITH their producer (trace consumers: `ISSUE_*` are used by `_item_stage._detect_issues` → `_item_stage`/`_item_stage_types`; `PRIORITY_*` → `insights` recommender/models; `ACTION_*`/`SKIP_*`/`_VALID_VALIDATION_STATUSES` → `maintenance/rescraper.py` or `verify/library_checks.py` per which module references them — grep before moving).
> 2. **`doctor.py` enum is UPPERCASE:** `CheckStatus.OK` / `CheckStatus.WARN` / `CheckStatus.FAIL` / `CheckStatus.SKIP` (NOT `.ok`/`.warn`). `CheckResult(name=, status=, message=)`. Register `_check_nfo_missing` in the real check list.
> 3. **Blast radius is wider than the migrate table.** Live importers to re-point at deletion: `commands/library/analyze.py` (write_json/read_json), `commands/library/maintenance.py` (disk_cleaner/validator/models), `indexer/scanner/_modes/_item_stage.py:45` (the scan-stage dataclasses — resolves when Task 3 moves them here), plus TEST files `tests/test_cli.py` (many `library.disk_cleaner`/`validator`/`models` patches), `tests/commands/test_library_analyze.py` (`library.models` patches), `tests/library/test_integration.py` (disk_cleaner + models + insights), `tests/library/test_index_aware.py` (`library.validator`), `tests/library/test_validator_fix.py`. Migrate/re-point ALL — the Step 5.4 grep must be zero before deletion.
> 4. **`tests/architecture/test_layering.py:33`** lists `"personalscraper.library"` as a layering-rule node — REMOVE that entry when the package is deleted (else the layering test references a dead package).
> 5. **ACC-06 force-now (operator pattern — same as ACC-05, NOT a re-scope):** after deletion, surviving DOCSTRING provenance notes containing the literal `personalscraper.library` remain in `indexer/scanner/_modes/_item_stage.py` (`:4`) and `tests/indexer/scanner/_modes/test_item_stage_golden.py` (`:4,:56,:70,:385`). To make `rg -t py 'personalscraper\.library' personalscraper/ tests/` → rc=1 pass LITERALLY, **scrub those provenance strings** (rephrase to e.g. "the former library scanner module" / "the legacy scanner" — keep the meaning, drop the dead dotted path). This is cosmetic + leaves no reference to a deleted package.
