...
| 4   | Indexer scanner modes split: `indexer/scanner/_modes.py` 1900 LOC → `_modes/` package (6 actual modes: full, quick, incremental, enrich, verify, backfill) | `phase-04-indexer-scanner-modes-split` | 5-7               | High (touches scan execution path) | Yes (each mode independently revertable) |
