...
| ACC-26 | 6 | L04's measured-defects table in `docs/reference/frontend-architecture.md` carries a **Now** column, and its cycles row reads **0**: `grep -c 'Measured before' docs/reference/frontend-architecture.md` returns 1. **Amended during phase 6, and the reason is on the record**: this criterion first read « `git grep 'Two import cycles'` returns nothing », which would have been satisfied by DELETING the row — losing the record of what the lot existed to fix. §7.1 asks for the measurement to be refreshed, not for the defect to be unwritten, so the row keeps its name and gains what the same command reports now. The letter was wrong; the intent is what is tested |
