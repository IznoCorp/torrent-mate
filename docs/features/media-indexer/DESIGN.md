...
- **macOS read-ahead hint**: every file opened for fingerprint or mediainfo gets a sequential read hint via `mmap(fd, size, ACCESS_READ).madvise(MADV_SEQUENTIAL)`. The wrapper is encapsulated in `indexer/_macos_io.py::sequential_hint(fd, offset=0, length=0)` and is a no-op outside Darwin. Encourages kernel read-ahead, reduces head-seek overhead on HDDs. **Implementation note (4.5)**: the original spec called for `fcntl(fd, F_RDADVISE, struct.pack('qi', offset, length))`, but `fcntl(2)` is variadic in C and Python's `fcntl` module + `ctypes` both emit non-variadic call sequences, which the arm64 ABI sends through different registers — the kernel rejects the call with `ENOTTY`. `madvise(MADV_SEQUENTIAL)` achieves the identical kernel goal (sequential prefetch advisory into the unified buffer cache) and is reachable from Python on Darwin. The `F_RDADVISE = 35` constant remains exported for the day a working arm64 wrapper exists. `os.posix_fadvise` is not used — it does **not exist on Darwin**.
...
- 4.5 `indexer/_macos_io.py::sequential_hint(fd, offset, length)` (Darwin: `mmap+madvise(MADV_SEQUENTIAL)`; elsewhere: no-op) on every fingerprint/mediainfo open. See §11.6 for the rationale on the `F_RDADVISE → madvise` substitution.
