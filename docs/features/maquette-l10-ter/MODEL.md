...
and tappable, as its rank already makes it.** The sheet's bottom edge equals the bar's top edge
...
| P31 | **The tab bar is the floor**: a bottom layer's box never overlaps the bar's box, at rest or mid-transition — `sheet.bottom === nav.top` in every frame, and the bar's buttons stay hit-testable while the layer is open | the oracle's geometry for the sheet's open states, plus a rule that samples `getBoundingClientRect` during the transition and taps a tab through the open sheet | **false** — the sheet is `bottom-0` and rises behind the bar (B-248) | L15 |
