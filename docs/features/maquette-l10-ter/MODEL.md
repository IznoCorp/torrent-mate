...
| P5 | **A declared transition between sibling surfaces**: a page switch runs inside `document.startViewTransition`, the rules are `::view-transition-*` in `base.css`, none is a script (D9) | **R115** — reads `document.getAnimations()` mid-switch under `no-preference` and NONE under `reduce` (invariant 14), and holds that the OLD snapshot is the page being left, by a name that exists on one side only | **true** since L12 — `grep -c "view-transition" design/src/styles/base.css` → 34, none of them scripted | — |
| P6 | **A shared element carries the poster from the card into the panel** — it carries a `view-transition-name` on the card and on the sheet. **TITLE AMENDED 2026-08-31** (operator): it read « survives navigation », which its own body never said and which sent L12 looking for a shared element on a route change. `startViewTransition` animates any DOM change, not only a navigation; the panel pushes its own history entry and is addressable, so the long press is where the poster travels. The media screen keeps its wide fanart DEFINITIVELY, and there is nothing honest to share between a 2:3 poster and a banner | **none, and none is owed** — the rule that would have held it (`::view-transition-old(carried-poster)`) was built and removed with the drawing | **REFUSED BY THE OPERATOR, 2026-08-31.** L12 built the carry (`5f24d829`) and the operator withdrew it after watching it on his own phone (`a6be3a6e`); `grep -rn "carried-poster" frontend/maquette` reads 0 at HEAD. What replaced it is transition A, which shares nothing: the arriving fanart fades while the body rises, on the reasoning already in this row — there is nothing honest to share between a 2:3 poster and a wide banner. **This row is not « to build »**; it is a property the operator looked at and declined, and it is written that way so no later wave re-proposes it as unfinished work | — |
...
| P16 | **Gestures survive the compositor**: every gesture proved under a real touch stream AND a real mouse | R55, R98, `deck.py`, `drag.py`, `mouse.py`, and since L12 **R112** — the long press and the pull-to-refresh under a real touch stream AND a real mouse, including a mouse `pointercancel`, which no driver raises and which had never been driven | **true** for the gestures measured, and the set is larger | — |
| P17 | **The keyboard resizes the content, not the viewport**: `interactive-widget=resizes-content` | static read of the meta, and it is a RULE: `scripts/check-viewport-directives.py` requires the directive on every host | **true** since L12 — B-234 closed, `index.html:20` | — |
...
| P20 | **Reduced motion is a designed state** for every transition and gesture | R80 under both preferences; and per transition **R115**, which drives every arrival under `no-preference` AND `reduce` and holds that under `reduce` NO view transition animates — peak 0, which an absent stylesheet also satisfies, so the pressed state is read as a DRAWN state (a darkening, no movement) rather than as an absence | **true** for what exists, and L12's four transitions are inside it | — |
...
| P24 | **No unvirtualised long list** (1 861 titles) | **R117** — counts the rendered rows against the total the window declares, drives a real touch scroll, and reads the window's MEASURED lane count rather than a typed one | **true** since L12 — `@tanstack/react-virtual` behind `ui/virtual-rows.tsx`, the operator having reversed D9's rule 2 to allow it. The paging by `IntersectionObserver` is still there and is a different mechanism: it decides what is LOADED, the window decides what is DRAWN | — |
| P25 | **No tap flash**: `-webkit-tap-highlight-color: transparent` on every interactive element | the compositor-CSS guard, whose floor for the property moved from 0 to 1 in the same move | **true** since L12 — `base.css:84`, declared ONCE on the root because the property is inherited, so no component has to remember to carry it | — |
| P26 | **A long press selects no text** on a tile or a card (`user-select: none`) | the compositor-CSS guard, and the real-touch long press `selection.py` already drives | **true** since L12 — declared on the surfaces that actually ARM a press (`[data-panel]`, and the `.card`/`.sugwrap` containing one), which is deliberately not every card: a resolution card arms none and its release name must stay selectable. The guard's floor is 7, measured by raising it until it bit | — |
...
| P29 | **No layout shift when a poster loads**: every poster box declares its size (`aspect-ratio` or width/height) | **R114**, and the static half is `scripts/check-poster-box.py`. The probe WITHHOLDS every poster request until the layout is recorded, then releases — a fixture that answers instantly measures nothing — and it establishes that the posters actually landed, because an image that never arrives never pushes anything | **true** since L12 — the gallery's last tile moves 0.0 px when the images arrive | — |
