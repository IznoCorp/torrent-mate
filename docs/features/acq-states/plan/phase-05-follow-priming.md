# Phase 05 — Amorce à la création du suivi
...
**Périmètre du run d'amorce** : `detect --series {id}` puis `grab --followed-id {id}`, restreint
au suivi créé. Ne jamais déclencher une passe globale : ajouter une série ne doit pas relancer
l'acquisition de toute la médiathèque.
...
### 5.3 — L'état « Vérification en cours »
...
2. Les deux tests de 5.1 échouaient avant, passent après.
