...
| **L13r — The engine's residue** | CONVERSION (r·3 behaviour said) | the interface constants and helpers, the fixture families to their seeds, the settings machine, `render()`/`applyState` and the search mount out of the engine, `__referentiel` and its nine slices, `engine-shape`'s family labels renamed to the contract's, the file and its instruments die — measured at ruling 98 (2026-09-15): 1 601 live lines, 13 unconverted fixture families, engine-shape's 23 families at 41 call sites, `__referentiel` read by 45 files through nine slices | 7 |
