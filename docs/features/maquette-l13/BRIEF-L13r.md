...
(`plan/phase-r01-…md` to `plan/phase-r12-…md`) — **they are the specification and this brief restates none of
it**. L13r is the twelve phases r·1 to r·12 (ruling 101 cut r·3 in two, ruling 105 cut r·6 in six), in that order: the interface constants and helpers find homes (r·1),
...
slices die (r·5), `engine-shape`'s family labels become the contract's names, one family group per phase (r·6 to r·11), the file
and its instruments die under the full gate (r·12).
...
- Verify the state, do not believe it:
      ...
      ls docs/features/maquette-l13/plan/ | grep -c "phase-r"        # 12
...
- **Sizes**: every phase ≤ 15 points against the stated mean (11); the former « contract names » phase was measured
  and cut into r·6–r·11 by ruling 105, whose method (the contract type first, `tsc`'s diagnostics as the reader list)
...
- **A phase's gate** = one wrapped `run.sh --contracts --oracle <named rules>` + the 26 cheap guards it runs, logs
  ...
  after r·4 and before r·5, its falls repaired by you before r·5; and at r·12 before the pull request, with `--a11y`,
...
- **The three arms**: `python3 scripts/check-frame-domain.py` on the head BEFORE each phase's move (send its ceiling
  ...
  and r·5; `check-no-french.py` after every vocabulary addition and `--counts` at r·12; `check-mock-seeds.py` after r·2.
...
- The engine only shrinks; `scripts/frontend_size_ledger.py` re-recorded DOWNWARD in the same commit (the entry dies
  ...
  arms and `resync.py` at r·12); re-aims said out loud in the docstring and the body; never `cd` into `design/src` (B-384).
...
r·12 under the tests lock. At r·12: merge `origin/main` in, bump the version above whatever `main` reads then (patch), the
