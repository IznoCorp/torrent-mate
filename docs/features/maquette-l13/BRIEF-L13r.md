...
(`plan/phase-r01-…md` to `plan/phase-r17-…md`) — **they are the specification and this brief restates none of
it**. L13r is the seventeen phases r·1 to r·17 (ruling 101 cut r·3 in two, ruling 105 cut r·6 in six, ruling 106 added the system families, ruling 107 cut r·10 in two, ruling 109 cut r·13 in two, ruling 111 cut r·15 in three), in that order: the interface constants and helpers find homes (r·1),
...
dies (r·15), its instruments die (r·16), and the full gate and the pull request close the lot (r·17).
...
- **A phase's gate** = one wrapped `run.sh --contracts --oracle <named rules>` + the 26 cheap guards it runs, logs
  ...
  after r·4 and before r·5, its falls repaired by you before r·5; and at r·17 before the pull request, with `--a11y`,
...
- **The three arms**: `python3 scripts/check-frame-domain.py` on the head BEFORE each phase's move (send its ceiling
  ...
  and r·5; `check-no-french.py` after every vocabulary addition and `--counts` at r·17; `check-mock-seeds.py` after r·2.
...
r·17 under the tests lock. At r·17: merge `origin/main` in, bump the version above whatever `main` reads then (patch), the
