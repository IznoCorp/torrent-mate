...
(`plan/phase-r01-…md` to `plan/phase-r18-…md`) — **they are the specification and this brief restates none of
it**. L13r is the eighteen phases r·1 to r·18 (ruling 101 cut r·3 in two, ruling 105 cut r·6 in six, ruling 106 added the system families, ruling 107 cut r·10 in two, ruling 109 cut r·13 in two, ruling 111 cut r·15 in three, ruling 113 cut r·16 in two), in that order: the interface constants and helpers find homes (r·1),
...
slices die (r·5), `engine-shape`'s family labels become the contract's names, one family group per phase (r·6 to r·13), the projection dies (r·14), the file
dies (r·15), its readers die (r·16), the fixture apparatus dies (r·17), and the full gate and the pull request close the lot (r·18).
...
- **A phase's gate** = one wrapped `run.sh --contracts --oracle <named rules>` + the 26 cheap guards it runs, logs
  ...
  after r·4 and before r·5, its falls repaired by you before r·5; and at r·18 before the pull request, with `--a11y`,
...
- **The three arms**: `python3 scripts/check-frame-domain.py` on the head BEFORE each phase's move (send its ceiling
  ...
  and r·5; `check-no-french.py` after every vocabulary addition and `--counts` at r·18; `check-mock-seeds.py` after r·2.
...
- The engine only shrinks; `scripts/frontend_size_ledger.py` re-recorded DOWNWARD in the same commit (the entry dies
  ...
  arms and `resync.py` at r·16); re-aims said out loud in the docstring and the body; never `cd` into `design/src` (B-384).
...
r·18 under the tests lock. At r·18: merge `origin/main` in, bump the version above whatever `main` reads then (patch), the
