...
| a·18-bis | The ≡ harness panel dies — the operator's ruling on Q2, 2026-09-13 ([phase-a01](phase-a01-harness-module.md) § Amendment) | — | `harness/panel.ts` and its five verbs, the « ≡ » button's markup, the `harness.css` rules and i18n keys only the panel used | `hiding.py`, `message_above_harness.py`, `chrome.py` (their panel holds go with it, named with their counts); `audit.py`, `dest.py` (the exclusions dropped). **Re-taken by grep on 2026-09-13**, readers the row did not name: `scripts/check-markup-contracts.py:25` (docstring, `data-hscen`) and `:211` (comment, `data-hphase`); `tests/scripts/test_check_markup_contracts.py:76` (`hphase`) and `:765` (the reading of `harness/panel.ts`'s `data-part`, whose docstring already says it has no subject once the panel goes); `scripts/markup_verbs.py` (its answering side reads every `registerVerb`, the panel's five among them — `harness/panel_verbs.mjs` reads `ui/panel` action targets and is NOT a reader); `scripts/nofrench_values.py:30,66` (`harness/panel`); `scripts/code-vocabulary.txt:455,467,481,482,483` (`hclose`, `hgo`, `hphase`, `hscen`, `htmdb`); `audit.py:118` (`SCROLLERS`). In `design/`: `index.html` (`#scenBtn` in `harness/bar`), `src/harness/drive.ts` (`closeHarnessPanel` in `go`, and `__etatsDetailles`, which only the panel reads), `src/harness/index.ts`, `src/app/layers.ts:93`, `src/styles/legacy.css` (the `.hpanel` rules live there, not in `harness.css`), and the comments naming « the ≡ panel » in `src/ui/variants/frame.ts:37`, `src/app/shell.tsx:47` and every `src/harness/states/*.ts:4`. **The grep reads no panel hold in `hiding.py` nor `chrome.py`**: `hiding.py` reads `#notesBtn` (the notes toggle `installHarnessPanel` also wires) and `chrome.py` (R51) reads `harness/bar`; they move only if the notes toggle or the bar goes with the « ≡ » button, which a·18-bis states before it deletes | — |
