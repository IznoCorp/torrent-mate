...
- **Amended 2026-09-14 (l13b 4):** Back already reopens the panel since b·9 (R188), so this phase is the drawing + B-275's hold, bound to R190 `panel_return.py` (red: « return is DRAWN — pseudo-elements seen: [] », B-275 and reduce halves already green); who else answers — no click moves, the reopening runs in `app/layers.ts`'s sheet-entry branch inside `document.startViewTransition`, the rank branch (a panel closed over a panel, the journey over the follow sheet) stays undrawn — the design draws `panel-down`'s reverse for the media screen's return only, and a panel replacing a panel has no departure to mirror.
