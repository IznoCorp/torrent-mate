...
- **`mediaNamedBy` and `openDeleteDialog`** move to `features/library/`, reading the library query's
  ...
  **Amended 2026-09-13 by phase a·10 (the steward's ruling 41):** « reading the library query's cache (a·10) » is
  ...
  `follow-facts.ts:103` does, and `openDeleteDialog` reads `window.INCOMPLETE`. Both switch in b·11, where the
  two fixtures die.
...
- **Amended 2026-09-13 (steward, the three arms' dry read, audit order 2):** the same dialog-door amendment as b·5 (the library never imports `app/dialog-host.ts`); `mediaNamedBy` reads `window.LIBRARY` until b·11 (ruling 41).
