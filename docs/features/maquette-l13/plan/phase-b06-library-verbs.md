...
- **Amended 2026-09-14 (rulings 81, 82, 84):** `clear-filter` and `search-again` had no tapping rule — their holds went first (`follows.py`, `pause_verb.py` R132); the fan-in arm refused `app/page-switch.ts` at 5 → a `replaceAddress` door; R164 fell twice under the 28-named gate load and is filed as B-498 with its capture armed; the gate on `e4a16d970` is green.
