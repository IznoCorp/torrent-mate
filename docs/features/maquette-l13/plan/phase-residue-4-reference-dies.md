# Residue 4 — `window.__referentiel` and the nine slices die
...
- 43–45 product files read `window.__referentiel`, through nine `features/*/reference.ts` slices (acquisition 31
  ...
  re-homed by residue 1–3, so this phase deletes the object, `app/reference.d.ts` (46 lines), the slices, their hook
