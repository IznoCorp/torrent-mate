# r·6 — `legacy.js`, `seams.ts` and every instrument that reads them die; the full gate
...
- The current `phase-b13-legacy-js-dies.md` text, unchanged in substance: `engine/legacy.js` (1 600 today, ~0 after
  r·1–r·3), `engine/seams.ts` 183, `app/engine-data.ts` 101, `app/engine-redraw.ts` 45, `lib/engine-drawing.ts` 73;
