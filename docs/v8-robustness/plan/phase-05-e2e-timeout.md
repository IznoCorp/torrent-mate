...
- [ ] Modifier `wait_for_completion()` dans `tests/e2e/setup_torrents.py` :
...
- [ ] Ajouter log au démarrage du wait : `"Waiting for {n} torrents ({size:.1f} GB), timeout={timeout} min"`
- [ ] Écrire tests dans `test_setup_torrents.py` :
...
- [ ] Lancer tous les tests unitaires (710+) → vérifier que tout passe
- [ ] Mettre à jour CLAUDE.md : ajouter note sur le timeout E2E
- [ ] Mettre à jour `.env.example` si nécessaire (settings circuit breaker)
- [ ] Mettre à jour IMPLEMENTATION.md : section V8 complète
