...
- [ ] `window.__pont` and `window.__ecrans` become direct imports: the engine is in the module
      graph now, so the seam that existed to cross worlds has no worlds to cross.
