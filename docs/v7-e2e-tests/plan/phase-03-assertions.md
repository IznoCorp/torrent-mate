...
- [x] Créer `tests/e2e/assertions.py`
- [x] `assert_ingest_complete(staging_dir, expected)` :
...
- [x] `assert_sort_complete(movies_dir, tvshows_dir, expected)` :
...
- [x] Tests unitaires avec structures simulées

**Commit** : `v7.3.1: Implement ingest and sort assertions`

### 7.3.2 — Assertions scrape + verify

- [x] `assert_scrape_complete(movies_dir, tvshows_dir, expected)` :
...
- [x] `assert_verify_complete(results)` :
...
- [x] `assert_dispatch_complete(disk_paths, expected)` :
...
- [x] `assert_pipeline_report(report)` :
...
- [x] `assert_cleanup_complete(registry)` :
