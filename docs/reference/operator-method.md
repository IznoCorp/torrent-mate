...
| TM refonte 21:0x | 17 — la copie du lecteur épinglée à la tête de la DERNIÈRE porte de phase, re-pointée à la PR READY (`git checkout --detach`) ; mesure : PR READY 19:33 → lecteur 20:08 = 35 min ce soir | ordonné 21:0x | **oui (15/09) : #601 READY 03:49 → lecteur spawné 03:52 → handshake 03:52:37 = 3 min 30 ; #603 READY 10:04 → reader C 10:13 = 9 min (cible 5) ; entérinable, cible à tenir** |
| TM refonte 21:0x | 18 — la branche du sous-lot suivant coupée à cette même tête, son commit docs poussé PENDANT la pleine porte de la dernière phase ; mesure : 26 min lecteur → push L13b ce soir | ordonné 21:0x | **non tel qu'écrit (15/09) : L13r coupée 04:43, 54 min après la READY, second créneau vide 55 min ; remplacé par l'ordre 39** |
...
| TM refonte 15/09 13:4x | 42 — avant le spawn du premier agent d'un sous-lot (L13c d'abord), chaque fichier de phase porte sa mesure d'ouverture (la commande que le STOP D aurait prise : sites à déplacer, lecteurs, familles) et les coupes se font en UN commit docs ; mesure : L13r coupée cinq fois à l'ouverture le 15/09 (r·3, r·6, r·10-bis, r·10 ×2), 6 phases planifiées → 14 réelles, ~10 min de créneau par STOP D + un commit de renumérotation chacun ; coût ≤ 30 min au plan | appliqué 13:5x (`review-archive/l13r/docs-pr-list.md` item 11 : mesures d'ouverture des phases c· sur la tête finale de L13r, coupes en un commit, par le bâtisseur docs de L13r avant le spawn de l13c 1) | à lire à l'ouverture de L13c : 0 coupe de taille après le premier spawn |
