...
| `BackfillStarted`          | `personalscraper.indexer.events`  | `scope: str`, `item_count: int`                                                                                                 | `run_backfill_ids` at entry (L80-91)                                                               |
| `BackfillItemCompleted`    | `personalscraper.indexer.events`  | `item_id: int`, `item_title: str`, `ids_added: tuple[str, ...]`, `ratings_added: tuple[str, ...]`                               | `run_backfill_ids` per row written (L95-108)                                                       |
| `BackfillSkipped`          | `personalscraper.indexer.events`  | `item_id: int`, `item_title: str`, `reason: str`                                                                                | `run_backfill_ids` per row left untouched (L112-123)                                               |
| `BackfillCompleted`        | `personalscraper.indexer.events`  | `scope: str`, `scanned: int`, `updated: int`, `skipped: int`, `failed: int`, `ids_added_count: int`, `ratings_added_count: int` | `run_backfill_ids` at return (L127-146)                                                            |
