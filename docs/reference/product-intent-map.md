...
| **DOIT-3** — « agir là où l'on observe » | `features/arrivals` (the pilot's bar: run, stop, pause, resume — `data-pipe` lives only in `features/arrivals/page.tsx`) and `/system` (the runs it produced) | `partly` | served: R66 (`harness/arrivals.py` § « the pilot's bar », whose own first paragraph names DOIT-3). Acting from the tunnel one is looking at is SERVED: « Remettre en file » and « Re-scraper » on the journey sheet — R126 (`harness/journey_verbs.py`, 16 holds, B-302) — and « Récupérer cette saison » on the seasons panel, calling `follows/{followedId}/seasons/{season}/grab` — R125 (`harness/season_grab.py`, 16 holds, B-301). **To draw**: « relancer le watcher » and the global levers (the parallelism bound, pause/resume of everything — §20) — **L20**; the tracker policy set from the ratio surface — **L16** |
| **DOIT-4** — « toujours accepter une action légitime, mise en file visible » | `features/arrivals` (a pipeline pass asked for during a run: « Votre passage est **en file** », `fr.json` `arrivals.queuedBold`); the resolve queue; under §20, an arrival that meets the parallelism bound | `partly` | served for the pipeline pass: R66 (`arrivals.py:145–166`, reached by TAPPING during a run — « queued, and says so », « not refused: the running pass carries on »), and for the ACCEPTANCE half of every other mutation: **R124** (`harness/busy.py`, L19). The « En file » pastille is DRAWN: `data-part="season/queued"`, on BOTH season surfaces, held in the cache by `features/media/queued-seasons.ts` — **R138** (`harness/queued_ask_mark.py`, 7 holds), which reads the mark AFTER the message has gone, because a sentence shown for four seconds tells the operator who was looking and nobody else. Mutated: removing the mark makes two holds fall with « 0 mark(s) ». L19 had MEASURED its absence and could not draw it — a behaviour change its contract forbade. ⚠ **`partly` AND NOT `served`, and the drawing is not what is missing**: the pastille is reachable by NO path a finger can take. It reads the layer's `pipelineState`, which only the pipeline operations write and which no surface calls; Arrivées' « Lancer le pipeline » writes the engine's interface store and touches no network. Two pipeline notions, and the hand can move only the one the pastille does not read — **B-371**, owner **L20**, whose subject the levers are, by the operator's ruling |
...
| **DOIT-6** — « des résultats chiffrés pour un run manuel » | `features/acquisition` (« Lancer la veille maintenant » in the status sheet) | `partly` | the trigger exists (engine producer, `legacy.js:32395`, moving with **L19**). **To draw**: the run's figures — `GET /api/pipeline/history/{run_uid}` is called by nothing — **L20**; the raw lines of an execution, folded inside that detail and never the default view — **L20** (B-296) **Amended 2026-09-12**: served by TWO operations, each with its own run's numbers — `POST /api/acquisition/detect` (the veille: detected / available / grabbed; the plan's « relancer la veille (`POST /api/pipeline/watcher`) » was wrong, that operation is the watcher's on / off) and `GET /api/pipeline/history/{run_uid}` (a pipeline run: per-step counts and reasons, in the history's detail); both **L20**, drawn at `docs/features/maquette-l20/DESIGN.md` |
