...
| **DOIT-4** — « toujours accepter une action légitime, mise en file visible » | `features/arrivals` (a pipeline pass asked for during a run: « Votre passage est **en file** », `fr.json` `arrivals.queuedBold`); the resolve queue; under §20, an arrival that meets the parallelism bound | `served` | served for the pipeline pass: R66 (`arrivals.py:145–166`, reached by TAPPING during a run — « queued, and says so », « not refused: the running pass carries on »), and for the ACCEPTANCE half of every other mutation: **R124** (`harness/busy.py`, L19). The « En file » pastille is DRAWN: `data-part="season/queued"`, on BOTH season surfaces, held in the cache by `features/media/queued-seasons.ts` — **R138** (`harness/queued_ask_mark.py`, 7 holds), which reads the mark AFTER the message has gone, because a sentence shown for four seconds tells the operator who was looking and nobody else. Mutated: removing the mark makes two holds fall with « 0 mark(s) ». L19 had MEASURED its absence and could not draw it — a behaviour change its contract forbade |
