...
| **DOIT-3** — « agir là où l'on observe » | `features/arrivals` (the pilot's bar: run, stop, pause, resume — `data-pipe` lives only in `features/arrivals/page.tsx`) and `/system` (the runs it produced) | `partly` | served: R66 (`harness/arrivals.py` § « the pilot's bar », whose own first paragraph names DOIT-3). **To draw**: « relancer le watcher » and the global levers (the parallelism bound, pause/resume of everything — §20) — **L20**; acting from the journey one is looking at — `requeue` and `rescrape` are uncalled, and a blocked tunnel's RESUME (§20) — **L19**; the tracker policy set from the ratio surface — **L16** |
