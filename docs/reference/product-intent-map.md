...
| **NE-DOIT-PAS-2** — « file ou attente invisible » | `features/arrivals` | `served` | R66 — live-database bound (see DOIT-1): it holds on the operator's machine and not in CI; `GET /api/maintenance/locks`, the operation that says WHO holds the lock behind « En file — pipeline en cours », is uncalled — **L19**. **2026-09-15**: the surface moves to Acquisition at L22 (auditor's lot-order delegation); verdict unchanged until it lands |
