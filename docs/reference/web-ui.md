...
| ------ | -------------------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| POST   | `/run`               | session + XRW | Spawn `personalscraper run --no-console --trigger-reason=web [--dry-run]` (detached). Returns `202 {run_uid, queued}`. `409` only when another PIPELINE run holds the lock (strict duplicate, §6); a MAINTENANCE holder queues the launch visibly instead (`pipeline-queue` row + detached waiter `web/pipeline_queue.py`, `queued: true`). The `run_uid` is injected to the subprocess via `PERSONALSCRAPER_RUN_UID`, so it matches the run's history row. |
...
| GET    | `/api/maintenance/destructive-log`         | session       | `DestructiveLogResponse` — the §7 append-only journal of destructive filesystem ops (overwrite / delete: who/what/when/where/why), newest first. Read-only, fail-soft (empty list on missing table). Written by dispatch REPLACE + disk-clean deletions (`personalscraper/indexer/destructive_journal.py`). The Star City forensic trail.                                                                                                        |
| GET    | `/api/maintenance/actions`                 | session       | `ActionsResponse` — the full action catalog (25 entries) with `category_counts`.                                                                                                                                                                                                                                                                                                                                                                 |
| POST   | `/api/maintenance/actions/{action_id}/run` | session + XRW | `202 {run_uid, queued}`. Body: `{options: {...}, dry_run: bool}`. Errors: `404` unknown action, `422` invalid options, `409` ONLY the strict duplicate (same command + same options + same mode, pid-liveness via `os.kill`) or an unverifiable duplicate check on a destructive action, `428` destructive apply without a fresh (<30 min) matching successful dry-run. A held pipeline lock queues the runner visibly (§6) instead of refusing. |
...
| POST   | `/api/decisions/{id}/resolve` | session + XRW + staging | `202`, `404`, `409`, `410`, `500` | `ResolveResponse {"run_uid"}` — spawns a detached `scrape-resolve` runner (see [runner lifecycle](#scrape-resolve-runner-lifecycle)). Body: `{provider: "tmdb"\|"tvdb", provider_id: int}`. `409` when THIS decision is already resolving or the decision is not `pending` (never for a held pipeline lock — the runner queues, #287). Staging returns `403`. |
...
`409` when a live runner is found — scoped to THIS `decision_id` (the strict
duplicate). Since constitution v2 §6 the maintenance surface follows the same
rule: `_guard_no_duplicate_action` refuses only the same command + same
options + same mode, so a running `scrape-resolve` **no longer blocks**
write/destructive `library-*` actions at the route. Serialization still holds
— `scrape-resolve` keeps `pipeline.lock` for its lifetime, and the
maintenance runner now WAITS in the shared visible queue
(`web/run_queue.py`, `queue` step on its run row) instead of erroring out,
executing when the lock frees.
