...
on one shared element. That asymmetry is the frame's, and the model (`docs/reference/frame-model.md` § 2, part 6)
...
4. **B-231** — the tab bar is rebuilt from scratch on every `render()`: `renderNav()` is called
   ...
   switch and every store bump. A persistent chrome is the first property of `docs/reference/frame-model.md` § 3 and
   it is false today.
