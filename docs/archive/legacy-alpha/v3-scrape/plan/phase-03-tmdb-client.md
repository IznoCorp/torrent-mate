...
- [x] Créer `personalscraper/scraper/tmdb_client.py`
- [x] Implémenter `TMDBClient.__init__(api_key, language)` avec session requests
...
- [x] Méthode privée `_get(endpoint, params)` décorée `@retry` (tenacity) :
  ...
  - Timeout 10s
  - tenacity : `wait_exponential_jitter(initial=0.5, max=10, jitter=0.5)`, `stop_after_attempt(4)`, `reraise=True`
...
- [x] Tests : vérifier qu'un appel basique fonctionne + retry sur mock 429 (24 tests)

**Commit** : `v3.3.1: Implement TMDB client base HTTP with tenacity retry`

### 3.3.2 — Recherche films et séries

- [x] Implémenter `search_movie(title, year=None)` → list[dict]
- [x] Implémenter `search_tv(title, year=None)` → list[dict]
  - ⚠️ Utiliser `first_air_date_year` (pas `year`) pour les séries
- [x] Normalisation du titre avant recherche (strip accents optionnel) — deferred to Phase 5 (confidence scoring)
- [x] ⚠️ `year` booste la pertinence mais N'EXCLUT PAS les autres années (vérifié) — le scoring en phase 5 doit filtrer côté client
- [x] ⚠️ Recherche vide = HTTP 200 avec `results: []` — vérifier `len(results)`, pas le status HTTP
- [x] Tests : 8 mocked tests (search movie/tv, year params, empty results, protocol dispatch)

**Commit** : `v3.3.2: Implement TMDB search for movies and TV shows`

### 3.3.3 — Détails, crédits et IDs (via append_to_response)

- [x] Implémenter `get_movie(movie_id)` → dict avec `append_to_response=credits,images,external_ids,release_dates` et `include_image_language=fr,en,null`
...
- [x] Implémenter `get_tv(tv_id)` → dict avec `append_to_response=aggregate_credits,images,external_ids,content_ratings` et `include_image_language=fr,en,null`
...
- [x] Implémenter `get_tv_season(tv_id, season)` → dict avec `append_to_response=images`
...
- [x] Tests : 11 mocked tests (append_to_response, include_image_language, FR certifications, season episodes)

**Commit** : `v3.3.3: Implement TMDB movie/tv details with append_to_response`

### 3.3.4 — Sélection d'images et URLs

- [x] Implémenter `get_image_url(path, size="original")` → URL complète (`https://image.tmdb.org/t/p/{size}{path}`)
- [x] Implémenter `select_best_image(images, image_type)` → sélection par priorité langue :
...
- [x] ⚠️ Les images sont DÉJÀ dans la réponse de `get_movie()`/`get_tv()` via `append_to_response` — PAS besoin d'endpoints séparés `get_movie_images()`/`get_tv_images()`
- [x] Tests : 10 tests (URL building, language priority, vote tiebreaker, empty list, protocol)
- [x] Test `select_best_image()` : vérifier priorité fr > en > null, tie-breaking par vote_average.
