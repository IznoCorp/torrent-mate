...
6. **P0.6 — Characterization: trailer outcomes.** Create `tests/trailers/test_orchestrator_characterization.py` pinning the current `TrailersOrchestrator.run()` outcome taxonomy as a normalized status map, normalizing paths/timestamps. **Plan-drift correction (verified against the code at P0.6):** the outcomes are NOT `found/placed/skipped/failed/cooldown/no-match`. The actual `item_results` status vocabulary is the six values `downloaded / already_present / no_trailer / bot_detected / error / skipped`, refined by ten `reason` codes (`skipped_by_state`, `skipped_by_filter`, `already_present`, `already_present_on_disk`, `no_trailer`, `circuit_open`, `http_error`, `ytdlp_error`, `bot_detected`, `downloaded`) plus an asymmetry: a generic finder exception increments `counts["error"]` and writes an `HTTP_ERROR` state but appends NO `item_results` entry (only the `CircuitOpenError` branch does). Each pinned outcome asserts the reported `(status, reason)`, the persisted `TrailerState` (status + attempts + whether a `next_retry_at` cooldown was written — `BOT_DETECTED` writes none), the filesystem effect (Plex-conformant placement, pinned for both movie-flat and TV-`Trailers/` families), and the download-only indexer outbox publish + `TrailerDownloaded` bus emit. Verify: `pytest tests/trailers/test_orchestrator_characterization.py -q` green.
