...
**Requires Phase 5:**
`pytest tests/conf/test_tracker_economy_integration.py` → `4 passed`

---

## Goal

Create `docs/features/tracker-economy/ACCEPTANCE.md` (executable ACC-NN shell commands), run all criteria, run `make check`, smoke-test import, commit.

No source code changes in this phase.

## Files

- **Create:** `docs/features/tracker-economy/ACCEPTANCE.md`

---

## Tasks

### Task 6.1 — Create `ACCEPTANCE.md`
...
- [ ] Verify: `ls -la docs/features/tracker-economy/ACCEPTANCE.md` → file exists

---

### Task 6.2 — Run all ACCEPTANCE criteria

- [ ] ACC-01: write to `/tmp/acc01_te.py` then `python /tmp/acc01_te.py` → both OK lines
- [ ] ACC-02: run the `python - <<'EOF'` block → `ACC-02 OK`
- [ ] ACC-03: run the `python - <<'EOF'` block → both OK lines
- [ ] ACC-04: `make check` → exit 0
- [ ] ACC-05: `python -c "import personalscraper; print('ACC-05 OK')"` → `ACC-05 OK`

If any criterion fails, fix the underlying source (no shortcuts) and re-run.
...
```bash
git add docs/features/tracker-economy/ACCEPTANCE.md
git commit -m "docs(tracker-economy): ACCEPTANCE.md + make check gate"
```
...
- [ ] All ACC-01 through ACC-05 pass
- [ ] `make check` exits 0
- [ ] `python -c "import personalscraper"` exits 0
- [ ] `git status` on `feat/tracker-economy` is clean
- [ ] Commit SHA recorded
