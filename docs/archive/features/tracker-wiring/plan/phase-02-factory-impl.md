# Phase 2 — Factory: `build_tracker_registry` implementation
...
```bash
python -c "from personalscraper.api.tracker._errors import TrackerConfigError; print('ok')"
# Expected: ok
```
...
Create `personalscraper/api/tracker/_factory.py`. The factory iterates
`tracker_config.providers`, skips disabled trackers, resolves API keys from
`env`, builds `HttpTransport` + client per enabled tracker, aggregates
validation issues (missing_credentials, unknown_provider, protocol_mismatch,
disabled_in_priority), raises `TrackerConfigError` on any error, logs warnings,
and returns a populated `TrackerRegistry`. No `api/metadata` import anywhere.

---

## Files

- **Create:** `personalscraper/api/tracker/_factory.py`

---

## Tasks

### Task 2.1 — Create `_factory.py`
...
def build_tracker_registry(
    ...
    *,
    ...
    Args:
        ...
        cb_policy: Circuit-breaker policy shared across all tracker transports.
    ...
    for name, provider_cfg in tracker_config.providers.items():
        ...
        if missing:
            issues.append(TrackerConfigIssue(
                severity="error", code="missing_credentials", provider=name,
        ...
        if name not in _TRACKER_CLASSES:
            issues.append(TrackerConfigIssue(
                severity="error", code="unknown_provider", provider=name,
        ...
        transport = HttpTransport(client_cls.policy(api_key), event_bus=event_bus)
        client = client_cls(transport)

        if not isinstance(client, TorrentSearchable):
            issues.append(TrackerConfigIssue(
                severity="error", code="protocol_mismatch", provider=name,
    ...
    if built:
        ...
        for name in sorted(priority_names & disabled_names & known_providers):
            issues.append(TrackerConfigIssue(
                severity="warning", code="disabled_in_priority", provider=name,
    ...
    for issue in issues:
        log.warning("tracker_boot_warning", code=issue.code,
                    provider=issue.provider, message=issue.message)
...
- [ ] **Verify no api/metadata import:**
  ```bash
  rg "api.metadata" personalscraper/api/tracker/_factory.py --type py
...
git commit -m "feat(tracker-wiring): build_tracker_registry factory"
```
...
- [ ] Smoke import → `ok`
- [ ] Zero `api.metadata` matches in `_factory.py`
