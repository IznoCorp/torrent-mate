# Phase 02b — Implement `library-dedup-titles`

## Gate

Phase 02a must be committed:

- `tests/integration/test_dedup_titles.py` — 4 tests exist and are **red**
  (command not found).
- `make test` collection must not error (tests skip or fail, not crash).

## Goal

Create `personalscraper/commands/library/dedup_titles.py` and make the 4 red
tests green.

## Files

- Create: `personalscraper/commands/library/dedup_titles.py`

---

### Sub-phase 2.2 — Implement the command (green)

**Commit:** `feat(nfc-dedup): library-dedup-titles command`

- [ ] **Step 1: Create `personalscraper/commands/library/dedup_titles.py`**
...
def library_dedup_titles(
    ...
    try:
        ...
        all_rows = conn.execute(
            "SELECT id, title, kind, year, date_metadata_refreshed FROM media_item"
        ).fetchall()
        ...
        for _key, members in groups.items():
            if len(members) == 1:
                ...
                if _is_nfd(raw_title):
                    to_normalize.append((_unicodedata.normalize("NFC", raw_title), int(sole["id"])))  # type: ignore[arg-type]
                continue

            paths = {int(m["id"]): _get_dispatch_path(conn, int(m["id"])) for m in members}  # type: ignore[arg-type]
            ...
            to_delete.extend(int(m["id"]) for m in members if int(m["id"]) != survivor_id)  # type: ignore[arg-type]
        ...
        emit(stats.snapshot().to_cli_json(apply=apply))  # type: ignore[arg-type]
    finally:
        conn.close()
```
...
```bash
make test 2>&1 | tail -3 && make lint 2>&1 | tail -5
```

Expected: `NNNN passed`, zero lint errors.

- [ ] **Step 4: Module size check**

```bash
python3 scripts/check-module-size.py personalscraper/commands/library/dedup_titles.py
```

Expected: under 800 non-blank LOC.

- [ ] **Step 5: Commit**

```bash
git add personalscraper/commands/library/dedup_titles.py
git commit -m "$(cat <<'EOF'
feat(nfc-dedup): library-dedup-titles command

One-shot dry-run/apply command that groups media_item rows by
NFC-canonical key and deletes NFD orphan twins (ON DELETE CASCADE).
Guards against false merges by requiring identical dispatch_path.
Also NFC-normalizes NFD-titled non-duplicate rows. 4 tests green.
EOF
)"
```
