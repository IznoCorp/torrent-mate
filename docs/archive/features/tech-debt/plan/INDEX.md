...
| 8    | Polish + Plan A reset + size hard-block + bonus                                                                                                            | phase-08-polish.md                  | 3-4 j   | [x] gate `c8f20dd` (15/15 sub-phases DONE; 8.10 PARTIAL at gate, COMPLETED post-gate via 8.10.b/c/d Plan A retry track — 91.3% external_ids ✅)                                                                                                                                                                                                                                                                                                                                                            |
| 9    | CLI Test Coverage (NEW — absorbe 8.7 SH-25 ; **20/20 library + 7/7 pipeline + 4/4 trailers/config + 4/4 non-library + 20/20 harmonized**)                  | phase-09-cli-coverage.md            | 1.5-2 j | [x] gate `89f4a34` (all sub-phases DONE: 9.1 helpers+pin, 9.2.abc pipeline E2E 85 tests, 9.5 trailers/config 47 tests, 9.6 non-library 17 tests, 9.7.a 4 shards harmonized 20 harnesses +104 tests, 9.7.b matrix doc + script + Makefile, 9.7.b/fix close 22 ❌ to ACC-50/53/54 ✅. Total +261 tests Phase 9.)                                                                                                                                                                                             |
| 10   | Archive DESIGN.md updates (**8 features** — arch-cleanup added)                                                                                            | phase-10-archive-docs.md            | 1-2 j   | [x] all sub-phases DONE. 10.1/1 (event-bus/provider-ids/media-indexer banners), 10.1/2 (pipeline-obs/trailer/logging/legacy-cleanup banners; logging banner was MISSED in earlier `329afbc`), 10.2.a (\_exclusions.py docstring placeholder), 10.2.b (VX leak sweep: 5 alpha refs archived + 4 inline rewrites), 10.3 (arch-cleanup 8th banner + ACC-49 ✅), 10.4 (HANDOVER.md deleted + IMPL cleanup). Gate: <this commit>. 8/8 archive banners verified.                                                 |
| 11   | Aggregated DeepSeek 5-angle review fixes (18 findings — durability fsync, stats ordering, AppleDouble, pytest.raises narrow, regex, decomposition, polish) | phase-11-review-fixes.md            | 2-3 j   | [x] all 7 sub-phases DONE; 25 commits; 5432 tests pass. Module decomposition: scan() 776→313 LOC, tv*service.py 998→832 non-blank LOC. Sanity sweeps clean (0 pytest.raises(Exception), 1 inline startswith('.*') = canonical).                                                                                                                                                                                                                                                                            |
| 12   | Pipeline-monitor findings fixes (12 deviations from 2026-05-25 run)                                                                                        | phase-12-pipeline-monitor-fixes.md  | 2-3 j   | [x] gate `1fe616e` — all 9 sub-phases DONE + 12.10-12.14 follow-ups (season triggers, orphan repair episode-level, variance regression, torrents-list telemetry restore, commands.md entries). Final `make test`: 5499 passed, 0 failed.                                                                                                                                                                                                                                                                   |
| 13   | Design-smell cleanups from PR review (5 deferred + 4 post-gate follow-ups)                                                                                 | (no formal phase file)              | 0.5 j   | [x] gate `4c9ef59` — 5 sub-phases DONE: 13.1 extract drift_persistence (`0e6d052`), 13.2 DriftIssueStore (`64ccd4b`), 13.3 CliFixStatsMixin (`a58b64d`), 13.4 RestoreOutcome sum type (`cedbe16`), 13.5 command_with_telemetry (`67e5bf0`). 4 post-gate follow-ups: 13.6 drop ScraperExternalIds shims (`a714f23`), 13.7 drop trailers verify alias (`f8185b4`), 13.8 DriftIssueStore log missing DB (`0916232`), 13.9 propagate OmdbQuotaExhausted (`ae9f10a`). Final `make test`: 5509 passed, 0 failed. |
| 14   | Pipeline-monitor reopen + CI cleanup (re-run 2026-05-25 23h49 — 11 sub-phases : 2 persist + 2 verify + 1 requalif + 5 new + 1 CI)                          | phase-14-pipeline-monitor-reopen.md | 2-3 j   | [x] gate `5a8e815` — 10/11 DONE, 14.6 DEFERRED (1730 `.actors/` dirs = 60× planned scope). 14.1 canonical_provider source-block + 194 inversions repaired; 14.2 NO_OP (env complete); 14.3 NO_OP (DESIGN_CONFORM); 14.4 CASCADE migration 009 + 102 orphans purged; 14.5 NO_OP (drift=0); 14.7 NO_OP (enqueued_at); 14.8 172 release_orphans purged; 14.9 agents prompts + 14.10 matrix v2.2 (`.claude/` repo); 14.11 CI 3.12-only static job. Final `make test`: 5515 passed, 0 failed.                   |
| 15   | CI Speed Optimization (cache venv + concurrency + split test + gitleaks cache + paths-ignore)                                                              | phase-15-ci-speed.md                | 0.5-1 j | [ ] DESIGN APPROVED 2026-05-26. 6 sub-phases: structural wins (15.1), cache .venv (15.2), gitleaks cache (15.3), split test + coverage merge (15.4), ACC criteria (15.5), gate (15.6).                                                                                                                                                                                                                                                                                                                     |
