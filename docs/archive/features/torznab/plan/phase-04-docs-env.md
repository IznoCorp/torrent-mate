...
| `.env.example`                   | audit COMPLET du delta .env → .env.example : chaque clé réelle présente (noms seulement, valeurs vides/factices), TORR9_* conservées mais `# DEPRECATED (tracker closed 2026-07, replaced by tr4ker)`, TR4KER_PASSKEY documentée (convention opérateur : variable unique, valeur envoyée en apikey= ; si Tr4ker rend « error 100 Invalid API Key » en phase 5, c'est la clé API du profil qu'il faut y mettre), TR4KER_ANNOUNCE_URL/API_URL documentées comme notes non consommées par le code — ou retirées si l'opérateur n'en veut pas (choix : les garder en commentaire descriptif) |
...
- **AUCUN secret** : `rg -n "<passkey réelle du brut>" docs/ .env.example` → 0 avant commit.
  Idem pour toute autre clé apparaissant dans le brut.
- `.env` réel : JAMAIS touché par cette phase (c'est le fichier de l'opérateur). La
  convention TR4KER_PASSKEY est déjà en place côté opérateur (2026-07-28).
