...
| **Total** | **149** | 130 by the waves through L11's own five, plus the four readers' 13 recounted above — at 2026-08-31, after L10-bis, its review, L10-ter's survey and its review, **L15 with its own review**, and **L11 with FOUR readers**. **L11's FIVE are the mutation half of the curve, and they say what mutation IS good for**: every one was found by breaking the wave's own rules on purpose, and two of them are the same rule going green twice over a shell that was not there. A rule can hold a CONSEQUENCE while the mechanism it names is gone, and only a mutation asks. **What mutation could not reach is the other forty**: four readers on that same green gate found a security regression, an update discipline that never swapped the worker, and a queue a refused replay jammed forever — none of them a defect the author had thought to break. The ratio is this table's oldest argument, arriving on a wave that wrote BEHAVIOUR rather than conversions: 5 by mutation, some 40 by reading. **And a second round on the REPAIRS found thirteen more, then a third found seven** — the two later rounds are where the sharpest findings are, not the first. Round two: the repair for a jammed queue had made a refused mutation vanish SILENTLY instead, and its classifier destroyed the operator's action on a 503, a 429 and a 401 — the very outage the queue exists for. Round three: that repair's own replacement still treated **401 as final**, so an expired session destroyed every queued mutation one after another when a re-login would have saved them all; and the notice's button said « Réessayer maintenant » while doing something else, because its name, its words and its action were written as three ladders that tested their conditions in different orders. **Each round's worst finding was in the previous round's repair**, and none of the three was found by a gate. |
| **L12** (#540, this wave) | **14** | **Recounted at the wave's close, and it more than doubled** — the first count of six was taken before the operator's late decisions, and every one of the eight added came out of building them. None was found by reading. **THE SHARPEST IS A RULE WRITTEN FOR THE VERY DEFECT IT THEN MISSED**: R115 held that the tab bar had a `::view-transition-group` of its own, and its own stated principle — « the hold is the GROUP, not a pixel » — was what blinded it. The bar's group existed and the bar was still painted UNDER the arriving screen; the operator saw it, the steward filmed it, and the rule stayed green one commit after being written for it. **The page transition was DEGENERATE for six phases** and R115 was green throughout, correctly by its own words: it counted animations, and animations ran — « a transition is RUNNING » and « a transition SHOWS THE PREVIOUS STATE » are different claims and only the first was held; proved by a name existing on one side only. **A fix that could not work**: `:active-view-transition` was used to silence the hero's second entry, and by the moment that animation starts the transition is over and the selector no longer matches. **A comparison whose two arms were the same comparison**: the transition option's selector carried a descendant combinator where the pseudo-elements hang off the root, so it produced timings identical to the default — which reads as « an option nobody needs » rather than one that never applied. **A hold driven against a read that was never slow**: `page.route` intercepts ZERO here because the mocks answer in the page, so the priming hold proved nothing until it drove the mock layer's own latency. **A discriminator that answered the same on both sides**: the artwork's cached-versus-fetched test raced `requestAnimationFrame` against `decode()` and marked `faded` either way. **A probe that truncated its own evidence** to eight rows of ten and reported a defect that was not there — the mirror of the guard that reports none that is. And **B-269's species in an INSTRUMENT, twice in one rule**: R55's 420 ms between press surfaces and its 340 ms after a close, both hand-set against durations that were later redrawn, neither wrong when written. Plus the original six — B-272, B-274, the viewport guard's floors counted after its own summary printed « 0 violation(s) », and three holds of this wave's own making that could not fail |
