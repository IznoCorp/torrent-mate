...
| B-295 | React 19 assigns `innerHTML` on the prop OBJECT's identity, string unchanged or not — so every re-render of a page subscribed to the store's version recreates its engine-drawn children, at nine sites                        | by L14                | `open`       |
...
**Owner: L14, phase 7.** One `ui/` helper returns the same `{ __html }` object for as long as the
string is unchanged — the comparison React 18 made — and every site goes through it; R100 gains
the hold that reads `isSameNode` on the page's own nodes across a bump.
