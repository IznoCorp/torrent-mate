...
| B-283 | During priming, the media screen prints its UNKNOWN parts as answers — « aucun synopsis », « aucune distribution » — about data in flight (§13) | by L12 review | `fixed #547` |
...
| B-302 | The journey sheet is the tunnel and offers neither « Remettre en file » nor « Re-scraper »; both operations are uncalled | by survey | `fixed #572` |
...
| B-379 | The mock layer answers **200** to every call unless a scenario arms a failure (`mocks/scenario.ts:145`), whatever code the contract declares — `grabSeasonForFollow` declares 201, `requeueJourney` and `rescrapeJourney` 202 — so no rule can hold a declared success code, and a sentence saying the layer « answers 201 » is false on the code | by L21 | `fixed #592` |
| B-380 | The season grab's `absorbedCount` is derived from `seasons.json` while the media sheet draws its season rows from the sheet's own catalogue (`media-sheets.json`): two families at one title that disagree on 13 of the 49 seasons both know, so on « Les Animaniacs »' sheet the row « Saison 5 · 0/23 · 23 manquants » offers the act and the answer says « aucun épisode à récupérer » | by L21 | `fixed #592` |
...
| B-383 | Four verbs SAY a sentence and send nothing to the layer: « Re-scraper les métadonnées » on the follow panel, « Remplacer la valeur » on a secret, « Lancer à blanc » on a maintenance action and « Lancer la veille maintenant » in the « more » menu each answer a canned sentence over an unchanged world — the « said, not done » class, identical on round one's candidate | by L21 review round two | `fixed #592` |
...
| B-460 | R161's h2 floor is green over an affordance made invisible by `opacity: 0`, a clip or an off-screen transform — it reads a box above zero and `visibility`, two of the five ways to take a mark away | by review | `open` |
| B-461 | `resolution_window.py`'s `LAST_FRAME = 6500` is called « the message's last reachable frame » while the message's opacity is 0 there, so only a SHRUNK window is armed — a message outliving the window passes all eighteen holds | by review | `open` |
...
| B-473 | `lib/verbs.ts` exports `registeredVerbNames` « for the rule that reads the seam from outside » and NOTHING calls it — not the shell, not a rule, not a test: a seam nothing reaches is a seam that does not exist, and the first rule that wanted it found nothing to ask | by the mock-layer micro-wave | `fixed #592` |
...
| B-497 | `scripts/build-mock-seeds.py --check` counts every converted family's seed as « no family claims it », so the tool's own drift report is red on a tree with no drift — closes: `build-mock-seeds.py` died with the builder at r·17 (ruling 114), its subject gone | by L13a | `fixed #605` |
...
| B-500 | The `card/pick` check mark on every resolution candidate card reads as « already selected »: nothing says the card is tappable to CHOOSE — closes: the « Choisir » pill built in #598 stands as the fix; the operator's Q4 ruling (2026-09-15, via the auditor) settles that a marked card during the resolution window is NOT built — the message « Identifié comme … · Annuler » names the choice for the 7 s instead | by operator | `fixed #598` |
...
| B-547 | `test_maintenance_panels.py::TestLocksRoute::test_locks_tmp_orphans` reads a background sweep before it has run under load: it creates three `_tmp_dispatch_*` dirs and asserts `len(data["sweep"]["orphans"]) == 3`, and under three pytest-xdist workers the route answered before the sweep ran — 0 read, not 3. 1 fall in 11 402 on the L13r docs pull request's pre-push run, 2026-09-16 00:2x; mechanism: the test awaits nothing, the sweep is documented as background. Same test as B-033's open flake under xdist. Owner none | the docs pull request's pre-push run | `open` |
...
**Closes at L13r r·17 (ruling 114, #605).** `build-mock-seeds.py` — the `--check` arm named above along with the
rest of the builder — died with the builder: correspondence (`build`/`rejoined`/`join`) and the projecting
functions (`projected_seeds`/`projection_for`/`seed_of`/`project`/`regroup`) are gone, `check-mock-seeds.py` keeps
only its three live local reads (`SEEDED_CLASSES`, `converted_families`, `file_for`). The tool `--check` complained
about no longer exists; its subject died with it.
...
**Closes `fixed #598`.** Operator ruling, Q4 = A (2026-09-15 ~20:3x, via the auditor): the resolution screen
closes on the pick as today; the message « Identifié comme … · Annuler » names the choice for the 7 s window;
the marked-card-during-the-window half above is **not built**, ruled so rather than left open. The pill shipped
in #598 is the whole of the fix.
...
described in `docs/features/maquette-l21/RESUME.md@2ffdc4ba`; this entry records only the defect as it stands
...
tier): 15 holds, no violation — one operation per declared family, what `answered()` records against what
the response carried, the corpus of non-200 operations, and the latency dial. **Mutation**:
`const success = 200` → 4 violations, naming the three declared codes and the latency dial.
...
**Split between three owners** (the operator's ruling of 2026-09-12, and the orchestrator's). This entry's
row closes on the half the mock-layer micro-wave owns; the other two verbs are not orphaned, they are
owned elsewhere:

| Verb | Owner | State |
| --- | --- | --- |
| « Re-scraper les métadonnées » on the follow panel (and its twin on the media sheet, B-470) | the mock-layer micro-wave | fixed #592 — `rescrapeMedia`, R171 |
| « Lancer à blanc » on a maintenance action | the mock-layer micro-wave | fixed #592 — `runMaintenanceAction` with `dryRun`, R171 |
| « Remplacer la valeur » on a secret | the settings wave | B-334, #588 |
| « Lancer la veille maintenant » in the « more » menu | L20 — it is DOIT-6's operation, `POST /api/acquisition/detect` | L20's plan |
...
**B-465 — a comment in the dying engine still names the empty function as the painter.**

`frontend/maquette/design/src/engine/legacy.js:9344` says « **paintSelBar() below draws the bar
directly, not through render()** », four lines above the call to `paintSelBar()`, which is
`function paintSelBar() {}` at `engine/legacy.js:7788`. React draws the bar
(`features/library/selection-bar.tsx`), which is what this wave's own repair wrote — truly, clause
by clause — into `features/library/page.tsx:16-27`.

**OLD, and not charged to that repair**: the sentence is identical on the wave's control head, and
`engine/` is not the file the repair named. It is **B-392's own species surviving one file away**,
in the file that holds the seam — the sentence a reader uses to judge who owns that node.
**Closes when** the comment says who draws the bar, **or it dies with the engine**. Owner: L13, or
...
card at its index while A's send still leaves, and the put-back on a list a third settled folder really
moved. **Mutation**: S.W.A.T.'s candidates emptied → 5 violations, the first naming it: « the second
folder is picked INSIDE the first one's window ». Navigation goes through `window.__screens.resolution`,
because no finger reaches a named folder's arbitration (B-474).
...
**B-301 — a season is printed « à récupérer » and nothing lets one take it.**

**DELIVERED, AND READ.** The seasons panel offers « Récupérer cette saison » on a season whose
owned count is short of what aired, and the verb calls
`POST /api/acquisition/follows/{followedId}/seasons/{season}/grab` — the operation the maquette
had never called. **R125** (`harness/season_grab.py`), 16 holds.

**RED FIRST AGAINST `main`, WITH NO MUTATION**, because the rule was written before the verb
existed — the strongest form this repository asks for. It walks the only subject the fixture has
(« Silo », season 3, six of seven aired episodes held), chosen from `window.SEASONS` BEFORE a
finger moves, so a failure is about the interface and never about the walk; it raises the panel
by a FINGER rather than through the producer's seam; and it reads the call on the NETWORK, path
included, so a build that toasts and sends nothing fails — which is exactly what
« Récupérer maintenant » did for a whole wave under a green gate (B-309).

<sub>the original report follows</sub>

`features/media/panel-
...
**B-302 — the journey offers no way to resume a tunnel.**

**DELIVERED, AND READ.** The journey sheet offers « Remettre en file » and « Re-scraper », told
apart by LABEL, and each calls its own operation —
`POST /api/acquisition/journeys/{infoHash}/requeue` and `/rescrape`. **R126**
(`harness/journey_verbs.py`), 16 holds. §20's « reprend là où il s'est arrêté, par l'opérateur »
is served from the surface the operator is looking at.

**READ ON THE LAYER'S ANSWER, never on the screen and never on the network**: the mock layer
replaces `globalThis.fetch`, so a mocked call raises no request event at all and a rule watching
for one would have been green over nothing. The stages are read the same way, because the producer
takes them from the query cache — a rule asserting a hard-coded list would pass a build that
called the operation and threw the answer away.

**« Re-scraper » here is NOT the media sheet's metadata re-scrape.** The resources already hold
« Re-scraper les métadonnées » twice; this verb re-runs the tunnel's scrape for one tracked staging
item, and the rule reads the JOURNEY panel's own action rather than matching the word in the tree.

<sub>the original report follows</sub>

The journey sheet is the tunnel seen by
...
for byte; drifted in tone, it would have reported about a list the surface never touched. Taking the
...
<sub>`python3 frontend/maquette/harness/machine.py` → « PASS as many schedulers drawn as PM2 schedules — 7 drawn vs 7 real · paired 7: [… `personalscraper-index-full = Analyse complète de l'index` …] · scheduled and not drawn: [] · drawn and not scheduled: [] », **91 rules**, no violation — the reading is a JOIN since round four, not two lists side by side: sorted separately they sort on different keys, and a reader pairing them by position mispaired six of seven · the mutation, `scripts/mutate.sh` on the seed with the seventh row removed → « FAIL … 6 drawn vs 7 real » · the derivation's own mutation, `fault.test.ts` with the key put back to the row's RANK → 4 of 10 red, the three « reversed » holds and the no-match one; and on the harness side, the seed's first label renamed → « and on the schedulers side — [] » with two more · `pm2 jlist` → 7 processes carrying a `cron_restart`</sub>
...
**REPORTED AGAIN by the operator on 2026-09-06** (« le bug de clignotement du panel bottom à
l'ouverture d'une fiche média est toujours là ») — the second time, which this register counts as its
own failure: the entry above carried the verbatim and no mechanism, so nothing could be said back.

**THE MECHANISM, MEASURED by the steward on 2026-09-06** — a build of `ec3a978ef` (identical to `main`
for `app/` and `base.css`), 390 × 844, one reading per animation frame from the tap on the follow
panel's « Voir la fiche » (the steward's probe `b310_probe.py`, readings `b310-head.json`):

    42 ms   the view transition starts and captures the panel OPEN under the name `leaving-panel`
    75 ms   the media screen is present; `::view-transition-old(root)` / `-new(root)` cross-fade over 300 ms
            (`--duration-3`); `::view-transition-old(leaving-panel)` plays `panel-down` over 450 ms
            (`--duration-4`) at z-index 20, ABOVE the root; `screen-body` rises and `screen-banner`
            comes in over 450 ms
    524 ms  the transition ends; the live `#sheet` was at `top: 844` (off-screen) from 75 ms on

So the panel is neither re-opened nor left behind: **its captured snapshot slides down OVER the media
screen for the whole crossing and 150 ms past it** — the sheet is fully in at 375 ms and the panel is
still leaving until 525 ms. That IS the departure L12 drew on 2026-08-31 (`app/panel-host.ts:126`,
`base.css` « ::view-transition-old(leaving-panel) »), and L19's round one read it identical on head
and control (« the panel never finished leaving »). Under `prefers-reduced-motion: reduce` nothing
animates and the effect does not exist; the live `#sheet` then stays `visibility: visible` off-screen
for 478 ms, harmless.

**NOT A CODE DEFECT — A DESIGN AMENDMENT, and it is the operator's.** Three readings, each a line or
two of `base.css`: (A) **align** — `panel-down` at `--duration-3`, so the panel is gone when the
cross-fade ends and nothing covers the sheet once it is in; (B) **cut** — the panel is not captured,
the sheet simply cross-fades in; (C) **sequence** — the panel departs first, the screen arrives after
(about 450 ms slower, the wait the engine's 260 ms stood in for). **Owner: the wave that touches the
frame's transitions (`app/`, `base.css`) — L13, or a micro-wave on the operator's word.** The rule
that lands with it reads the frames, as this probe did: the panel's snapshot must not be drawn over
a media screen that is already fully in.
...
**FIXED, AND THE RULE THIS ENTRY ASKED FOR NOW EXISTS.** The secondary journey action carries the
guard « Voir la fiche » already had beside it — it is offered unless it is ALREADY the primary,
which is the case for a medium with no sheet that nothing is chasing. One condition, on the file
the wave had already opened.

**R139** (`harness/panel_label_once.py`), 3 holds, is not written around the producer that was
wrong: it raises EVERY panel a finger can reach on `arr-queued` and `acq-follows-list` and refuses
a repeated label in any of them — the defect's family, which is what this entry's own body asked
for (« nothing counts a panel's actions by label »).
...
**Three things the rule is anchored on, each because the obvious alternative is green over the
screenshot that opened this entry:**

- **It counts LABELS, not destinations.** Both of the operator's two buttons carried the SAME
  destination, so a rule reading destinations would have passed the very defect.
- **The subject hold reads `data-journey`**, the action's own destination — never the French word.
  An anchor on the copy stops measuring the day the copy is retouched, and says « no violation »
  while doing it.
...
**(a) WAS REOPENED BY THE OPERATOR AFTER THAT READING, and the second report is the one that
matters**: « le bouton est toujours beaucoup trop gros, pourquoi pas un bouton de la même taille
que les autres ? les boutons de l'interface doivent être des composants qui n'autorisent pas toutes
les tailles, l'icône et le bouton sont énormes ».

**AND HE WAS DESCRIBING A DEFECT, NOT A SCALE.** Measured before anything was changed: the button
rendered **332 x 245** with a **227 x 227** icon in it, on a screen whose other controls are 24 to
44 px tall. `svgIcon` emits an `<svg>` with no width and no height, and `base.css` already records
what happens then, at its `.installkey` rule — « it falls back to the replaced-element default of
300x150, and the « add » button came out 313px tall ». The action-button system escapes it because
`.sact / .cfoot / .mediaadd svg` size its icons by descendant rule; this button wears none of those
classes, so nothing sized its icon. **R136's four holds read a type step and a padding step and
passed all of it** — the instrument was green over the very property being reported.

**BOTH HALVES OF THE SECOND RULING ARE DELIVERED.** The size is now a bounded set on the shared
component — `actionButton`'s `size`, `screen` or `footer`, each branch a string literal so
`residue.py` keeps reading it — so a size outside the set is a compile error and no surface can
hand a button one; all 19 `actionButton()` call sites across 9 files pass no argument and their
emitted class set is unchanged, held token for token by `variants.test.ts`. And the `footer` branch
carries the ICON's size, because a button that wears no legacy class cannot rely on the residue to
size what it contains. **The height is settled at 40 px with a 16 px icon — the operator's own
answer, not a provisional value** — and it is held by R136's fifth hold, which compares the
RENDERED BOX to the action-button system's own, measured on a screen that draws one, with no figure
typed for either. Green at 40 against 44; the mutation emptying the icon's size hook brings back
`245.328125` tall with a `227.328125` icon and fells only that hold.

**One thing said rather than left to be discovered**: the footer size carries no 44 px floor, so
the box is shorter than the action-button system's. It clears the 24 px target minimum — and it IS
smaller under a thumb, which is the trade the operator made knowingly when he asked for the other
buttons' size. **Width is deliberately unchanged**: a footer action is full width here, as the
retry beside it is, and what was reported was 245 px of height.

First reported by the operator on 2026-09-05: « Dans Découvrir, le bouton "charger plus" est trop gros,
...
**AND A FOURTH THING, found in round one's walk on 2026-09-05: the button is reachable from no
named state, so (a) cannot be walked at all.** `[data-sugmore]` is absent from
`acq-discover-deck` and from `acq-discover-exhausted`; switching to the deck gives three cards and
no empty state; forcing `__store.write({sugOrder: []})` still leaves three cards and no button.
The list and poster modes have no such button at all. Identical on both builds. **The oracle's
« no divergence » above is therefore about the SURFACES, not about this button** — a region root
the oracle measures is not the same thing as a control a finger can reach, and (a)'s reading
stands on the oracle alone. Whoever amends the size will first have to give the empty state a
named state, or the amendment lands with nothing measuring it.
...
**B-316 — the suggestion producer is registered and no finger reaches it.**
`window.__panel.producers()` lists `suggestion` and, produced directly, the panel is right:
« Ajouter / Voir la fiche / Pas intéressé », plus §5's « ce film quittera automatiquement votre
liste ». **There is no door.** On the poster tile `data-panel="sug:N"` and `data-mediasheet` sit on
the SAME node; on the deck card the media verb is on its child. Five points down each card — 8 %,
25 %, 50 %, 75 % and 92 % of its height — tapped with a finger: the media SCREEN opens every time,
`panelOpen: false` at all ten points.

**Pre-existing, and read on both builds.** `suggestion-deck` and `suggestion-poster-tile` read
byte-identical on the head and on a control of `4c0e274a7`, so L19 moved a producer that was
already unreachable rather than making it so. The producer is right and only its path is missing,
which is why the conversion's oracle could not see it: the oracle measures a surface at rest, and
this is a question about where a finger lands.
...
`legacy.js` was DELETED, and the rubric PUSHES its arrival: the entry carries the rubric's own name,
because `lib/addresses.ts` declares a `topic` dial for Maintenance and none for Configuration, so
the entry's STATE is where it travels and a pop reads it back. `TopicView` draws
`data-part="screen/back"` and it POPS — `window.__bridge.back()`, the affordance every screen of
this interface wears — and the secrets rubric wears it too.

    the rule            R165, `harness/topics.py`, 16 holds — a real load at the address, a real
                        click on a rubric row, and THREE readings rather than one: the entry
                        pushed (`history.length` + 1), the back affordance drawn, and where
                        `history.back()` lands. A hold on the first alone is green over a rubric
                        that pushes and draws nothing; a hold on the second alone is green over a
                        button that pops the wrong entry
    seen RED first      against the tree as it stood, with no mutation needed — the engine's branch
...
the rubric on ANY pop — so the first Back that shut a panel opened over a rubric also shut the
rubric under it, and a reader who had just filed an edit landed on the list. A layer's entry has no
opinion about what it is laid over: the reader ignores one.
...
**FIXED #588 (the settings micro-wave).** The action carries a registered verb
(`features/settings/secret-verbs.ts`), the panel gained somewhere to TYPE a key
(`panel-secret-field.tsx`, a `password` input with every browser assistance off — a key in an
autofill store is a key in one more place), and the tap writes it through the layer. The value is
read at the moment of the tap and held nowhere else, so a secret exists in one place between the
keyboard and the write. An EMPTY field is refused rather than sent: the layer clears a key on an
empty value, so sending one would be the destructive act by the harmless door. A write the outbox
HELD says so and does not say « remplacée ».

    the rule            R167, `harness/secret_acts.py` — it counts the LAYER's write
                        (`window.__mocks.answered()`), never the message: a message is what the
                        defect already produced, so counting it would certify the defect
    seen RED first      against the tree as it stood: « offers somewhere to PUT a new key » false,
                        « is an act, not a sentence » false, « WRITES the key through the layer »
                        `[]`
    the run             `python3 frontend/maquette/harness/secret_acts.py` → 14 holds, no violation
...
**FIXED #588 (the settings micro-wave), in the operator's own words.** « Retirer la clé
<fournisseur> ? » / « La clé sera retirée pour tous les comptes du foyer. Ce fournisseur ne
répondra plus tant qu'une nouvelle clé n'est pas saisie. » / « Annuler » · « Retirer la clé »
(danger, right), and « Clé retirée. » after confirming. The provider is named from the LAYER and
never from the panel's heading: the question names what stops answering, and reading it off the
screen would make the sentence depend on how the panel happens to be drawn.

    the rule            R167, `harness/secret_acts.py`, and THE WALK GOES THROUGH THE CANCEL —
                        the half that separates a confirmation from a delay. Cancelling: the key
                        still posed, and `updateSecrets` called ZERO times. Confirming: the key
                        cleared, through the layer
    seen RED first      against the tree as it stood: « is an act, not a sentence » false, « it
                        ASKS before it cuts » None
    the run             `python3 frontend/maquette/harness/secret_acts.py` → 14 holds, no violation
...
**FIXED #588 (the settings micro-wave).** « Valider » is an action of the field's panel, with a
registered verb that reads the input where it is drawn and files the edit on the tap. THE NATIVE
PATH STAYS, unchanged and for its three measured reasons — this is a second way to one act, never
a replacement — and the button is offered for the kinds one TYPES into: a switch and a list file
their edit on the tap that changes them, so a « Valider » there would file what is already filed.
The two labels say what they are: « Nouvelle valeur » for the pending one and « Valeur
enregistrée » for the file's, and the first line only calls itself new when there IS something new.

    the rule            R166, `harness/settings_editing.py` — a real keyboard, one tap, and the
                        NATIVE COMMIT PATH CUT while the button is measured. That last part is
                        what makes the hold mean anything: the button sits in the same panel as
                        the field, so tapping it blurs the field and the native listener commits —
                        a hold that merely typed and tapped would be GREEN over a « Valider » that
                        does nothing at all, which is the defect. It is also the real case B-341
                        reports: a keyboard that commits without blurring
    seen RED first      against the tree as it stood: « the field's panel offers « Valider » »
                        false, « a tap on it files the edit WITHOUT the field losing focus » false,
                        « the panel names the typed value « nouvelle » » false
    the run             `python3 frontend/maquette/harness/settings_editing.py` → 15 holds, no
                        violation
...
**FIXED #588 (the settings micro-wave).** `updateConfigurationFile` reads the request's body and
writes each value into `held.settings`, keyed by the identity the row, the address and the save all
spell — and the identity is taken APART rather than composed a second time. **A file that CHANGED
ON DISK takes nothing at all**: « rien n'a été écrit » is what the conflict banner says, so a layer
that recorded the values anyway would make the banner a lie one level down.

    the rule            R166, `harness/settings_editing.py` — it saves and READS BACK through the
                        LAYER (the query cache's own answer) and through the ROW, never through
                        the message: the message is what B-342 IS
    seen RED first      against the tree as it stood: the layer answered the seed after a write it
                        had acknowledged
    the run             `python3 frontend/maquette/harness/settings_editing.py` → 15 holds, no
                        violation
...
**FIXED #588 (the settings micro-wave).** The flag left `SettingsState` entirely: a restart owed
is the LAYER's fact, answered by `/api/config/status`, and the banner is a reader of that query, so
it re-renders the moment the answer moves. `panel-setting.ts` no longer writes an engine object;
the save invalidates the status, and « Redémarrer maintenant » asks the layer rather than dropping
a local boolean and saying a restart had happened over a call nobody made.

    the rule            R166, `harness/settings_editing.py` — the banner is reached THROUGH a real
                        save (edit, « Valider », « Enregistrer »), and the walk goes on to raise
                        B-300's confirmation from there, which is the path a hand walks
    seen RED first      against the tree as it stood — and the operator's own attempt is the first
                        reading: he saved, was told « Enregistré », and no banner came
    the run             `python3 frontend/maquette/harness/settings_editing.py` → 15 holds, no
                        violation

**RE-AIMED WITH IT, and said out loud rather than quietly**: the four places `harness/page_host.py`
and `harness/settings.py` read that flag now read the layer's answer. The property each holds is
unchanged — « the save asks for a restart », « cancelling leaves the restart OWED » — and only
where the fact LIVES moved. Left on the old place they would have read `undefined`, gone green, and
said nothing.
...
the desktop layout it reveals is the one the oracle would measure without the frame.

**Closed by the `maquette-desktop-frame` micro-wave: the stylesheet draws it, and a harness script remembers it.** A checkbox and its
label in `design/index.html`, inside `.stage` and OUTSIDE `#device` (`data-part="harness/desktop-switch"`,
id `desktop-switch`); in `styles/harness.css` the frame's 520 px block AND both re-assertion
breakpoints scoped on `:root:not(:has(#desktop-switch:checked))`, the control itself `display: none`
below 520 px and `position: fixed` above it, and added to the `html.measuring` hide list. **No script
of the app's**: the state is the checkbox's and is read by `:has()`, so neither the dying engine (D5) nor
...
`desktop_frame_page.py`, **24 holds** at 390 x 844, 1280 x 800 and 520 x 800 — the
frame's own breakpoint, the tightest width it is ever drawn at — plus a presence
sweep over seven widths derived from the stylesheet's breakpoints and an 87-state
walk out of the frame, read
...
to keep that honest: each forced property must read DIFFERENTLY with the class off, or a property
the frame never moved could not show the frame leaving. Framed it reads `flex / 14px / none`;
unframed, and after one press, `none / 24px / block`. Mutation-tested, each mutation falling on its
own hold alone: the small-breakpoint guard opened → the phone hold falls naming the box it found
(`client rects 1, area 2516.3125, takes focus True`); one re-assertion unscoped → the « the app's own
stylesheet answers » hold falls naming the property still reading the frame's value (`tab-bar: flex`
against an unframed `none`).

**RESHAPED by the operator's ruling of 2026-09-08**, verbatim: « le desktop est une préférence localStorage ». It reversed this entry's « no script anywhere » and the brief's « a reload while switched out returns him inside the frame »: a reload now opens where he left it. The script is the harness's — inline in `design/index.html` right after the control, inside `.stage` and outside `#device`, beside the one that restores the appearance; never in `app/`, never in `legacy.js`, and it goes with the frame. The checkbox stays the drawing's state: the script writes it to `tm-desktop-switch` on change and restores it while the document is parsed, before any module runs. It restores only where the control is drawn, asked of the stylesheet rather than typed, because `:root:not(:has(#desktop-switch:checked)) .device { overflow: clip }` is declared outside any breakpoint and a box restored at 390 px would take the clip off the phone.

**R141**, `frontend/maquette/harness/desktop_frame_memory.py`, **9 holds**: a press writes the choice; a reload opens out of the frame with the box checked when parsing ends (an init script records `checked` as `readyState` leaves `loading`, before any module runs); the press back takes it back and a reload opens inside; a 390 px window with the choice stored neither applies it — overflow and box read what they read before anything was stored — nor forgets it; and the key is in `index.html` and in no file under `design/src`. RED first on the build of `64bc56670`, before the script existed: 9 holds, 4 violations, the reload hold reading « checked when parsing ended False ». GREEN: 9 holds, no violation, on `3b9dd94d9` and again on `1ae097a4c`. Mutation-tested: the restore removed (`control.checked = true;` → `void 0;`) fells « a reload opens OUT of the frame » alone, reading « checked when parsing ended False »; the width guard removed (the `display` test → `true`) fells the 390 px hold alone, reading « checked when parsing ended True ». On `3b9dd94d9` the first mutation also felled the way back, driven on the same page after the failed reload — one defect named twice, so the way back now opens a page of its own.

**The operator's word is what closes it in fact**: he opens the design host in Chrome on his Mac and
toggles it himself. This entry records what the rule read, not what the hand found.

**Renumbered at the merge of `main`, on the steward's ruling of 2026-09-11.** This wave filed its five register entries as B-370 to B-374 on its branch while L21 filed its own B-370 and B-371 on another; L21 merged first, so the two blocks collided and `check-bug-register.py`'s `duplicate-row` arm refuses the pair. The five moved as ONE contiguous block, in their order, past the numbers another branch already holds: B-370 → **B-388** · B-371 → **B-389** · B-372 → **B-390** · B-373 → **B-391** · B-374 → **B-392**. `main`'s B-370 and B-371 are L21's and did not move. Commit `738999215`'s message and the pull request's history keep the old numbers; this paragraph is the table that reads them.
...
**B-365 — a refusal hold that reads a network the refusal never crosses.**
`harness/busy.py` holds NE-DOIT-PAS-3 with three checks, and one of them cannot fail. « no mutation
was answered 409 » reads `refused`, a list filled by `page.on("response", …)` where
`answer.status == 409`. **The mock layer replaces `globalThis.fetch` and answers IN THE PAGE**, so a
mocked 409 raises no response event at all: the list is empty whatever the layer answers, and the
hold has been green over nothing for as long as it has existed.

**IT WAS DECLARED BEFORE IT WAS PROVEN, and that is the worse half.** `DESIGN.md` § 3.1c,
written by phase 2, already says it: « ⚠ `busy.py`'s « no mutation was answered 409 » hold is
vacuous for this reason — it listens for a 409 on a wire nothing is put on. **Phase 6 owns
`busy.py` and repairs it there.** » Phase 6 repaired the SIBLING hold and left this one, and
the design document went on carrying the sentence that said it would not. So this entry does
not record a discovery; it records an obligation this lot wrote down, owned, and missed — and
a mutation is what established that it still stands.

**PROVEN BY MUTATION, not by reading.** The 409 was forced through the layer's own status path
(`outcomeFor` in `mocks/scenario.ts`, whose `outcome.status >= 400` branch returns `problem(...)`)
for `grabSeasonForFollow`. One violation out of fifteen: the SIBLING hold — « every one that was
asked was answered WITHOUT a refusal », which reads `window.__mocks.answered()` — fell and named
`409 POST grabSeasonForFollow`. This one stayed green while the refusal was real.

**IT IS THE SAME DEFECT AS THE ONE REPAIRED THREE LINES ABOVE IT.** That sibling's first version
collected Playwright response events too and was rewritten to read the layer's own record; the
repair did not travel to its neighbour. A third hold on the same clause — « nothing anywhere said
the machine was busy » — did NOT fall either and is **legitimately orthogonal**: it reads whether
the INTERFACE says « occupé » / « déjà en cours », which a caught 409 need not, and saying which of
the two an unmoved hold is costs a sentence and is the whole difference.

**THE GENERAL FORM, and it is worth more than the instance**: mutation testing usually asks « did a
hold fall? ». The question that finds these is « did EVERY hold that claims this clause fall? » —
the fallen set against the set of holds whose text names the clause. `common.Journal` prints both,
so it is a set difference on strings already in hand.

**What a fix takes**: read the refusal from `window.__mocks.answered()` across ALL operations, keep
...
**B-369 — a fixture rename that made a named state's own premise false.**
A paused follow was renamed to « The Venture Bros » so that it would have a media sheet. That
title is what the state `mediasheet-suggestion-series` shows, and that state's own label reads
« Fiche — suggestion NON possédée (série) ». After the rename the medium IS followed, so the sheet
draws the disabled « already followed » button instead of the offer, `[data-part="media/add"]`
carries neither `data-follow` nor `data-fkind`, and `follow_verb.py` reads
`{'title': None, 'kind': None}` — 10 rules EXECUTED, 2 violations.

**THE SHAPE, AND IT IS THE POINT.** The rename was chosen BECAUSE it touched only a fixture: the
two roads that repair the defect properly both grow the dying engine, and a fixture looked like
the containable option. It was not. A fixture is an input to every rule that reads the surface it
feeds, and this one falsified the premise of a rule two files away — while ALSO silencing the
instrument it was meant to satisfy, which the entry it was made for already records. One rename,
two instruments, neither of them named in the change.

**AND NOTHING COULD HAVE SEEN IT UNTIL THE FULL SUITE RAN.** Between that commit and this reading
only the eighteen contract rules had run, and `follow_verb.py` is not one of them. A tier that runs
on every pull request is not a tier that reads everything, and a fixture edit is precisely the kind
of change whose blast radius the cheap tier cannot bound.

**REPAIRED** by moving that follow to « Dark Matter », chosen against four filters rather than
picked: it has a media sheet, it is not already followed, it is the subject of no named state, and
it is named in no harness rule. The FOLLOW was moved and not the state, deliberately — the
`acq-follows-*` states already carry accepted oracle divergences whose recorded reason is this very
rename, so moving the follow again keeps that reason, where editing the state would put a fresh
divergence on a state that has none.

<sub>`git log -S"The Venture Bros" -- frontend/maquette/design/src/mocks/seeds/follows.json` names one commit · `engine/states.js`: `window.__screens.mediaSheet("The Venture Bros")` on `mediasheet-suggestion-series` · `follow_verb.py` → `10 rules EXECUTED — 2 violation(s)`</sub>
...
for this page), so this verb pushes through `__bridge.record` and the entry restores the rubric on
its own; what was missing was the push itself and the affordance. The engine's `dataset.maintopic`
branch is DELETED, the verb is `features/maintenance/topic-verb.ts`, and the row that used to say
« toutes les commandes » — a cross-reference that navigated by writing the rubric away — is
`data-part="screen/back"` and POPS.

    the rule            R165, `harness/topics.py`, the same 16 holds walking BOTH pages: what is
                        one mechanism is held by one rule
    seen RED first      against the tree as it stood, no mutation needed: « PUSHES an entry »
                        4 → 4, « DRAWS its way back » False, « Back lands on the page's OWN list »
                        /acquisition
    the run             `python3 frontend/maquette/harness/topics.py` → 16 holds, no violation
...
**B-392 — the comment that says who owns the selection bar names an owner that no longer draws it.**
...
**Thirty-six** cases in `tests/scripts/test_check_maquette_comments.py`; wired into `make check` and
...
**FIXED by L14 (#547), and neither the flag nor the SITES were the ones the brief named.** With
placeholder data the query reports `status: "success"` and `isPlaceholderData: true` while the read
is still out; `pending` is the state with NO placeholder at all, so a rule reading
`status === "pending"` is green over the primed case, which is the whole case. The flag is
`isPending || (isPlaceholderData && isFetching)` on the sheet and `isPending` on the seasons. And
the repair is field by field, never block by block: the hero's metadata line was gated on the whole
sheet being null, so a placeholder carrying the title and not the year printed « année inconnue ·
Série » — an assertion about the KIND of a medium whose kind was in flight; « Possédés 0 » is a
number about seasons still out; and the season list's « Épisodes non détaillés » is said about the
sheet's episode lists while the seasons have landed and the sheet has not. All three are gated now,
and the error path — where the placeholder is dropped by the library — keeps what the tap knew and
says the read failed instead of printing « le provider n'en fournit pas » over a 502.

<sub>`frontend/maquette/harness/priming.py` (R119) — it thins the reference's `sheetFor` to `{t}`
alone through an init-script setter, holds the read back through the mock layer's own knob, and
holds the EXACT skeleton count for a known thinning plus the absence of any i18n « unknown » value
in the screen's text, read from `fr.json` rather than retyped. Its control walks the complete
placeholder at the same latency, and further walks fail the read — over the complete placeholder,
over the thinned one, on a series owned incompletely, on the seasons read alone and on a film. The
rule's own header enumerates its cold loads and is the only place their number is written. Mutations: each of the
gated fields forced back to its assertion — the text hold falls on every one.</sub>

<sub>`grep -rc "Unknown\|noTrailer" frontend/maquette/design/src/features/media/` — the
assertions live in the sheet's parts, not in the screen, since that file was cut. **How to drive it,
corrected**: with a partial placeholder, and NOT on `status === "pending"` — the flag this very
entry refutes four paragraphs above, since a query holding a placeholder reports success while the
read is out; and not on `[data-part="no-info"]`, a name worn by two of the nine sites that can print
an assertion. The reading is the EXACT skeleton count against a known thinning, plus the screen's
own words read from `fr.json`.</sub>
...
| **L14** (#547, the surfaces that outgrew their file)| **7 by the wave, 30 by twenty-one readers** | **A unit is credited to whoever found it, and this row got that wrong twice — first by crediting three reader majors to the wave, then by leaving out five the wave HAD found.** The wave's seven, all the RED form or found by reading the figures its own new holds print: R119's CONTROL, written as « the complete placeholder draws ZERO skeletons » and red over the design working as drawn · `check-component-once.py`'s first floor, at 50 declarations against a corpus of 69 · and the five vacuities caught while mutating its own rules — a branch unreachable for an owned series, so the mutation fell nothing; one latency for two reads, so the interval the hold names did not exist; a thinning naming one title while the walk opened another, so it measured a COMPLETE placeholder and printed zero skeletons (the number said so and the hold did not read it); « in flight » read as the mock layer's total, true on a page whose sheet landed long ago; and a walk on the leanest placeholder that cannot decide « field by field » at all. **The readers' sixteen**, each an instrument whose READING did not decide what it claimed: R119's floor of 6 against 11 with a part name worn by 2 of 9 sites · `export default function` refused by a regex under a docstring saying « exported or not » · hold (f) driving `touch()` alone · its selector never naming a `path` · its floor of ten met by the page beneath a screen · `acq-follows-group` undriven · the incomplete lens's memo read by no driven state · **R117's delete hold GREEN on the code it was written against** · « in flight » read as `data === undefined`, which is also a failed read · an action counted as a CHILD · a floor standing in for reading that a walk's thinning applied · hold (e) reading that an error surface was PRESENT, never what it said · **hold (e-i) reading a counter the reducer resets, 1 before the click and 1 after** · R119's error walk driving the COMPLETE placeholder only, so the failure's own thin case was outside it · R100 (g) green without a replacement, on a window that keeps every row · and hold (i) reading three sentences while six skeletons shimmered beside them forever. **A FOURTH ROUND, on the third round's repairs, returned NINE MORE — every one of them an instrument this wave had just written or just repaired.** R117's new selection-mode hold measured a walk no reader can take (a click driver scrolls a static control into view, so the mode change it timed happened at the top of the list) and compared no place at all, holding « the port is past zero » over a repair that walked the reader four rows up each time · R100's (g-i), written in round 3 to close a vacuity in its own sibling, was green with the delete stubbed to a no-op — the same vacuity, reproduced by the repair for it · R119's (e-iii) walked a title the fixture records no owned numbers for, so the season bodies it was written to hold are empty whatever the code says · and its four terms are all maskable by one row's branch, leaving the hold nothing to fall on · hold (i)'s `aria-busy` read descendants only, so the attribute moved one level up would leave it green over a busy screen · the `season/missing` name was worn by an air date as well as by a shortfall, so the term counting « missing chips » read seven on a sheet missing nothing · `<details open>` was read by no hold, and a mutant opening every season row at rest stayed green · hold (g) REFUSED « Saison annoncée : aucun épisode diffusé », a sentence the seasons read had answered — the rule falling on the screen being right · and `virtual.py`'s two numbers read out of a TypeScript source failed as an attribute error on `None`, naming neither the file nor the declaration that moved. **Two more were found by the wave and are NOT counted here, because neither is this species**: an exclusion typed as a French substring stopped excluding anything the day the sentence was reworded, which is a rule going RED; and the anchoring suppression written beside the place restore fell no rule when mutated, which is a repair with no subject rather than a gate with a blind spot — it was removed. **A FIFTH ROUND, on the fourth's repairs, returned TWO MORE of this species.** (e-iii-a), the term written in round 4 to refuse the sentences that speak for the provider, walks a title the fixture gives a trailer — so that sentence cannot be printed there whatever the code says, and a build printing it over a 502 passed all twenty-nine holds of its rule. And the wave's own hold list still counted its walks in three different numbers, in the rule's header, in the report and in the pull request, while the file made ten cold loads. **A SIXTH ROUND added THREE more, all of them in what the fifth round had just written.** The 300 px hold compared the top row's TITLE, and at three hundred pixels and at the container's own start that is the same row — so it passed on the code it was written against, which is the exact species of R117's round-2 finding repeated by the rule written to close it. The bare-unknown term was written over ELEMENTS while the kind's word lives in a text node inside a fragment, so its first form was green over the one site it existed for. And no rule read OWNERSHIP at all: reverting `ownershipKnown` to the form that classed a null answer as known left all thirty-two holds of R119 green, on the screen whose whole subject is what may be claimed about data nobody has. |
| **L19** (the producers) | **27 across three rounds** — 11 by the wave, 7 by round one's readers, 9 more from rounds two and three | **THE ASYMMETRY THIS TABLE PREDICTS, ARRIVING ON SCHEDULE.** The row read « 9 by the wave, 0 by readers yet » and said so as a claim awaiting readers; four readers on the green gate returned seven more, and every one of the seven is in an instrument, not in a product surface. **The seven, each named: (1)** the size ledger is its own oracle — the FILE is compared with the record and the RECORD with nothing, so a growth is legalised by moving the number in the same commit, which is B-306's species one level up. **(2)** `check-state-ownership.py`'s engine-owned exemption proved by SUBSTRING, so a comment spelling the path keeps it alive after the import goes — in a file full of comments naming feature paths. **(3)** `panel.py`'s boot needle, likewise: a commented-out `import "./panel-field";` left the hold green, exit 0, where the deleted line fails it. **(4)** `paths_to_sheets.py`, twice over — four of its five reachability branches select nothing on all six surfaces, so the rule had narrowed to one attribute with nobody reading it, and its message counted the four-item SAMPLE as the number of dead ends. **(5)** `fanout.py` holds the invalidation map against ITSELF: a key dropped from the declaration is invisible (B-319). **(6)** `hold-counts-baseline.json` is compared by nothing at all — no guard, no CI job — and now carries eleven rows that are wrong or absent. **(7)** `busy.py` raises its panels through the SEAM, which is the exact vacuity this wave had already caught in R103's journey hold, reproduced one rule along. **Six of the seven are in guards, and three of those six are the same shape: a fact asserted by searching a file's text rather than parsing it.** The wave's own nine follow — **and REPAIRING those seven turned up two more of the wave's own**, which is why this row reads eleven: `app/focus.ts` asserting « `[autofocus]` first » over a selector LIST, where `querySelector` answers in DOCUMENT order and the comment had been wrong for as long as it existed; and this wave's own new history hold, which cleared the whole query cache and was therefore refused by the engine's `resolves` before it reached the suppression it measures — `open=False · history.length 4 -> 4`, identically with the repair and with the repair mutated away. This row is the wave's own count and it is the count a wave has been wrong about three times in a row, so it was written as a claim awaiting readers rather than as a total — and it was right to be. **Four are in the wave's OWN new rules**, caught by their own mutations: R120's refusal check accepted ANY throw, so a `TypeError` from the crash it was written to prevent passed it; R103's new journey hold drove the SEAM, so the 260 ms wait it refuses was never on its path; R122's reachability was satisfied by a second path, which is why its first mutation was survived CORRECTLY and a second was needed; and `pop.py` read the popover's text while `SssEnn` is composed from the CELL, so it is right whichever episode was looked up. **Two are older rules asserting the very defect they sat beside**: `page_host.py` held « a real tap on the restart offer TAKES it », which is B-300 written down as a requirement; and `pop.py`'s `main()` computed a verdict and never reached the exit code, so that half could print « needs review » and pass the suite. **Three are product surfaces no instrument reached at all**: a destructive maintenance command could have worn the SUCCESS tone, §5's film/series distinction was drawn and read by nothing, and the settings panel could have told the operator their edit did not take. **And one more that is not counted here because it is an instrument of this wave's own making rather than a gate going green**: a probe for B-311 read « 900 → 900 KEPT » twice, on two builds, having never scrolled at all. It is written into that entry. |
| **maquette-schedulers** (#567, the B-308 micro-wave) | **8** — 3 by the wave, 5 by five independent readers over six rounds | **RECOUNTED AT THE POST-MERGE GESTURE, 2026-09-06: the row read « 1 » while the readers were still reading, and their five are all inside the rule the wave repaired.** The wave's three: (1) the one below; (2) its own new unit test's cadence hold, which compared the substituted line against the row it NAMED — true by construction, green under the exact mutation it was written to catch (repaired in `7d218bcc5`); (3) the join's first green line, which kept a `None` label for the five named twice and printed « PM2 names with no drawn row: [five names that DO have rows] » under a PASS. **The readers' five, each in the hold added or repaired the round before**: round three — the new label-table hold read `fr.json` from the MAIN checkout through `~/dev/PersonalScraper`, green over the label's absence on any worktree, and it raised at import off that path with no verdict at all; round three — the same hold tested PRESENCE and never AGREEMENT, so a label with the wrong text passed; round four — the agreement hold joined by SET MEMBERSHIP, so `personalscraper-index-full` given a DIFFERENT scheduler's drawn label passed green while its green line printed the size of its own table; round five — the join's drawn side subtracted the WHOLE mapping, so a scheduler removed from PM2 with its row still drawn was named NOWHERE; round five — the join hold held the COUNT and not the sets, so a renamed scheduler passed at 7 vs 7 while naming the unmatched job in its own sentence. Round six, the steward's re-take with a `pm2 jlist` shim, came back empty. **What was counted as the wave's one, verbatim:** **One, and it is the rule this wave repaired, on the half nobody was looking at.** `machine.py`'s « a scheduler's badge follows the declared state » compares the DRAWN tones against a declared source, and for the schedulers that source was `window.SCHEDULERS` — the engine's fixture. The page stopped reading that fixture at L08 and has read the mock layer since. So the hold compared what the surface drew against an object the surface never touched, and it was green because the two happened to agree byte for byte: the day the seed and the fixture drifted in tone, it would have reported agreement or disagreement about the wrong list. Found by the removal rather than by reading — taking the fixture out made the expression RAISE, which is what named it. The source is the query cache now. **Counted 1, not 2**: the backend mirror B-324 files is not an instrument green over what it does not read, it is a list no instrument reads at all, and D5's stale `254` is a figure with no guard behind it — neither is a unit of this species, and saying so is this table's own rule |
| **maquette-departure** (#573, the B-310 micro-wave) | **4** — 2 by the wave, 2 by gates refusing its own work | **ALL FOUR ARE INSTRUMENTS, and two of them are the seam's own rules stopping ONE PROPERTY SHORT — which is why both defects reached the operator's phone under a green suite.** `transition.py`'s `hold_the_panel_departs` asserts `panel-down` RUNS on `::view-transition-old(leaving-panel)` — by animation name, so the user-agent cross-fade cannot satisfy it — and never reads what the snapshot does when it ENDS; that is the whole of B-310, held by nothing for the life of the departure. `exits.py` (R103) holds that a leaving layer stays `visible`, and that is all it reads — while the reason the idiom EXISTS is written, in those words, at three other sites: « the layer stays hit-testable until it has finished leaving » (`ui/variants/layout.ts`, `harness/transition.py`, `styles/base.css`). The property the codebase says the idiom is for and the property its rule measures are not the same property, and the gap between them is B-338, read from the finger's side. **The first version of this row put that sentence in `exits.py`'s own header, where it has never been** — a misquotation found by a reader running `git log -S` on it, in the row this table exists to keep honest. **The other two were found by gates refusing this wave's own work, which is the only reason anybody looked at them.** `check-maquette-comments.py` refuses a FILE's reference count going up, and it DOES read the corpus size — as a floor 10 % below the record, against the reader silently stopping. What it never asks is whether the record has gone stale UPWARD: a corpus that GREW is far above that floor, so the guard printed « clean » over a wave that added a source to it while the floor was being derived from a tree that no longer existed. Its TEST is what failed, `assert 318 == 319`. **The first version of this row said the guard reads the corpus size « not at all », which is false and checkable at `scripts/check-maquette-comments.py:279-281`** — a guard read from memory instead of from its source, in the row about guards that are not read. And `check-bug-register.py`'s closure arm refused an honest `fixed #573` on B-310 while standing ready to accept a SILENT closure of B-249, over one paragraph: `BODY_HEAD` is `^\*\*([BE]-\d{3})\b` and `entry_bodies` keeps the FIRST span, so `**B-249's FAMILY…` inside B-310's entry ended B-310's body at 3 065 characters of 9 740 — cutting off the mechanism that stands and every device reading taken on the operator's phone — and, preceding B-249's real entry, took B-249's own body as the discarded one (**B-346**, 25 of 278 heads are second-or-later). **A fifth was found in a sibling wave from this one's finding and is NOT counted here**, because it is not this wave's: `--record` on the comment baseline would have entered a lot code as that file's allowance, freezing a violation as the baseline. |
| **L21** (#572, the tunnel's verbs) | **22 across three rounds** — 11 by the wave, 11 by independent readers (round one 0, round two 7, round three 4) | **RECOUNTED against the lot's own design document before publication, and the recount changed the COMPOSITION rather than the figure.** A first count of eight was assembled from the rotation's hand-over alone; reading `docs/features/maquette-l21/DESIGN.md@2ffdc4ba` § 3.1c underneath it showed one unit that was not a discovery at all and one the hand-over had never carried. **That first recount came out at eight either way, and the coincidence is exactly why the itemisation is the count and this sentence is not — a NINTH was found afterwards, by a bisect, and the itemisation is where it went.** Itemised in « L21's twenty-two, itemised » — one line per unit with what establishes it. **One of § 3.1c's four is deliberately EXCLUDED**: « two sources for one fact » — the absorbed count crossing the sheet's provider catalogue with the owned-episode seed and answering 10 for a season printed as « 1 manquant » — is a wrong ANSWER in a mock handler, not a hold green over what it does not read, and this table's species is the second. **Zero by independent readers** was written before its readers came, and this table's own evidence said it was a figure awaiting them. **They came: ELEVEN, over three rounds, and ten of them after the wave had declared its instruments sound** — round one's reader found none, round two's seven, round three's four, each one a rule this wave wrote or repaired, and five of round three's and round two's are in the very holds the previous round had just added. **And the wave found an eleventh of its own while repairing C1**: R158's « Agent Elvis » hold certified the same defect as R160's. Recounted at the post-merge gesture, on the steward's ruling of the criterion — the title, « green over what they do not read », with the mechanism that leaves the reading absent not part of it: B4 is IN (a mock answering in ~15 ms is what R157 did not read), B5 and B8 are OUT (a false claim; no rule named) |
| **maquette-desktop-frame** (#576, the B-344 micro-wave) | **36** — 11 by the wave, 25 by five independent readers over five rounds | **RE-TAKEN ONCE ON THE FINAL HEAD, and the row read 4 while the readers were still reading.** The 4 were honest and they were a quarter of the answer, which is this table's oldest prediction arriving on a micro-wave of one rule. **The wave's 11**: R140's accessible-name hold reading `textContent`, caught by a probe before the rule ever ran; **B-388**, R51 holding a class of thing by one literal; **B-273's third case**, `mutate.sh` printing « NO RULE FELL » over a rule that died on a refused port; the **accessibility tier**, which audits at 390 px and so read nothing of a wave whose subject does not exist there; **B-273's fourth case**, two FAIL lines from a CRASHED rule being indistinguishable from two from a rule that finished; a dictionary keyed by SELECTOR alone, collapsing the tab bar's five readings into one; `height` placed in the device's SKIN where it cannot be compared (800 px against a frameless 861); the skin compared against an eight-key control document by a four-key reading; **B-391**, four declarations of the « ONLY accepted divergence » that restate what the app already says and can therefore witness nothing; the animation-wait that, asked ALONE, read the page EARLIER than the fixed delay and took the count from 5 to 13; and `desktop_frame_page.py` run as a rule by both discoverers after the split, for ever unparseable. **The readers' 24, and every one is in an instrument this wave had just written**: the frame's SKIN able to stay while all fourteen holds read green · the completeness claim reading three of the four declarations it claimed to hold all of · three declarations surviving the press while the hold said none did · the two-verbs-at-once name still green after the repair that claimed to close it · « stops 16px short » held by nothing and false · the control document called the app's cascade without its limit · **B-390**, no arm of `check-no-french` reading text in the file whose ruling cited it · the gap hold green over a control that was not drawn at all · hold 16's five literals, at their most vacuous in the one state they were written for · completeness reading SCOPED declarations only · its parser skipping a selector list that mixed scoped and unscoped · the survivors hold reading ONE key while its comment claimed three and its block declared seven · the sweep green over a page that had never left the frame · « differs from the control document », which a wrong value satisfies as well as a right one · the parser knowing one spelling of the device · the sweep asking what the control covers and never what covers the control · a 120 ms settle against a 450 ms drawn duration, which MANUFACTURED fifteen of sixteen findings · presence read at two widths with the band between them unread · `.device` matched by substring · **the probe FED the declarations it checks**, so `background: red` written into the block read red against red · the derived widths knowing `min-width` only · `getAnimations()` unsatisfiable in 20 states, so the ceiling IS the settle there · two exclusion lists agreeing by comment · and two name holds testing containment where they meant equality. **Three of those are the SAME defect in the same hold, three rounds running** — « differs from the control », then hand-computed meanings, then a self-fed probe — each repair moving the tautology rather than removing it, which is why the rule now carries the sentence « an expectation derived from the thing under test is not an expectation » in its own header. **Reader E's round five adds one, on the steward's ruling of 2026-09-11**: MINOR-E2, the breakpoint derivation R140's presence sweep rests on, which required the unit `px` and so read `@media (min-width: 55em)` as nothing — green over every breakpoint it could not convert, found by a reader and not by the wave. |
| **maquette-resolution-card** (#585, the B-393/B-394/B-395 micro-wave) | **13** — 4 by the wave, 9 by two independent readers (round one 5, round two 4) | **COUNTED AT THE POST-MERGE GESTURE, because the wave wrote no row at all** — and a row missing and a row saying zero are not the same statement, which is this table's own rule. **The wave's 4 are its mutations, and three of them are the same shape**: a named mutation that is INERT reads exactly like a rule that does not bite. `hidden` on `candidatePick` changed nothing, because the emitted stylesheet writes `.hidden{display:none}` before `.inline-grid{…}` and `iconButton` puts the second on the same element; `inert` written on the tab bar in JSX never reaches the DOM (React 19.2 drops it), and set through a `ref` it does reach it and the app then removes it, `setBackgroundInert(null)` clearing every `[inert]` when a layer closes; an `[height:20px]` utility, and even an inline `style`, lose to `.card`'s `min-height: 126px` in the dying engine's stylesheet. **The fourth is R163 green over the very defect it was written for**: the frame marks the background `inert` while a layer is open, and `inert` takes an element out of hit-testing without changing what is PAINTED — B-381's lesson met again on its own subject — so the hit test answered « the message » over a button declaring rank 70. The rule lifts the attribute, asks what is in front, and puts it back. **Round one's 5, by one reader on a gate green in every tier**: R161's affordance holds satisfied by an affordance that is not drawn (A1); R162 unable to tell a 7 s window from a ~1.5 s one (A2); the poster's initials fallback announced as part of the card's name, over an accessibility tier that had read 87 states at 0 violations and never read a name (A3); the ranked list not recording every rank, five declarations outside it (A4, old and re-asserted); and no fixture offering two pickable folders, so the window's riskiest path is proved by a seam and by no finger (A5, filed **B-396**). **Round two's 4, by a second reader on round one's repairs, and every one is inside a repair**: the h2 floor green over `opacity: 0`, a clip and an off-screen transform (B1, **B-460**); `LAST_FRAME` named « the message's last reachable frame » where the message's opacity is 0, so only a SHRUNK window is armed and a message outliving the window passes all eighteen holds (B2, **B-461**); the ranks arm attributing a declaration to the nearest `export const` above it (B4, **B-463**); and three scopes that arm never opens (B5, **B-464**). **Three of the readers' findings are deliberately NOT counted, and saying so is this table's rule**: A6, closing the message discarding the undo, is a product behaviour the operator RULED on (option A, keep as built) and no instrument is green over it; A7 and B6 are a false SENTENCE in prose (**B-465**, B-392's species one file away), not a guard reading too little; and B7 (**B-466**) is a ceiling with one line of room — arithmetic, not blindness. **B-462 is excluded too**: the card announcing less than it shows is the measured COST of a repair the decided list asked for, ruled by the operator on 2026-09-12, and R161's h9 reads the name it was written to read. |
| **Total** | **321** | 130 by the waves through L11's own five, plus the four readers' 13 recounted above — at 2026-08-31, after L10-bis, its review, L10-ter's survey and its review, **L15 with its own review**, and **L11 with FOUR readers**. **L11's FIVE are the mutation half of the curve, and they say what mutation IS good for**: every one was found by breaking the wave's own rules on purpose, and two of them are the same rule going green twice over a shell that was not there. A rule can hold a CONSEQUENCE while the mechanism it names is gone, and only a mutation asks. **What mutation could not reach is the other forty**: four readers on that same green gate found a security regression, an update discipline that never swapped the worker, and a queue a refused replay jammed forever — none of them a defect the author had thought to break. The ratio is this table's oldest argument, arriving on a wave that wrote BEHAVIOUR rather than conversions: 5 by mutation, some 40 by reading. **And a second round on the REPAIRS found thirteen more, then a third found seven** — the two later rounds are where the sharpest findings are, not the first. Round two: the repair for a jammed queue had made a refused mutation vanish SILENTLY instead, and its classifier destroyed the operator's action on a 503, a 429 and a 401 — the very outage the queue exists for. Round three: that repair's own replacement still treated **401 as final**, so an expired session destroyed every queued mutation one after another when a re-login would have saved them all; and the notice's button said « Réessayer maintenant » while doing something else, because its name, its words and its action were written as three ladders that tested their conditions in different orders. **Each round's worst finding was in the previous round's repair**, and none of the three was found by a gate. **L12 adds 30 — 143 + 30 = 173 — and the addition is the itemisation's, not a counter's.** For two days this cell read 151 while the itemisation read 15 and `main` read 143, because the figure had been incremented ONCE PER REGISTER ENTRY FILED as B-271 to B-278 landed: 144, 145, … 151. Four of those eight entries are not units of this species at all, and the itemisation moved 6 → 14 → 15 without the total moving with it. A total that tracks a different quantity from the table under it is the drift this whole page is about, and it took an adversarial reader to do the subtraction. **docs-cleanup adds 1 — 173 + 1 = 174** (B-293, the 38 dead `Design:` markers the design-gaps pair is green over; found by the wave's inventory, 0 by readers). **L14 adds 37 — 174 + 37 = 211**, seven found by the wave and thirty by twenty-one independent readers across SIX rounds, on a gate green in every tier each time. **The fourth round's nine are all in instruments the third round had just written or just repaired** — including the hold written to close a vacuity, carrying the same vacuity — which is this table's oldest reading arriving from the closest possible range. **The wave counted itself three times and was wrong three times**: 2 when the readers had already returned three more; « 5 by the wave » when three of those five were reader majors; and 2 again when its own report named five vacuities it had caught in its own new holds — the species this table exists for, uncounted in either column. The lesson is not that the counting is careless. It is that a wave cannot audit its own instruments: every correction here came from someone else reading the same tree. **L19 adds 27 — 211 + 27 = 238**, over THREE rounds, and the shape of the curve is the reading. Eleven by the wave — nine by its own mutations, and two found REPAIRING the readers' findings. **Seven by round one's four readers, all seven in instruments.** **Nine more by rounds two and three**, and every one of those nine is in an instrument too: the comment arm's corpus floor placed below anything that could happen; the CI-depth hold blind to the second job that runs the guard, then asking a SUBSTRING of `run.sh` where it needed the array; R124 calling a `row.click()` a finger; R82's own new hold evicting a query key this interface does not have, so it walked a warm cache and passed with the repair removed; the comment reader's two cases asserting that string literals are safe over shapes containing no `//` at all; R120's order hold comparing `None` with « danger »; the same hold reading UNSCOPED over a tree that persists, so a produce that opened nothing answered about the previous panel; and `add`, the one registered kind nothing drove — ten moved, nine walked. **TWO were deliberately NOT counted**, and saying so is the rule this row has been wrong about before: `PANEL_OUT`'s banner claiming more than its constants did, and R124's toast helper named for a gesture it does not perform, are false CLAIMS rather than instruments green over what they do not read. **The count trebled after the wave declared itself done, and every one of the sixteen added since came from someone else looking** — which is this table's oldest argument, arriving three times in one pull request. The row above wrote the nine as « a claim awaiting readers » rather than as a total, on this table's own evidence that the reader half arrives later — and it did, at 78 % of the wave's own count, on a gate green in every tier. What it did NOT predict is the shape: L11's readers found product defects the author had not thought to break, and L19's found guards. **Three of round one's seven are one shape — a fact asserted by SEARCHING a file's text instead of parsing it** — and that shape had already shipped twice in this wave under its own author's review. A wave cannot audit its own instruments, and it cannot see its own habits either. **maquette-schedulers adds 1 — 238 + 1 = 239**, and it is the only kind of unit a one-entry micro-wave can honestly contribute: found by the removal, in the rule it was repairing, with two adjacent findings deliberately NOT counted because neither is an instrument. **maquette-schedulers adds 7 to the total — its row moved from 1 to 8 at the post-merge gesture, and the 1 was already inside 239: 239 + 7 = 246.** Three by the wave and five by five fresh readers over six rounds on ONE rule, `machine.py` — every one of the five inside the hold the previous round had just added or repaired, and the sixth round, the steward's own, empty. A micro-wave of one rule produced the same curve a lot does. | **maquette-departure adds 4 — 246 + 4 = 250**, and its shape is the one a micro-wave can honestly produce: two of the seam's own rules measuring a proxy for the property their headers claim, and two gates that could only be seen by being WRONG about this wave — one refusing an honest closure, one printing clean over a corpus it never counts. **Zero by independent readers at the time this row was written**, which this table's own evidence says is a figure awaiting its readers rather than a total. **L21 adds 10 — 250 + 10 = 260**, itemised one line per unit in « L21's ten, itemised », and the itemisation is the count. **The first version of this figure was also eight and was wrong**: it was assembled from the rotation's hand-over without reading the lot's own DESIGN § 3.1c underneath it, so it carried B-365 as a DISCOVERY — where § 3.1c had named that hold vacuous and assigned its repair to phase 6, which repaired the sibling and left it — and it missed « the message is not in `#toast` » entirely. The two errors cancelled in the total and in nothing else, which is the argument for itemising made by the arithmetic rather than by anyone's judgement. One of § 3.1c's four is excluded with its reason. Zero by readers, so far. **L21 is recounted to 22 at its post-merge gesture — 250 + 22 = 272**: the ten, an eleventh the wave found repairing round three's C1, and eleven by independent readers over three rounds — 0, 7, 4 — every one in an instrument, and ten of the eleven arriving after the wave called its gate green. The curve is the one L14 and L19 drew, on a lot that wrote BEHAVIOUR: the first reader found product defects, the next two found the holds written to catch them. **maquette-desktop-frame adds 4 — 272 + 4 = 276**, and three of its four are the instruments the wave's own proof is made with: a hold in its brand-new rule reading `textContent` and so green over a name that never changes, R51 holding a class of thing by one literal (B-388), and `mutate.sh` saying « NO RULE FELL » over a rule that had crashed before reaching the page (B-273's third case) — a false reading that was BELIEVED for one command, on the tool every mutation proof in this register rests on. The fourth is the accessibility tier, which audits at 390 px and therefore certified « the markup is usable » over a wave whose whole subject is a control that does not exist at 390 px. **The recount at the close: 272 + 35 = 307.** The row read « 4 — zero by independent readers » while four readers were still reading; they returned 24, six times the wave's own count at that moment, and every one of the 24 is in an instrument this wave had just written. A micro-wave of ONE rule produced the same curve a lot does, and produced it four times over. **maquette-desktop-frame adds one more — 307 + 1 = 308** — reader E's MINOR-E2, a derivation green over every `em` breakpoint it could not read. **maquette-resolution-card adds 13 — 308 + 13 = 321**, counted at its post-merge gesture because the wave itself wrote no row: four by the wave, of which three are INERT MUTATIONS — a mutation that does not mutate reads exactly like a rule that does not bite, and the wave checked every one of its own for having actually mutated before reading its verdict — and nine by two independent readers over two rounds, five then four, every one of the second round's inside a repair the first round had just demanded. Four are filed here as **B-460**, **B-461**, **B-463** and **B-464**, and A5 was filed by the wave as **B-396**. Four more findings are excluded with their reasons in the row above, which is the discipline this total has been wrong about before.
...
| **L20** (the global levers and the history) | **7** — 7 by the wave, 0 by readers yet (the reader round follows the pull request) | itemised in `docs/features/maquette-l20/REPORT.md@60c6d9b1d` § 4: R184's value hold read a length, R181's loading state measured the page's skeleton, R180 read the wrong run and a page-wide success word, R182's count hold took any number, R183 read `offsetParent` over a closed `<details>`, and the oracle read 0 over a control's word and look changing |

### L21's twenty-two, itemised

**One line per unit, each with what establishes it.** The count follows the itemisation, never the
other way round — and this list is what caught the first version, which had B-365 as a discovery
and no `#toast` unit at all.

| # | The unit | What establishes it |
| ---: | --- | --- |
| 1 | **A mocked call reaches no network.** `mocks/index.ts` replaces `globalThis.fetch`, so Playwright's request and response events never fire for an API call — every hold written as `page.on("response")` over a mocked operation is green whatever the interface does. **Counted ONCE as the class**, with its two cases named rather than counted again: R124's own non-vacuity guard, whose first version collected response events and would have left « nothing was refused » green over an empty list for ever; and `busy.py`'s « no mutation was answered 409 » hold, which is **B-365** | DESIGN § 3.1c-1, measured: a verb that demonstrably ran produced ZERO page requests |
| 2 | **The message is not in `#toast`.** That element is the dying engine's and the message layer is React: `#toast` stayed EMPTY through a verb whose message was on screen throughout, so a hold reading it is green over a message the operator can see. ⚠ `busy.py`'s refusal-word hold still reads `'#toast, #view'` — the `#view` half is what keeps it measuring anything | DESIGN § 3.1c-2 · `grep -n "SAID = " frontend/maquette/harness/busy.py` → `:80` |
| 3 | **`window.__go` RE-SEEDS the mock layer.** A pipeline set running before the state is driven is idle again by the time the act lands, so the walk measures a clause it has itself switched off while the hold saying « the machine is busy » is green over a reading already discarded. Found by phase 2, **re-met the hard way by R138's first version** — one unit, met twice | DESIGN § 3.1c-3 · R125's ordering comment · R138's docstring |
| 4 | **A DOM observer cannot see two writes in one task.** R137's « exactly one sentence » hold was vacuous: the message host is a component, both writes are batched into one render, and the DOM never holds the first value. **B-322's own first reading — « sampled every 180 ms, only the second is ever observed » — describes the limit of the instrument it was taken with**, and the rule reproduced that limit while quoting it | a mutation writing a second sentence into the verb on purpose: only the « both halves » hold fell |
| 5 | **R137 counted the prototype's WELCOME HINT as a sentence of the gesture** — the engine toasts it on a timer after boot, so the hold answered about a message no finger had asked for | spent through the engine's own « disappears on first interaction » contract rather than by copying its delay |
| 6 | **R137 read the screen BAR WHOLE**, so its subject came out « Retour Silo » and appeared in no message it was comparing | the printed subject |
| 7 | **A hold green over an unreadable value is not a hold.** R138's first « the pipeline is running » read `window.__mocks.mockState()`, which is NOT published: it answered `unknown`, and `unknown != "idle"` passed | it reads the layer AND the interface now, and disagreement fails |
| 8 | **R124 pressed with `act.click()`**, which dispatches on the node whatever covers it | the scrim raised on purpose (`z-[46]` → `z-[60]`): four holds fall now and name the coverer, where the old rule would have been ENTIRELY GREEN over a panel no finger could use |
| 9 | **R65's « closing a layer does not lose where the page was scrolled » was green over a page that could scroll 18 px.** The walk asks for `scrollTop = 300`, the boot page was shorter than its viewport, the port took **18** — and `after == before` then compared 18 with itself and passed with nothing to lose. The rule set an offset and never checked that it took. **Nothing about the interface changed when it went red**: a seeds commit lengthened the boot list and gave the hold its first subject in the lot's history | found by bisecting the fall over all 67 commits and probing the parent: « asked for 300, the port took 18 » · the hold that closes it reads « asked for 300, the port took 300 » and the rule goes 28 → 30 |
| 10 | **R136 passed the very box the operator was complaining about.** Its four holds read a type STEP and a padding step, and all four were green on a button rendered 332 x 245 with a 227 x 227 icon in it: the size had not come from a step at all — an `<svg>` with no width or height takes the replaced-element default and a flex box stretches it. A rule that reads only what a size was DECLARED as cannot see a size arriving from somewhere else | the fifth hold added and the icon's size hook emptied → `8 rules EXECUTED — 1 violation(s)`, naming « the button is 245.328125 tall, its icon 227.328125x227.328125, and the system's button is 44 », while holds 2 to 4 stayed green through it |
| 11 | **R158's « Agent Elvis » hold certified round three's C1** — found by the WAVE while repairing it. The hold read « the follow moved to `acquiring` » whatever the season answered, so a follow with nothing to get went `pending → acquiring` under a green rule | re-aimed to « the follow agrees with the answer »; mutation (the unconditional status write restored) R158 63 executed / 1 violation |
| 12 | **R158 counted the season act on an owned-and-followed sheet and never pressed it** (round two, B1). The press on Silo's own sheet created a SECOND follow and left the real one untouched while R158 stayed green | R160 (`followed_sheet_act.py`) presses the three twins by a finger: red 24 / 12, the phantom follows named |
| 13 | **R159 read the message's placement on a sheet, a panel, a confirmation and at rest — never what the top edge covers on a SCREEN** (round two, B2). On every screen the message lay over « Retour » and swallowed the tap for its whole life | leg 10 over five kinds of screen: red 26 / 10 |
| 14 | **R157 read text, dots and the queued mark — never the panel's scroll nor what lies under the finger** (round two, B3). The redraw threw a scrolled panel back to 0 and moved the pressed act from under the finger | the finger leg on a panel scrolled by a real touch stream: red 15 / 1, « scrollTop 25 → 0 » |
| 15 | **R157's double press ran against a mock answering in ~15 ms, so it never read the in-flight state** (round two, B4). With the answer held 2.5 s the pressed act showed nothing and further presses vanished | the taken leg under `latencyMilliseconds`: red 20 / 1, « aria-busy None » |
| 16 | **R159 never read a transition between the two edges** (round two, B6). A message up on a screen that closed crossed 708 px in one frame at full opacity | leg 11, read frame by frame: red 34 / 4 |
| 17 | **R155 held the list's containment and never read its end mark's words** (round two, B7). The emptied list said « La réserve en garde d'autres » under a footer saying the loaded reserve had ended | holds 4 and 5 read the resource's sentences: red 11 / 2 |
| 18 | **R158 emptied `#toast` before every act (`EMPTY_THE_TOAST`)**, so none of its holds was about a message on screen (round two's « what each does not read ») | re-aimed to the host's own close, the message held gone from paint: 56 → 62 executed; mutation (the gate back to `owns &&`) 59 / 2 |
| 19 | **R160's hold 3, « the follow that existed moves to `acquiring` », held a proxy and certified the defect** (round three, C1). « Furious » (5/5, up to date) went to « En cours d'acquisition » over an answer of zero, and R160 stayed green — and would have gone red on the repair | re-aimed with the repair: red 24 / 2 (Furious, President Curtis); mutation 24 / 2 |
| 20 | **R159's leg 11 stayed green over a crossing done at partial opacity** (round three, C3): moved 50 ms into its fade, a 660 px teleport at 0.4 opacity is never whole in two consecutive frames | the rule before the re-aim under `setTimeout(showAgain, 50)`: 43 executed, no violation; after it, 45 / 2 |
| 21 | **R157's taken leg read the follow PANEL's act only** (round three, C4 (a)): with `aria-busy` dropped from the media screen's season list, every rule stayed green | R157 before the new hold under that mutation: 25 executed, no violation; after it, 28 / 1 |
| 22 | **R155 read the LIST's end mark only** (round three, C4 (b)): with the exhaustion restricted to the list (`inList && isReserveExhausted()`), the pile went on offering a load after « Réserve épuisée » and R155 stayed green | R155 before the new hold under that mutation: 11 executed, no violation; after it, 13 / 1 |

**B-365 is the one to read twice, and not for its mechanism.** DESIGN § 3.1c-1 named that hold as
vacuous and wrote « Phase 6 owns `busy.py` and repairs it there ». Phase 6 repaired the sibling and
left it, and the design document went on carrying the sentence saying it would be met. **A wave that
records an instrument defect, assigns it, and then reads its own document as though the assignment
were the repair has invented a new way to be green over what it does not read** — one level up, in
the prose. It is counted inside unit 1 rather than as a ninth, because the defect is one hold and
the class was already recorded; what is new is the missed obligation, and that is what B-365 says.

**Five findings adjacent to this table and deliberately outside it**, because saying which is
which is the difference between a count that means something and a count that flatters: **B-363**
(`residue.py` cannot read a factory built from a shared constant) and **B-364** (a hit test that
throws on an SVG instead of pressing it) are LOUD — one reports, the other raises, and this table's
species is a hold that PASSES; and **B-366** (a grid tile promising a media sheet a follow has not)
is a product defect `audit.py` CAUGHT, so the instrument worked. **B-367 makes a fourth**: `drawer.tsx`'s comment claiming « The bump is what redraws the pressed
state » is measurably false, and a false comment READS nothing and MEASURES nothing — it is a
documentation defect, not an instrument green over what it does not read. L19's row excluded two
false claims on exactly that ground, and this table counts ONE species or it stops being comparable
across waves. The entry itself keeps the quotation and the four readings, which is where it does
its work. **B-369 makes a fifth, and it is
excluded for the same reason as B-363 and B-364**: a fixture rename that falsified a named state's own
premise is LOUD — `follow_verb.py` fell and named the empty emitter. It is a real defect of this
wave and it is filed as one; it is simply not this table's species, which is a hold that PASSES.

**Named by round three's reader without a finding — listed here, NOT counted**, because each is a
blind spot read in the rule's code and none has yet let a defect through: **R158**'s `MESSAGE_GONE`
accepts opacity 0 alone as gone, while the host stays hit-testable until its visibility step (the 450 ms
wait saves it today); **R157**'s finger leg holds `scrollTop` unchanged rather than what lies under the
finger, so a redraw inserting content ABOVE the act would stay green; **R84**'s `WRITE` matches only a
literal `setProperty("--tm-…`, so a writer through a variable or `style.cssText` is invisible; **R159**'s
leg 10 reads named states driven by `__go`, leaving the boot path, a rotation and a desktop width unheld;
**R160**'s three subjects are all dated/bare twins of ONE identity, so a base-title match across two
identities is unread; and **`priming.py`** and **`screen_addresses.py`** read « the screen is busy » as any
`[aria-busy="true"]` under it, which a season act in flight now satisfies too. A wave that touches one of
these rules takes its line with it.
...
### The fifteen after the first close, itemised the same way
...
**Twelve of the fifteen came from readers; three the wave found itself**, and saying which is the
point of a table about attribution. The twelve are units 17 to 28, returned by four independent
lenses on a wave whose every tier was green — 86 rules, `--a11y` at 0, `make check` at zero, the
oracle at 2 958 with no divergence. The three the wave found are **16** (its own truncated probe,
admitted here only because the review corrected the criterion), **29** (found by mutating the
drawing the hold had just been written for) and **30** (**B-289**, which the full suite's first run
after the repairs went red over).

*The heading and this paragraph read « the fifteen the adversarial review found » until the third
round: the cancellation described below swapped a unit the review found for one the WAVE found, and
the sentences that attributed them were not re-derived. Exchanging two rows does not preserve who
found what.*

**FOURTEEN OF THE FIFTEEN ARE IN INSTRUMENTS THIS WAVE WROTE OR HAD ALREADY REPAIRED**, which is the
reading the L11 row states and the L15 row states before it: the round that reads the previous
round's work is where the sharp findings are. Enumerated, because a proportion nobody can check is a
proportion nobody should print:

| Unit | The instrument | Written or repaired by this wave? |
| ---: | --- | --- |
| 16 | the R115 evidence probe | written by it |
| 17 | R115 `hold_the_priming` | written by it |
| 18 | R115 `hold_one_entry_one_owner` | written by it |
| 19 | `check-feedback-seam.py` | written by it |
| 20 | the CI filter and `test_ci_filter_covers_the_guards.py` | **neither** — pre-existing, and the finding is that TWELVE guards fall outside it |
| 21 | R113 `feedback.py` | written by it |
| 22 | R112 `press.py` | written by it |
| 23 | R118 `chrome_pixels.py` | written by it |
| 24 | R114 `poster.py` | written by it |
| 25 | R114 `poster.py` | written by it |
| 26 | R55 `touch.py` | repaired by it, twice, for B-276 |
| 27 | R113 `feedback.py` | written by it |
| 28 | R115's faded branch | written by it |
| 29 | R115's body hold | written by it, in this round's own repairs |
| 30 | `check-frame-domain.py` | **neither** — never touched by this wave, and it went red over prose the wave had just written |

Fourteen of fifteen, and the two exceptions are named rather than rounded away: one is a filter this
wave never edited, the other a guard it never edited either.
...
**B-397 — a panel re-produced after an edit pushes a second history entry.**
Measured by R166's own walk (`harness/settings_editing.py`): after typing into a setting's field and
filing the edit, shutting the panel took MORE THAN ONE `history.back()` — the rule walks the steps
in a bounded loop rather than driving `__panel.close()`, precisely so the extra steps stay visible
instead of being hidden by a seam. The cause is that a commit RE-OPENS the panel
(`window.__panel.produce("setting", identifier)`) and the panel is addressable, so each re-production
lays another entry carrying the same `?panel=` address.

**IT IS NOT THIS WAVE'S**, and that is the reason it is written down rather than repaired here: the
field's NATIVE commit path has re-produced the panel the same way since the panel became this
feature's, so « Valider » walks a road that was already there. What the wave added is a second door
onto one act, not a second entry.

**What it costs the reader**: a reader who edits three settings in one panel and taps Back finds the
panel still open, and taps again. B-290's family — a ladder with more than one shape for one gesture —
and the same owner: the ladder's HANDLER is L13's.

**What would settle it**: re-producing a panel at the SAME address replaces its entry rather than
pushing one, which is D1b rule 1 read literally (re-drawing an open surface is an adjustment, not an
...
second entry for the entry page and a Back that changed neither the address nor the page.
    ...
    tap [data-navgo=acq]   /acquisition              depth 6  page=acq   ← lands correctly
