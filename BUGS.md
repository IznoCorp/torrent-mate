...
<sub>The gate answers 401 for `/` itself, read with the error CAUGHT — `urlopen` raises on a 4xx and never reaches `.status`, so the first version of this line printed nothing and exited 1, which is a claim rather than a command: `python3 -c "import urllib.error as e, urllib.request as u"` then `try: u.urlopen(...)` / `except e.HTTPError as x: print(x.code)` → `401`.</sub>
...
`common.Journal` and is the rule that started the incident) · `grep -l "open_page"
frontend/maquette/harness/*.py | wc -l` · `grep -l "Journal(" frontend/maquette/harness/*.py | wc -l`
(the two ends `common.py` asserts at, which is what protects a rule run by hand)</sub>
