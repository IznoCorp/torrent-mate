...
| **Total** | **147** | 130 by the waves through L11's own five, plus the four readers' 13 recounted above — at 2026-08-31, after L10-bis, its review, L10-ter's survey and its review, **L15 with its own review**, and **L11 with FOUR readers**. **L11's FIVE are the mutation half of the curve, and they say what mutation IS good for**: every one was found by breaking the wave's own rules on purpose, and two of them are the same rule going green twice over a shell that was not there. A rule can hold a CONSEQUENCE while the mechanism it names is gone, and only a mutation asks. **What mutation could not reach is the other forty**: four readers on that same green gate found a security regression, an update discipline that never swapped the worker, and a queue a refused replay jammed forever — none of them a defect the author had thought to break. The ratio is this table's oldest argument, arriving on a wave that wrote BEHAVIOUR rather than conversions: 5 by mutation, some 40 by reading. **And a second round on the REPAIRS found thirteen more, then a third found seven** — the two later rounds are where the sharpest findings are, not the first. Round two: the repair for a jammed queue had made a refused mutation vanish SILENTLY instead, and its classifier destroyed the operator's action on a 503, a 429 and a 401 — the very outage the queue exists for. Round three: that repair's own replacement still treated **401 as final**, so an expired session destroyed every queued mutation one after another when a re-login would have saved them all; and the notice's button said « Réessayer maintenant » while doing something else, because its name, its words and its action were written as three ladders that tested their conditions in different orders. **Each round's worst finding was in the previous round's repair**, and none of the three was found by a gate. |
| **L12** (#540, this wave) | **6** | Every one found by MUTATION or by another guard, none by reading — which is the only reason they were cheap. **B-272**: the compositor manifest's floors carried slack while its own `taken_at` said « no slack to hide in » — `touch-action` at 8 against 11 real sites, so THREE declarations were deletable under a green guard, inside the instrument written to prevent exactly that deletion. Found because a two-declaration mutation produced ONE violation where two were owed: the rule fell, and falling was not enough. **B-274**: `page_host.py`'s state-alias arm read DOCSTRINGS as code — it strips quote characters deliberately, so a docstring ending « … the pressed state. » before `errors = []` flattens to `state. errors =` — and accused a rule that mutates nothing. B-085 with its SIGN REVERSED: red over what it should not read. **The viewport guard's own floors ran AFTER its summary printed**, so a run that fired one said « 0 violation(s) » on the line a human reads while exiting 1 — found by a mutation that looked like a rule which had not bitten. **R112's tolerance hold could not fail twice over**: its driver ramped the drift across the whole hold so the press timer fired before the tolerance was ever consulted, and under a real touch stream the tolerance is UNOBSERVABLE at all — the compositor cancels at every drift ≥ 14px whether it exists or not, measured both ways at seven distances. Only a real mouse isolates it. **R112's swallow hold could not fail either**: every `pointerdown` clears the mark, so a deliberate tap is never swallowed whatever the point check does — the check's real subject is a click arriving with NO pointerdown. **R113's reduced-motion hold was satisfied by a stylesheet saying nothing at all**: « nothing animates » is true of an absent rule, so reduced motion had to be read as a DESIGNED state — the marked node must look different — rather than as an absence |
