...
3. **A fix is not a fix without a rule that bites, and the rule is RUN.** Mutation-tested: break
   ...
   **When no instrument reaches the defect, building one is part of the work — never a reason to
   ...
   the first half of this rule _perfectly_ — a rule nobody wrote cannot fail its mutation — which is
...
| B-295 | React 19 assigns `innerHTML` on the prop OBJECT's identity, string unchanged or not — so every re-render of a page subscribed to the store's version recreates its engine-drawn children, at nine sites                        | by L14                | `fixed #547`  |
...
— and `{ __html: … }` written inline is a fresh object on every render. The library's incomplete
lens survives only because `LibraryPage` subscribes to `state` and a bump moves `version` alone;
the acquisition page subscribes to `version` on purpose (« Récupérer maintenant » moved a medium
and left every counter unchanged), so every action, and every cache landing `app/engine-redraw.ts`
turns into a `render()`, rebuilds its cards. **This is B-247's acquisition mechanism**: a tap
between `pointerdown` and `click` on any of those cards is lost whenever anything bumps the store.

**Owner: L14, phase 7. FIXED by #547.** `ui/markup.tsx` memoises the `{ __html }` object on its
string — the comparison React 18 made — and the thirteen sites in the files this lot cut go through
it. The library's window was the second mechanism and is repaired beside it: its draw key named the
store's `version`, so every bump emptied the window and rebuilt every row; it names what makes a
row's markup differ instead — the mode, the selection mode, and the first page's identity, which
structural sharing keeps while the rows are equal.

<sub>`frontend/maquette/harness/persistence.py` hold (f): on seven named states, every card, tile,
row, button, pill, image and key-value row is the SAME node after `window.__store.touch()` — 56, 88,
35, 61, 61, 110 and 27 captured, all kept. Mutations: the window keyed on a value that moves every
draw (16 of 61 kept on `lib-list`), and the markup object made fresh per render (5 of 56 kept on
`acq-now-loaded`).</sub>

<sub>drive `acq-now-loaded`, `quiet()`, hold a handle on `#view section[data-part="section"]` and
its first `[data-part="card"]`, `window.__store.touch()`, 250 ms: section `isSameNode` true, card
false, `innerHTML` equal · `grep -n 'case "dangerouslySetInnerHTML"' frontend/maquette/design/node_modules/react-dom/cjs/react-dom-client.development.js`
→ the `setProp` branch assigns unconditionally</sub>
...
**FIXED by L14 (#547), and the flag is not the one the brief named.** With placeholder data the
query reports `status: "success"` and `isPlaceholderData: true` while the read is still out;
`pending` is the state with NO placeholder at all. A rule reading `status === "pending"` is green
over the primed case, which is the whole case — so the flag is
`isPending || (isPlaceholderData && isFetching)` on the sheet, and `isPending` on the seasons,
which carry no placeholder. Field by field, never block by block: a part the placeholder carries is
drawn, a part it lacks stands as a `SkeletonLine` INSIDE the element that will carry the answer, and
the assertion is printed only once the read has landed. The body holds its eight children at every
instant. An errored read is not in flight — it has answered, and the screen prints what it can.

<sub>`frontend/maquette/harness/priming.py` (R119) — it thins the reference's `sheetFor` to
`{t, k, y}` for Broadchurch through an init-script setter, holds the read back 2 000 ms through the
mock layer's own knob, and reads the screen in flight (11 skeletons, 0 no-info) and landed (0
skeletons, one no-info naming the trailer). Its CONTROL walks the complete placeholder at the same
latency: 4 skeletons, the cast strip drawn and the synopsis a sentence — « zero skeletons » would
have been green over nothing and red over the design working. Mutations: `inFlight` forced false,
and the cast's branch forced to its assertion — hold (b) falls both times.</sub>
...
- **A FALL is not evidence of a defect** _by itself_, and specifically not when the suite ran it
...
fell » for a GUARD under mutation _whatever the guard says_: it decides by reading journal `FAIL`
...
installed application is declared _unless a written reason says why not_. This is that reason: a
...
**THE SURFACE HALF IS DISCHARGED BY L14 (#547); THE ROW STAYS OPEN FOR THE PRODUCER HALF.** The
seven surfaces L14 cut keep every node across a store bump, held by `persistence.py`'s hold (f) —
and the mechanism turned out to be two, one of them nobody had named: **B-295**, React 19 assigning
`innerHTML` on the prop object's identity. What is NOT repaired: the Découvrir containers the engine
fills, which move with their producers (L19); `features/maintenance/page.tsx`, where the defect was
first seen and which no lot has yet reduced — hold (f) says so rather than reading a surface the
repair did not reach; and the engine's bumping itself, which is unchanged. A re-render now keeps its
nodes; the engine does not bump less.
...
<span class="ps-dot ps-dot--done …" title="Temps réel connecté">
  <span class="ps-dot__d … bg-success"></span
  ><span class="ps-dot__label …">Connecté</span>
</span>
```
...
2026-08-20: _if it were true on main, CI would not have passed_ — and a gate that is green by
...
`scripts/check-frontend-boundaries.py`'s size arm is careful about the list's _composition_: it
...
seven-hold floor was a pre-satisfied counter _one wave after_ the wave that named that trap. L08's
...
_The heading and this paragraph read « the fifteen the adversarial review found » until the third
...
B-079's fix landed; § _Guards green over what they do not read_ exists, which is B-085's.
...
status outside § _Status vocabulary_; `invariant-numbers` is B-103; `corpus` PRINTS the number of
...
_the frame does not name the domain_. That invariant owns the number. « No French in the code » is
...
`3316e550`. B-219's row read `` `fixed #NNN` `` — the placeholder from § _Status vocabulary_'s
...
and left unbuilt.**
`IMPLEMENTATION.md`'s « In flight » row goes back to _none_ after a merge — the first post-merge
...
built**: _« a guard is code, and the steward who found this does not carry code (§ 7.2) »_. That
...
- **A guard whose subject no CI filter names runs in no job.** `IMPLEMENTATION.md` was named by no
  filter, and a post-merge gesture is _precisely_ a pull request touching that file alone.
...
> past tense: _that was the debt, and this phase paid it._
...
was written.** Asked on 2026-08-29 whether a legible button was enough: _« Elle réserve pas sa
place elle passe par dessus c'est une notification comme une autre, elle est fermable. »_ So the
...
> progress, which § 1 already assigned to it as _the only state_, and gains a « Landed, in order »
