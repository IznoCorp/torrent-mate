...
| **In flight** | **L02 — test anchors move to `data-*`**, Phase 0. Branch `refactor/maquette-l02`, version 0.98.12. It carries the `docs/cold-start` commit, which is not a PR of its own. **Phases 1 to 4 of 6 gated** — the instrument; the first contracts (`screen`, `sheet`, `scrim`, `data-open` on five layers); the card family and `deck/card`, with a named state so R26 can fall; the result list, the suggestions, the episode and its season — baseline 834 → 444, 83 states, ACC-03 and ACC-04 fallen; phase 5 (the flux's rows, the filter states, the settings and their fields — 76 entries) is next. Findings per phase in `docs/features/maquette-l02/plan/INDEX.md` § What phase N found. Every dispatch since the end of phase 1 runs on Opus: the DeepSeek provider is out of credit |
