...
| **In flight**              | **L07-bis — the tidy-up.** NOT A LOT, and it does not take L08's turn: a correction wave, arbitrated by the operator on 2026-08-25 immediately after L07 merged, clearing the ground L08 lands on. Seven arbitrations were taken by the operator and recorded in **#498**, whose four documentation commits this branch carries; this wave executes them and closes thirteen register entries. **Three L07 findings stay open on purpose** — B-061 (the oracle is arbitrated NOT widened), B-068 (the prose inventory) and B-071 (the design-notes toggle, which lives in the dying engine and belongs to L13). Nothing renders differently except two declarations on the keyboard skip link, which the oracle does not measure and which are named rather than smoothed over |
