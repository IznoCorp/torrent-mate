...
| **In flight**              | **none.** A wave writes its own row here when its pull request opens, and the post-merge steps move it out (§ 5 of the architecture file). **Held by `scripts/check-implementation-state.py` since 2026-08-29**: a row naming a version `main` has reached is refused, because the gesture that clears it had been missed at the close of L09, L10 and L10-bis in a row |
...
| **Next**                   | **L10-ter — the application template** first, and it is NOT a lot: a design phase whose deliverable is `frontend-architecture.md` itself, declared in § 1 there and defined in `docs/features/maquette-l10-ter/`. **§ 0's rule cannot elect it** — nothing schedules a non-lot — so this row is the only place that says it precedes. **Then L11 — Offline and PWA**, which is why: L11's own « Where it lives » line calls the service worker, the offline shell and the mutation queue FRAME rather than feature, and designing a frame after building its offline behaviour is the order reversed. **L12 and L14 also satisfy § 0's rule** once L10-ter has run, and L11 is first in the file's order |
