...
| **Then**                   | **A correction wave between L08 and L09**, scheduled by the operator on 2026-08-25. It carries the register entries whose subject is what the design screen asserts against what the repository holds — **B-079** (the host cannot name the commit it serves), **B-080** (the drawer states a hard-coded version and calls itself up to date) and **B-081** (design notes cannot be hidden, and the oracle measures without them) and **B-082** (`hidden` is inert on five elements, so an unseen button is still tappable) — plus what the audit of L08 added: **B-083** (L08 was never archived), **B-084** (a wave that found twenty defects wrote none of them in the register) and **B-085** (guards green over what they do not read — seventeen across three consecutive waves, counted by nobody). The audit is done, so this wave is ready to open. Until it lands, the layout the operator judges is not the layout the oracle proves
...
on it: no design, no plan, no branch. A wave opens with both — and that wave ARCHIVES L08's, which
is why `docs/features/maquette-l08/` is still here rather than under `docs/archive/features/`. What L08 leaves it, ready to use:
