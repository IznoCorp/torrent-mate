...
| 27  | Cycle 5 polish + flaky test fix           | phase-27-cycle-5-polish.md                    | [ ]    |
...
Execute Phase 27 via `/implement:phase` to close S1/S2/S3 + the flaky `test_indexer_budget_resume`. Then `/implement:feature-pr` + `/implement:pr-review` (will count as cycle 4 of the reset log).
