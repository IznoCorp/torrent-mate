...
| **In flight**              | **none.** A wave writes its own row here when its pull request opens, and the post-merge steps move it out (§ 5 of the architecture file). **Held by `scripts/check-implementation-state.py` since 2026-08-29** for a row naming a version; **a prose-only wave names none and is cleared by hand** (B-238) |
