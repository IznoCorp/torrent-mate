...
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last landed**            | **L08 — The data contract and the mocks**, PR **#503**, merged 2026-08-26, version 0.98.42, squash `ce1d7b5a`. The contract the interface REQUIRES as its own artefact — 49 paths, 53 operations, 43 schemas — with its TypeScript types generated and held against drift two ways. **46 seeds, every value taken out of `legacy.js` and none invented**, by a declared projection that renames keys and regroups positional arrays and never re-derives: that is what makes L09 provable, because a mock returning exactly what the fixture returns renders the same thing and the oracle holds the wiring at zero divergence. **The oracle reads 0 divergence over 2 739 measurements with the layer live** — this lot displays nothing, and that is its own proof. The demands on the backend are COMPUTED and not written (`docs/reference/frontend-backend-demands.md`). No backend work beyond the version line, no surface wired. **Seven arms hold the correspondence** (`scripts/check-mock-seeds.py`) and each says what it does NOT read before it says what it does; R85 holds the layer at run time with 20 holds. **Four adversarial reviewers read it before the merge, and what they found is the part worth reading.** The season tuple was `[number, aired, owned]` in the engine and the projection declared `[number, owned, aired]`, so a seed said one series holds 175 episodes of 117 — three independent cross-checks agree, and no automatic check could have caught it. A media sheet substituted one family's synopsis for another's on 213 of 259 titles, several of them in English — a re-derivation, which the rule forbids. Twenty provider identities name two sheet keys each, and taking the first returned nine EMPTY season lists for the three shows a reader opens first. `Setting.value` is the engine's `displayedValue` — a rendered French summary, one of them lossy. Four mutations changed nothing the next read could see. The guard reading the handlers did not read `state.ts`, which is the file a hand-typed row would live in. And the extractor required a TypeScript install the continuous-integration runner does not have, so the cheap tier would have been red for a reason foreign to every change under test |
| **In flight**              | **L08-bis — the correction wave between L08 and L09**, PR **#505**, version 0.98.43. **NOT A LOT**, and it does not take L09's turn: the single wave the operator arbitrated on 2026-08-25 to carry everything that had accumulated, in place of a trail of small pull requests. It closes **B-079** and **B-080** (the served identity, computed per request by `host_identity.py`, shown in the drawer — the operator's arbitration — and worded by `lib/served-identity.ts`, which owns the case nobody published one), **B-081** (the design-note pair restored in `harness.css`; the oracle reads NO divergence after it, which is the proof the measured document and the judged one are the same document again), **B-082** (preflight's `[hidden]` remedy alone, replacing a twelve-name hand-maintained list, and the action button taken away while a message is up), **B-083** (L08 archived, and a post-merge step for the gesture that slipped three waves out of eight), **B-084** (thirteen entries read out of #503's squash body — B-086 to B-098 — as CLASSES and not as names, with B-090 left `open` because it is not repaired) and **B-085** (the count exists: `BUGS.md` § Guards green over what they do not read, re-measured at every wave's close as step five of § 5). It also closes **B-099** (a test pass wrote 13 GB of real zeroes into `/tmp` and pytest kept three of them: 47 049 files before, 0 after, and the pass is a third faster). **Two new rules, R86 and R87, 52 holds between them, sixteen mutations seen red.** `make check` no longer fails where no npm install exists, and the skip names the three arms that need the parser rather than all seven. **Four adversarial reviewers read it before the merge, and what they found is the part worth reading.** A SEVENTH writer of the message state, which the wave's own seam counted as six — measured, the primary action was absent for four seconds on the first tap of every session. A branch name may legally end a script element, and `json.dumps` escapes nothing that stops it. A detached head rendered as a branch called « HEAD », which is the very incident B-079 records. `.note` was not only the prototype's annotation: twenty-seven product paragraphs wore it, four of them guidance, including the only sentence saying a maintenance rubric DELETES. And five hand-listed corpora in `check-no-french.py` meant a file split OUT of `serve.py` inherited none of its coverage. **B-082's register entry was wrong about three of its five elements, and the correction was wrong twice more** — `#nav` is the tab bar, not the drawer, and neither remaining element is ever given the attribute at all. B-085's table is where all of that is counted: **nine instances this wave, total 26** |
...
| **Next**                   | **L09 — The data layer, surface by surface** (depends on L01, L05, L08 — all `LANDED`), once the correction wave above has merged: no wave opens without its design and its plan. It also OWES THE UNIT-TEST LAYER, deliberately handed to it by L04 — the mock layer non-vacuous tests rest on is now built. The lots run strictly in sequence |
...
on it: no design, no plan, no branch. A wave opens with both. L08's design and plan are
ARCHIVED — `docs/archive/features/maquette-l08/`, by the correction wave, which is one wave later
than the post-merge steps ask for (B-083). What L08 leaves L09, ready to use:
