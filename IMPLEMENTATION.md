...
| **Next**                   | **L08 — The data contract and the mocks** (depends on L04 — `LANDED`): no wave opens without its design and its plan. The lots run strictly in sequence | |
...
**Next action**: **L08 — The data contract and the mocks** is the next lot, and nothing is open on it:
no design, no plan, no branch. A wave opens with both. L07 is archived under
`docs/archive/features/maquette-l07/`, and the two deferrals it left — the prototype fragment and
R72's renegotiation — are written into L13's entry in `docs/reference/frontend-architecture.md`,
not here.
