...
| **In flight** | **L02 — test anchors move to `data-*`**, Phase 0. Branch `refactor/maquette-l02`, version 0.98.12. It carries the `docs/cold-start` commit, which is not a PR of its own. **Phases 1, 2 and 3 of 6 gated** — the instrument; the first contracts (`screen`, `sheet`, `scrim`, `data-open` on five layers); the card family, fourteen tokens plus `deck/card`, and a named state so R26's no-poster branch can fall — baseline 834 → 490, 83 states; phase 4 (lists and episodes, 46 entries) is next. Findings per phase in `docs/features/maquette-l02/plan/INDEX.md` § What phase N found. Every dispatch since the end of phase 1 runs on Opus: the DeepSeek provider is out of credit |
