...
| 31  | PR review cycle 6 — trailer-aware dedup fixes | phase-31-pr-review-cycle6-trailer-dedup-fixes.md | [x]    |
...
**Next action**: All 31 phases complete — re-push `feat/registry`, verify CI green, then `merge_mode=manual` squash-merge PR #27. Phase 31 closed via `/implement:phase` (3 sub-phases + 1 caplog auto-fix): 31.1 shared `is_trailer_filename` predicate + VERIFY exemption, 31.2 `_find_video_file`/orphan-loop trailer exemption (+ D2/D3 tests), 31.3 orphan-loop robustness (surface OSError to result.warnings + root-canonical guard + D1 test). The caplog auto-fix root-caused a flaky phase-30 Gourou test (capture_logs could not intercept the cached stdlib `scraper` logger under xdist → switched to the project-standard `caplog`). Full suite green at 5683, `make check` exit 0. Known pre-existing intermittent flake unrelated to this feature: `test_registry_http::TestHalfOpenProbe` (cross-test isolation under `-n auto --dist load`; passes deterministically alone + in its file). Post-merge: re-run `/pipeline-monitor` (matrix v2.5) with the 2 Gourou torrents re-ingested + optional `/implement:archive`.
