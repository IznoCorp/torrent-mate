...
**Next action**: All phases complete (29/29 `[x]`). Cycle 5 verdict: **MERGE — fixes applied** (regression test pinned against production `_SHOWS_COUNT_SQL`, mutation-test verified ; phase-28 plan bullet rendering fixed). `merge_mode=manual` — squash-merge PR #27 via GitHub UI when ready, then optionally `/implement:archive`.
