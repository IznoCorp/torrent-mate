...
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last landed**            | **L19 — The producers**, PR **#558** merged 2026-09-05 (squash `9fa13da57`), version 0.98.69. Every surface the dying engine still PRODUCED moves to its feature: `grep -c "panel.open(" legacy.js` reads **0**, the inventory command lists only the harness panel, and `legacy.js` goes 32 461 → **31 645** non-blank. **The oracle read 2 958 measurements with NO DIVERGENCE on every one of the eighteen phases** — the conversion's own proof. The two behaviour phases land alone, each with its rule seen RED first: B-299's version-conflict banner and B-300's restart confirmation, the second saying what §17 makes it a confirmation for. Two delegation verbs move, each as two phases — the rule written against the engine and seen red, then the move read green **with its assertion count unchanged**. **SIX DEFECTS NOBODY HAD MEASURED**, each found by writing the rule rather than by reading the code: **B-309**, « Récupérer maintenant » THREW and took nothing (one unguarded branch swallowed every `data-take`; its rule was red with no mutation needed); **B-247's producer half live** on the poster gallery, where all sixty tiles were replaced on any store write, losing a tap between press and click; a **destructive** maintenance command that could have worn the SUCCESS tone; **§5**'s film/series distinction drawn and read by nothing; a settings panel that could have said the operator's edit did not take; and **`page_host.py` asserting B-300 as a requirement**. **WHAT IS MEASURED AND NOT DONE, said rather than implied**: the nine large fixture families did NOT die (26 375 lines, unchanged) — a producer was never their last reader, they sit behind `cardHTML`, `tileHTML` and `sheetFor`, which is L13's; DOIT-4's « En file » pastille **does not exist**, measured, so drawing it is L21's; and « the delegation handles only the frame's verbs » is CARRIED with its inventory rather than met, because moving the remaining verb readers is a behaviour surface the first review rule forbids a conversion wave — and B-309 is the evidence, since moving ONE uncovered it. Gates, each alone: full suite **92 rules** with `machine.py` the only failure (**B-308**, pre-existing); `--a11y` **0** with the light ceiling unmoved at 166; hold counts with `failed` read FIRST at **1**, EIGHT changed and five new, every movement upward; `make check` **11 132 passed, 0 failed, 0 errors**, coverage 91.09 %. **B-306 discharged**: a grandfathered file's size is recorded and refused upward. Five new rules — R120 to R124. **EIGHTEEN « guards green over what they do not read »** — nine by the wave, **seven by round one's four readers, and all seven are in INSTRUMENTS**, three of them one habit: a fact asserted by SEARCHING a file's text instead of parsing it, shipped twice in this wave under its own author's review. **Two more came out of REPAIRING those seven**, which is the closest range this table has recorded. **Six operator reports arrived during the closing gates** (B-310 to B-315), each walked on this head AND on a build of `origin/main` in a worktree of its own: **none of the six is this wave's**. **ROUND ONE returned 0 blockers, 5 majors and 21 minors, and every one is answered.** The plan's D5 refreshed in the wave that made its figures false, and the verb remainder placed in L13 and L21 on the operator's ratification of 2026-09-05. The deferred producer open put back inside the history suppression it escaped — `history.length` **4 → 4** with the repair, **4 → 5** with it mutated away. The size ledger's own RECORD made a ratchet, read at the merge base and refused upward: **1 violation with it, 0 with the call neutralised** — and its only CI job cloned shallow, which would have made it inert exactly where it matters. The restart confirmation opening on « Annuler » rather than on the act, which uncovered `focus.ts` asserting a priority a selector LIST cannot express. Three guards proved by SUBSTRING repaired to PARSE, one of them getting its first test file. R124 raised its panels through the seam, and a finger showed its pause half had been asking a panel nobody on that page could open. **B-287 armed and closed**: every one of this wave's seventy-five temporal comments reworded — the arm's own scan read **301 references in 98 files** when it landed, on this head and on `4c0e274a7` alike, so the wave adds none; it reads **299 in 97** on the final head, the two having gone when the contract's own dated lines were reworded at the source — and that corpus is frozen per file. And four entries filed that are nobody's here, **B-316 to B-319**. **ROUND TWO returned 0 blockers, 2 majors, one thing to establish against `main` and 17 minors, and every one is answered.** Both majors were the REGISTER not following the repairs — B-306 asserted discharged in three durable documents while `BUGS.md` read `open`, and B-299/B-300 wearing `to confirm` over bodies naming no mutation; both carry their readings now and B-306 reads `fixed #558`. **The thing to establish was a third defect and it is this wave's**: a cold addressed `setting:` reopen is refused where a build of `4c0e274a7` puts the panel back — measured in a worktree of its own, `open TRUE · 5 → 5` there against a refusal here — repaired in the SEAM and held by a rule that reads `open=False` with the repair mutated away (**B-321**). **Three of round two's findings landed in rules this wave itself wrote**, one of them caught by a mutation rather than a reader: the new hold evicted a query key this interface does not have, so it walked a warm cache and passed with the repair removed. Two entries are FILED rather than repaired, with the reason: **B-322**, whose clean fix adds lines to `legacy.js` that D5 forbids and the size ledger refuses with an exit code, and **B-320**, a hook-order defect in components this branch never touches. Gates re-taken: oracle **2 958, no divergence**; suite **92 rules, `machine.py` alone**; `--contracts` 18 + **27**; `--a11y` **0**, light **166**/166; hold counts `failed` **1**, eight changed and five new, every movement upward; `make check` **11 137 passed, 0 failed**. **ROUND THREE returned 0 blockers, 2 majors and 8 minors, and every one is answered** — both majors were FIGURES (a stale corpus count in three places, and R120's own new hold comparing `None` with « danger », a vacuous pass inside a hold added days earlier to refuse one). Repairing the second uncovered three more: the hold read UNSCOPED over a tree that persists, `add` was the one registered kind nothing drove, and my first repair made the rule fail on a correct panel by demanding an action per kind where `setting` legitimately offers none. **B-085's tally is recounted to 27 for this lot — 211 + 27 = 238** — sixteen of them added after the wave declared itself finished, every one by someone else looking. The rotation brief is `docs/features/maquette-l19/RESUME.md@9fa13da57` Its documents left the tree at this gesture; they are read with `git show 9fa13da57:docs/features/maquette-l19/REPORT.md` (and `RESUME.md`, `BRIEF.md`, `DESIGN.md`, `plan/`). **The gesture re-anchored the oracle and did NOT re-record the hold-count baseline**: `machine.py` was red for B-308 (six schedulers drawn, seven scheduled since #557), and a baseline recorded over a failed rule certifies a defect (B-291). **That obligation is DISCHARGED**: the B-308 micro-wave repaired the rule and re-recorded the baseline with `failed` read FIRST — PR **#567**, version 0.98.71. **AUDITED by the steward on 2026-09-05 — the office's six steps, on `main` at `427ea2bc8`**: the order respected (L14 before it, L15 · L09 · L12 landed); every « Done when » line true or discharged — a THIRD discharge (DOIT-4's pastille, measured absent, `partly` in the map, placed with L21) was recorded in the map and here and not in the plan's entry, and is now; the 27 repository guards clean; the oracle RE-CERTIFIED on a clean rebuild (**2 958 measurements, no divergence**, served copy stamped `427ea2bc8` `dirty: false` — the gesture's own certification had read a copy stamped `dirty: true`); two figures corrected with their command — the engine's 260 ms waits read **7 remaining, not 5** (9 before the lot, not 7: the counting command read one line, **B-323**), and `data-take` moved by HALF, its release-screen half placed with L21 (RATIFIED by the operator the same day); D5 now records the two delegation branches the lot added (`data-confirmrestart`, `data-reloadsettings`), and the operator ruled the exception covers them; B-299, B-300 and B-309 read `to confirm` and await the operator on a real device; the audit adds **zero** units of B-085's species |
| **In flight**              | **L21 — The tunnel's verbs**, PR **#572** (draft), version **0.98.79**, branch `feat/maquette-l21`. **The pull request is a DRAFT on purpose and opens before the work is done**: this repository's CI triggers on `pull_request` alone, so a pushed branch with no pull request carries ZERO check-runs and « CI green » cannot be read for it at all — measured on `67580575d`, `gh api …/check-runs` answering `total_count: 0`. It leaves draft at the lot's close, not before. **That reading was true when it was written and is no longer the whole of it**: on 2026-09-08 the operator ruled that CI must not run while a pull request is a draft, and every job in `.github/workflows/ci.yml` now stands down on one. So a draft carries no green reading either — **CI is read after `ready_for_review`, or on a draft by adding the `run-ci-on-draft` label**, which dispatches the run without taking the pull request out of draft. A draft is still the right shape for a lot that opens before its work is done; it is no longer how CI is read for it. **And « zero check-runs » now has a second cause**: it used to mean the run never started — no pull request, a `paths-ignore`, or a `CONFLICTING` pull request, which dispatches no check-suite — and it now also means the pull request is a draft. Read the draft state before diagnosing anything else. **This branch met BOTH causes within one hour**: it was reported `CONFLICTING` with `total_count: 0` while it was also a draft, and only the draft state told the two apart. **ALL SEVEN PHASES HAVE LANDED ON THE BRANCH.** **Phase 1, the contract** — three operations, their generated types, three recorded demands, mocks that MOVE state rather than reporting success over an unchanged world. **Phase 2, « Récupérer cette saison »** (B-301): R125 at 16 holds, written BEFORE the verb existed and RED against `main` with no mutation needed; it raises the panel by a finger and reads the call on the NETWORK, path included, because a hold reading the screen alone passes a build that toasts and sends nothing — which is what « Récupérer maintenant » did for a whole wave under a green gate (B-309). **Phase 3, the journey's two verbs** (B-302): R126 at 16 holds, seen red first at 9 violations, bringing `lib/verbs.ts` — a domain-free tap registry, because a panel action emits only `data-*`, `ui/panel` attaches no handler by contract, and the only reader was the dying engine, which D5 forbids growing. **Phase 4, the five acts** — `follow · dropsug · sugmore · pause · remove` leave the engine and the act grep reads **0**; ARM 7 of `check-markup-contracts` refuses a `data-*` verb the markup emits and nothing answers, seen red by hand on the real tree. **B-316 is CLOSED by the operator's ruling of 2026-09-06** — reading (i), a tap opens the media sheet and a long press the suggestion panel, reusing L14's gesture — and **B-315 (a)** landed with it — and was then REOPENED by the operator, who was describing a defect rather than a scale. Measured: the button rendered **332 x 245** with a **227 x 227** icon, because `svgIcon` emits an `<svg>` with no size and this button wears none of the legacy classes whose descendant rules size the action-button system's icons — the failure `base.css` already records at its `.installkey` rule. **R136's four holds read a type step and a padding step and passed all of it.** Both halves of his second ruling are delivered: `actionButton` now carries a BOUNDED `size` (`screen` or `footer`, each branch a string literal so `residue.py` keeps reading it), so a size outside the set is a compile error and no surface can hand a button one — all 19 call sites across 9 files pass no argument and their emitted class set is unchanged, held token for token by `variants.test.ts` — and the `footer` branch carries the icon's size. **The height is settled at 40 px with a 16 px icon, the operator's own answer**, held by R136's fifth hold, which compares the RENDERED BOX to the action-button system's own measured on a screen that draws one: 40 against 44, and the mutation emptying the icon hook brings back 245.328125 with a 227.328125 icon. **Phase 5, the release take** (B-322, B-323): **the collision was REMOVED, not guarded** — the orchestrator ruled road C, because two doors telling themselves apart by what a shared `data-take` value NAMES keeps the engine calling both, and `lib/verbs.ts` holds ONE handler per name. The picker emits **`data-pick-release`** and the panel keeps **`data-take`** and MOVED ONTO THE SAME REGISTRY; that second move is what makes the six-verb grep read 0 while arrivals keeps its name. Six ends moved in one commit, and the one a source grep misses is `frontend/maquette/a11y-light-debt.json`, which records accessibility debt **keyed by selector** and carried three spelled `button[data-take="1|2|3"]`. `function releases()` went with its last caller; **R137** and `exits.py` hold it, ledger 31 484 → 31 470 → **31 467**. **Phase 6, DOIT-4's pastille**: `data-part="season/queued"` on BOTH season surfaces, the fact held in the cache by `features/media/queued-seasons.ts`, the verb reporting its own outcome. **R138** at 7 holds, **mutated** — removing the mark makes two fall with `0 mark(s)` — and it reads the mark AFTER the message has gone, which is the whole of « visiblement »: a sentence shown for four seconds tells the operator who was looking and nobody else. **R124 is repaired and extended to 16 holds**: both panel actions are pressed through a hit test at the action's own centre, mutation-proved by raising the scrim (`z-[46]` → `z-[60]`) — four holds fall and name the coverer, where the old `act.click()` would have been ENTIRELY GREEN over a panel no finger could use — and the three new operations are swept by operationId under `busy.py`'s busy scenario. **NO NAMED STATE, and it is B-352**: `states.js` is grandfathered at 786 and road B was refused, so DESIGN § 4.0 carries the hand path instead. **Phase 7, the panel's doubled action** (B-313): the secondary journey action carries the guard « Voir la fiche » already had beside it. **R139** (`harness/panel_label_once.py`, 3 holds) is not written around the producer that was wrong — it raises EVERY panel a finger can reach on two surfaces and refuses a repeated label in any of them, which is the family this entry's own body asked for. RED on `main`'s producer with no mutation (`git diff origin/main HEAD` over `follow-actions.ts` was EMPTY at the reading): « Stuart Fails to Save the Universe (2026) » offering « Voir le parcours » twice, the operator's own subject by name; green after, with the panel count UNCHANGED at 18. It counts LABELS and not destinations because both of the operator's buttons carried the SAME destination, and its subject hold reads `data-journey` rather than the French word. **B-247's producer half** is discharged for this wave's surfaces: `persistence.py` reads `followsheet-complete`, `followsheet-gaps`, `sheet-journey` and `screen-releases`, each floor READ by raising it to 999 and taking the capture. **The numbers that gate the wave**: `legacy.js` **31 467** non-blank against a record of 31 467; the six-verb grep **0** (from 12); **109 rule files** (103 at the wave base — R136, R137, R138, R139, R155 and R156 are the six added). **Register**: B-301, B-302, B-313, B-315, B-322, B-323 read `fixed #572`; filed by this lot — **B-329**, **B-330**, **B-350**, **B-351**, **B-352**, **B-363** (`residue.py` cannot read a factory built from a shared constant) **B-364** (a hit test that throws on an SVG instead of pressing it), **B-367** (the drawer's appearance control paints the theme and leaves its selection behind — FILED, owner the settings micro-wave) and **B-368**; **B-365** and **B-368** now read `fixed #572` too. **B-366 was RE-RULED by the operator** — a follow without a sheet is not a state the product may represent — so its repair is to make the state unrepresentable rather than to guard the tile; **R156** refuses one in the prototype by asking the drawing's own resolver, which is a gate and not that repair, and the repair stays L13's. **DOIT-4 reads `served`** in the clause map; DOIT-3 stays `partly` because L20's watcher and global levers and L16's tracker policy are still owed — the plan asked for both rows to read `served` and the second would have been false. **B-085's tally: TEN for this wave, 250 + 10 = 260, RECOUNTED and itemised** in « L21's ten, itemised » — one line per unit with what establishes it. The recount changed the COMPOSITION and not the figure: the first version carried B-365 as a discovery, where DESIGN § 3.1c had already named that hold vacuous and assigned it to phase 6 (which repaired its sibling and left it), and it missed « the message is not in `#toast` » altogether. The two errors cancelled in the total and nowhere else — which is why the itemisation is the count. One of § 3.1c's four is excluded with its reason (a wrong answer in a mock handler is not a hold green over what it does not read), every one of them in the wave's own new holds and every one found by MUTATING them rather than by a gate going red, with B-363, B-364 and B-366 named and EXCLUDED — the first two because they are loud (one reports, one raises), the third because it is a product defect `audit.py` actually caught. **Zero by independent readers so far**, which on that table's own evidence is a figure awaiting its readers and not a total |
| **Between L19 and L21**    | **maquette-schedulers — the B-308 micro-wave**, merged 2026-09-06, squash `e9820e6a4`, version 0.98.71, PR **#567**. **NOT A LOT**, and it took nobody's turn: L21 is still next. It repaired the one failing rule of the full suite on `main` — `machine.py` read **6 drawn vs 7 real** since `personalscraper-index-full` was scheduled (#557) — so the wave after it does not inherit « one red, it is known ». The row could not be added where the list lived: the size ledger refuses `legacy.js` upward and D5 has the engine dying by subtraction, so the FAMILY left for the seed the Système page already read (`legacy.js` 31 645 → **31 591**, the ledger re-recorded downward in the same commit, the register marking it `converted` — 31 → **32**). `SCHEDULERS_DOWN` went with it, derived now in `features/system/fault.ts`. **Two findings that are not the missing row**: **B-324** filed — the BACKEND's own cron mirror names three of the seven and nothing reads it against anything, which is B-308's finding on the end with no guard at all — and D5's « 254 declarations republished on `window` », already false before this branch (**158** on `main`, **156** here), corrected with its method. **It discharged the obligation L19's gesture left open**: the hold-count baseline was re-recorded on the branch with `failed` read FIRST, and re-recorded again at THIS gesture on the squash (`failed` read FIRST: **0** — 92 rules, 2 180 holds, no movement against the branch's own record; only the pointer moved to the squash). **The review took SIX rounds, five independent readers and the steward's own re-take, each round a fresh reader on the previous round's REPAIRS**: 5 minors → 1 major + 5 → 1 major + 5 → 7 minors → 5 minors → **empty**. Both majors sat inside the previous round's fix, which is the office's oldest reading: Réglages « Les passages programmés » still drew SIX schedulers (pre-existing — the wave made the disagreement between two surfaces VISIBLE; the row cannot be added without converting the 1 461-line `SETTINGS` family, so the label half landed and the row half is **B-327**, L13's), and the hold added for it read `fr.json` from the MAIN checkout through an absolute path — green over the label's absence on any worktree. Seven of the readers' findings are instruments green over what they did not read and are counted in B-085's row. Five entries filed that are nobody's here — **B-324** (the backend's cron mirror), **B-325** (`machine.py` cannot be pointed at a build), **B-326** (the heavy lock's probe), **B-327**, **B-328** — and B-307 at its fourth instance. **Its folder left the tree at this gesture**: `git show e9820e6a4:docs/features/maquette-schedulers/BRIEF.md`. The oracle's reference re-anchored on the squash at this gesture **And a second one the same day: maquette-departure — the B-310 micro-wave**, merged 2026-09-06, squash `f70ca0295`, version 0.98.74, PR **#573**, brief at `docs/features/maquette-departure/BRIEF.md@f70ca0295`. **NOT A LOT**, and it took nobody's turn: L21 ran beside it. Three review rounds by three fresh readers, each on a worktree pinned at the head — **0/1/3 → 0/1/4 → 0/0/3** (blockers / majors / minors), round two's major INSIDE round one's repair (the arrivals' fill holds passing on the user agent's own `both`, held by NAME since), round three's three items prose. The brief's last step — the steward re-reading the transition's last frame on the operator's phone — was VOID before it was due: the operator ruled on 2026-09-06 that device readings are over (« Mac seulement »), and the frame had been read on the Mac as well. Two readings of one seam — a layer outliving the crossing it was supposed to leave on. **B-310**: the `animation:` SHORTHAND resets `animation-fill-mode` to `none`, which the user-agent stylesheet had set to `both`, so `panel-down` ended and the departing snapshot snapped back to the panel AS CAPTURED — open, opaque, at rest — for one frame above the arrived screen. The fill mode is declared as a longhand on all **three** pseudo-elements of that species (`banner-in` and `body-rise` snap back too and show nothing only because a NEW snapshot's un-animated state is its final one); `panel-down` keeps its 450 ms and its curve. **B-338**: `visibility: visible` is hit-testable as well as visible, so the closed scrim answered `elementFromPoint` at the centre of the arrived screen and swallowed the tap — `pointer-events-none` on the CLOSED state only, the fade and the delayed `visibility` untouched, R103's holds unmoved at 18. **One rule, R127** (`harness/departure.py`, 19 holds), seen RED first: `541ms opacity=1 transform=none panel-down=None` against `524ms opacity=0 transform=matrix(1,0,0,1,0,486.172) panel-down finished` — the animation RETAINED where it was dropped. Each mutation falls on its own holds alone (1 and 2 violations). **A measurement nobody predicted**: under `reduce` the crossing ends after THREE frames instead of thirty while the scrim's delay runs unchanged, so the lost-tap window is not shorter there but more than twice as long — **95 → 910 ms over 50 of 92 frames** against 559 → 924 ms over 23 of 64. Gates: hold counts `failed` **0**, 0 changed, one new row; oracle **2 958 measurements, no divergence**, reference NOT re-recorded; `--a11y` **0**, light 166/166; `make check` **11 170 passed, 0 failed, 0 errors**. **Four « guards green over what they do not read », total 250** — `transition.py` asserting the snapshot RUNS and never what it does when it ENDS, `exits.py` reading a leaving layer's `visible` and nothing else, while the property the codebase says that idiom is FOR — « the layer stays hit-testable until it has finished leaving » — is written at three OTHER sites (`ui/variants/layout.ts`, `harness/transition.py`, `styles/base.css`) and measured by none, `check-maquette-comments.py` printing clean over a wave that added a file to its own corpus — it reads the corpus size at `:279-281`, as a floor 10 % BELOW the record, and never asks whether the record has gone stale upward, which is the case that happened, and `check-bug-register.py`'s closure arm refusing an honest `fixed #573` on B-310 while ready to accept a silent closure of B-249, over one paragraph that its `BODY_HEAD` reads as a body head (**B-346**) — the last two caught by gates refusing this wave's own work, not by a reader |
...
| **Next**                   | **L21 — the tunnel's verbs**, since L19 merged on 2026-09-05. § 0's rule elects it: it is the first lot in the file's order that « Landed, in order » does not record as landed and whose dependency (L19) it does. The order is `frontend-architecture.md`'s and is not copied here. **L21 inherits from L19, on the operator's word of 2026-09-05, the acquisition's acts** (`data-follow`, `data-pause`, `data-remove`, `data-dropsug`, `data-sugmore`) written into its « Done when », and owns B-313 (RATIFIED by the operator on 2026-09-05) and B-316's surface. **The steward's audit of L19 (2026-09-05) wrote two more into that « Done when »**: DOIT-4's « En file » pastille on the resolve queue (the map placed it on the day), and `data-take`'s release-screen half with its 260 ms wait and B-322 (B-323), RATIFIED by the operator the same day. **Its brief is written: `docs/features/maquette-l21/BRIEF.md`** (the steward, 2026-09-05) — the B-308 micro-wave merged on 2026-09-06 (`e9820e6a4`), so the steward launches L21's agent NOW (operator, 2026-09-05: the steward launches, never the operator), with a one-line invocation naming the steward's address and the brief's path **AND THE LOT AFTER IT IS L20 — The global levers and the history**, whose dependencies (L15, L19, L10) « Landed, in order » already records as landed, so it is what § 0's rule elects the moment L21 lands. **This row still names L21 and that is deliberate**: § 0 elects the first lot the history does not record as LANDED, and L21 is in flight, not landed. The plan's close asks for this row to move to the lot after L21; moving it before the squash would make the row say something § 0's own rule denies, so what is written here is the election and its condition. **The row moves at the post-merge gesture**, with the « Landed, in order » history and the « Last landed » cell, which is where it has always moved. |
| **Before it**              | L14 — PR **#547**, version 0.98.65 · L12 — PR **#540**, version 0.98.58 · L11 — PR **#534**, version 0.98.57 · L15 — PR **#528**, version 0.98.55 · L10 — PR **#512** and **#513**, version 0.98.48 · L09 — PR **#509**, version 0.98.45 · L08-bis — PR **#505**, version 0.98.43 · L08 — PR **#503**, version 0.98.42 · L07-bis — PR **#500**, version 0.98.40 · L07 — PR **#494**, version 0.98.37 · L06 — PR **#490**, version 0.98.32 · L05 — PR **#482** · L04 — PR **#478** · L03 — PR **#475**, version 0.98.18 · L02 — PR **#470**, version 0.98.13 · L01 — PR **#467**, version 0.98.10. **Every one of them is in git history** — `git show 79ccebe2:docs/archive/features/<lot>/DESIGN.md` reads a lot's design, the tree holds no archive since 2026-09-01 — a fact stated once, because the sentence it replaces was a LIST that grew a clause per wave (« all three archived; L04 and L05 are archived beside them; L06 and L07 are archived beside them ») and had stopped at L07 while three more lots landed. This row exists beside « Landed, in order » to carry what that row does not: the pull request and the version. It carries nothing else, and it names no wave as « this pull request » — that phrase sat here for weeks pointing at a PR merged long before, which is B-152's shape and the reason this row was re-read at all |
| **Landed, in order**       | L01 · L02 · L03 · L04 · L05 · L06 · L07 · L08 · L09 · L10 · L15 · L11 · L12 · L14 · L19, plus the correction waves L07-bis and L08-bis, and the design phase L10-ter. **This row is a HISTORY — the order things LANDED, which is not the plan's order; that is `frontend-architecture.md`'s alone.** And it names no lot that has not landed, not even to say so: `check-frontend-boundaries.py`'s size arm reads this cell for its list of landed lots and refuses a ceiling label naming one of them, so a lot mentioned here in passing is a lot the guard reports as gone. Written after that arm went red over four labels for exactly that reason, in the pull request that added a clarifying sentence to this cell. **This row is the state the plan's § 0 reads** — since 2026-08-28 `frontend-architecture.md` carries the order and the dependencies and no status at all, so a lot's progress exists once and cannot go stale in a second copy |
...
« what is NEXT » and nothing else now — the table above owns what LANDED.

What L10 leaves L11, ready to use: a live relay installed at boot (`lib/relay.ts`), its condition
published for a component to draw (`lib/relay-condition.ts`), the events and the cursor
(`lib/relay-events.ts`), and a fake transport that can be silent, hung, refused or slow to close —
which is what L11 will need when a service worker starts intercepting the same connection. And two
things that are not the relay's: **`scripts/mutate.sh`**, which refuses a dirty tree so a mutation
can no longer destroy the repair it is testing, and the standing evidence that **an instrument
written by the person whose work it measures inherits that person's blind spots** — 4 findings by
