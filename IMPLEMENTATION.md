...
| **In flight**              | **L14 — The surfaces that outgrew their file**, PR **#547**, version 0.98.63, opened 2026-09-02. The four feature surfaces over the 400-line ceiling come back under it by decomposition — 756 → 48, 796 → 262, 613 → 102, 430 → 213 — and `--arm size` reads **2** at or over the ceiling, the engine's two, which is the contract's own line. `Icon` is written once, held at a hard zero by `scripts/check-component-once.py` and its ten tests. **B-283 and B-295 close; B-247's SURFACE half is discharged and its row stays open for the producer half (L19).** The wave found a mechanism nobody had named: React 19 assigns `innerHTML` on the prop OBJECT's identity, so every inline `{ __html }` recreated its children on every render — 32 sites, repaired through one `ui/` helper. **NINE INDEPENDENT READERS OVER TWO ROUNDS READ A GATE GREEN IN EVERY TIER AND RETURNED 13 THEN 9 MAJORS**, every one repaired here with its mutation seen red: a swipe-delete beyond the first page left the row on screen; seventeen markup sites and every icon's `<path>` were still rebuilt per render; the fields waited while `owns` and `isFilm` kept their defaults; and the error surface asserted a timeout over a 502 whose reason it held. **The sharpest finding was inside a round-1 repair**: the rule written for the delete regression was GREEN on the code it was written against — a door name that had moved, a precondition reading a scroll offset where it meant an index, and a wait that outlasted the refetch. Gates: oracle **2 958, no divergence** after every phase and every repair; full suite **87 rules and 26 guards**; `--a11y` 0 with the light ceiling unmoved at 166; hold counts three movements, all upward and all this wave's; `make check` at zero failures and zero errors. « Guards green over what they do not read »: **14 — two by the wave, twelve by the readers**, and the wave's own count was wrong twice in the flattering direction. One decision is flagged for the operator (D-L14-3): an open swipe survives a store write that changed nothing about the rows |
