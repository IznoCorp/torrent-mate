...
| **In flight**              | **L12 — Native interaction**, pull request **#540**, version **0.98.58**, opened 2026-08-31. Transitions, gestures, mobile geometry and the performance floor. **P25, P26, P11, P17, P5, P20, P24, P29 land; B-234 and B-252 close.** The press arbitration and pull-to-refresh leave the engine as vocabulary — `legacy.js` 32 604 → 32 420 non-blank, **184 lines subtracted, nothing added** (D5); their engine-side callers stay for L19. **The operator reversed D9's rule 2 mid-wave** — a maintained, proven library that solves exactly the problem is now PREFERRED to re-coding — and named `@tanstack/react-virtual` after a survey; the wave's own design had argued a refusal and that argument is void, kept above its replacement per § 7.1. **P6 IS BUILT**: the poster travels from the tile into the panel, plus transitions A and A-extended — the operator ruled on 2026-08-31 and corrected the premise this wave had declared blocked on (the media page keeps its fanart definitively, and `startViewTransition` animates any DOM change, not only a navigation). Building it exposed the wave's SHARPEST defect: **the page transition was DEGENERATE and every gate was green over it** — the snapshot was taken after the commit, so the « old » state was the new page, and R115 counted animations that showed nothing. Proved by a name existing on one side only. Gates: 85 rules + 25 guards, `--a11y` clean, hold counts **0 changed / 6 new / none lost**, `make check` 10 961 passed at zero errors, oracle 2 958 measurements no divergence. **Six rules were VACUOUS when first written and mutation said so every time** — that is the wave's real output. Register 147 → 151: **B-272** (the compositor guard's floors carried slack its own note denied — three sites deletable under a green guard), **B-274** (`page_host.py` read DOCSTRINGS as code — B-085 with its sign reversed), B-271, B-273 |
