...
### Cycle 3 (2026-06-02) — fresh full re-review (code / silent-failure / test-coverage)

**code-reviewer + silent-failure-hunter: NO actionable correctness findings** (order-table parity re-verified byte-exact vs legacy; `apply_fixes` ctx-threading correct; MediaFixer deletion clean; external_ids correct; every error path logs + surfaces). Retained = MEDIUM test gaps + LOW robustness, all fixed (`c8d67945`, `d01f5aec`; golden+parity stayed 9 passed; `make check` EXIT=0, 5925 passed, cov 91.24%):

- **A (MEDIUM, test)**: `DirNaming.fix()` target-exists collision (canonical dir already present → silent no-op) was untested → added `test_dir_naming_fix_target_exists_no_rename`.
- **B (MEDIUM, test)**: `catalog.list_checks()` flag-derivation + `run_check()` were not tested on the REAL singleton (they gate `--check` validation) → new `test_catalog.py` (6 tests: roster superset, per-spec fixable/indexable flags, run_check happy + KeyError).
- **C (robustness)**: `validate_library`'s `apply_fixes` block was OUTSIDE the per-item `try/except` → an unexpected raise aborted the whole disk scan. Now wrapped: logs `library_fix_error` (exc*info) + marks that item `issues` + continues (mirrors the read-path + `verify_all*\*`isolation). Test monkeypatches a`fix` to raise → scan completes, item flagged.
- **D (MINOR, UX)**: `library_validate --from-index --check <fs-only>` now prints an advisory Note that non-`IndexableCheck` names produce no results in DB-mode (no raise). CLI test added.
- No design contradictions. **Loop converged** (Cycle-3 findings were all MEDIUM-or-below and are fixed; remaining is the documented rglob sign-off item).
