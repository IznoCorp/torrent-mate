...
| **In flight**              | **L15 — The frame**, PR **#528**, version 0.98.55, opened 2026-08-30. The engine draws nothing of the frame: one navigation table where four copies stand, the chrome and the layers as components behind descriptors, the entry out of the engine, and the four behaviour changes each alone with its rule (B-229 the dialog's rung, B-237 its z-order, B-233 `theme-color`, B-230 the viewport fallback). B-142's instrument lands with it. Design and plan: `docs/features/maquette-l15/`. **This row is written when the pull request opens and moved out by the post-merge steps** (§ 5 of the architecture file); `scripts/check-implementation-state.py` refuses it once `main` reaches either the version or the pull request |
...
| **Next**                   | **L15 — The frame**, since L10-ter merged on 2026-08-30: § 0's rule elects it — it is the first lot in the file's order that the « Landed, in order » row does not record as landed and whose dependencies (L07, L09) it does. It sits BEFORE L11 in the file because an offline shell caches the chrome and the chrome is the engine's until L15 draws it; the whole argument is under its heading in `frontend-architecture.md` § 4. **L14 also satisfies § 0's rule** and the operator pulled it forward to the head of Phase 5 on 2026-08-30 (`docs/features/maquette-l10-ter/QUESTIONS.md` Q3) — it still runs after L15, whose brief is `docs/features/maquette-l15/BRIEF.md` (#524). The order from here: L15 · L11 · L12 · L14 · L19 · L20 · L16 · L17 · L18 · L13 — the file's order, never the numbers (L14 pulled forward by the operator on 2026-08-30) |
