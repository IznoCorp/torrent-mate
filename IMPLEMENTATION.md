...
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last landed**            | **L15 — The frame**, PR **#528** merged 2026-08-30 (squash `212faf0a`), version 0.98.55. **The engine draws nothing of the frame.** One navigation table where the same fact stood in four copies (`app/navigation.ts`, read app → lib so `features/x` cannot cycle back through it); the tab bar, the action button, the bottom slot, the message, the drawer, the confirmation and the popover as components fed by DESCRIPTORS; the entry's logic out of the engine. **Fifteen conversions, every one at zero oracle divergence**, and six behaviour changes each alone in its own commit with the rule that holds it — B-229 the confirmation's rung, B-237 its rank above the chrome, B-245 the pre-paint words, B-233 the status bar's ground, B-230 the viewport fallback, B-248 the bottom sheet over the tab bar. `SURVEY.md` § 1.1's inventory command read NINETEEN sites when the wave opened and reads NINE: seven of the Découvrir feed (L19's) and two of the harness panel, which ships nowhere. Four rules — **R100** the chrome persists, **R101** one ranked order hit-tested, **R102** the appearance and the status bar, **R103** a layer's exit is seen — and two guards, `check-viewport-directives.py` and B-142's `check-intent-map.py`, with which **B-244 closes at SEVEN guards rather than one**: the CI-filter hold asked whether a path is named by ANY filter where the question is whether it is named by the filter that GATES THE JOB. **75 rules and 22 repository guards clean, `--a11y` 0 violations, `make check` exit 0 at 10 936 tests, and 167 oracle divergences — all on `shell/sheet-content`, all B-248's, enumerated line for line.** **THEN THREE ADVERSARIAL READERS AGAINST THAT GREEN GATE RETURNED FIFTEEN MORE OF B-085's SPECIES AND TEN PRODUCT DEFECTS.** Thirteen of the fifteen are in the wave's own instruments, two of which had already been mutation-tested. **The other two are the finding that outlives the wave: THE ORACLE MEASURES REGION ROOTS.** `.dlg p` lost its `color` in the move and a destructive button rendered transparent with white text — **white on white under `data-theme="light"`, contrast 1.00** — and 2 958 measurements were truthfully green over both, as was the light-theme audit at exactly 166 before the repair and after it. A conversion wave can be proved at zero divergence, honestly, over a defect a reader sees at a glance in a child node. **B-085's figure for L15 is 12 + 15 = 27** — 12 by its own mutations, 15 by the review — total **125**. **B-247 and B-249's producer half stay open by decision**, named, in surfaces this lot may not extend (L14's, L19's) |
| **In flight**              | **L11 — Offline and PWA**, pull request **#534**, version **0.98.57**. The shell is precached and opens with the network gone (P7, R105, measured against a server that has actually STOPPED — `set_offline` does not reach a worker's own requests in Chromium); a mutation issued offline is held rather than rolled back, survives the process in IndexedDB, and departs exactly once (P8, R107); the three entry points Q4 elected are declared and the address `share_target` names really pre-fills (P9, R108); P27 and P30 arrive by the operator's arbitration of 2026-08-30. **B-256 rides it as phase 1** — a lock prevents, a stamp detects, and every rule is covered by `run.sh`'s wrapper (`ls frontend/maquette/harness/*.py | grep -v common.py | wc -l`); it was STILL open after the first repair, through `scripts/mutate.sh` and `scripts/harness-hold-counts.py`, which rebuilt the copy with neither lock nor stamp — the assembly is one function now and all three call it. **B-085's species: five by the wave, and FOUR ADVERSARIAL READERS ON THE GREEN GATE RETURNED SOME FORTY MORE** — including a security regression (the shell outlived the session), an update discipline that never swapped the worker, and a queue a refused replay jammed forever. The wave's own sharpest was R105 going green TWICE over a shell that was not there: once over Chrome's disk cache, once over a fallback entry that happened to BE the document. **79 rules and 23 guards clean, `--a11y` 0 violations over 87 states, the oracle at NO divergence over 2 958 measurements, `make check` exit 0 at 10 957 tests, and no rule lost a hold.** Three defects the rules found in the code they were written for: a fifteen-times reload loop, `platform.py` shadowing the standard library (B-260), and the design host answering 401 for `/` itself (B-259). A wave writes its own row here when its pull request opens, and the post-merge steps move it out (§ 5 of the architecture file). **Held by `scripts/check-implementation-state.py` since 2026-08-29** for a row naming a version; **a prose-only wave names none and is cleared by hand** (B-238) |
