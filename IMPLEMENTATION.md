...
| **In flight**              | **L10-ter — the application template**, branch `docs/maquette-l10-ter`, PR **#521**, opened 2026-08-29. **NOT A LOT** and no bump — prose alone, `no-version-bump` — which is why this row names no release: `check-implementation-state.py` reads a version here as a claim that a wave is landing. What it delivered: `docs/features/maquette-l10-ter/SURVEY.md` (19 drawing sites, nine surfaces, the engine draws no page and no screen, ten panel producers all the engine's), `MODEL.md` (the frame in thirteen parts, 24 testable properties with their instruments, B-142's arm specified), `QUESTIONS.md` (eight for the operator), `docs/reference/product-intent-map.md` (every DOIT and NE-DOIT-PAS clause mapped to its surface, verdict and owner), and the plan amended: **L15 the frame before L11; L19 the producers after L12; L16/L17/L18 for §18/§19/§17; L20 the control station; L13 re-cut**. Nine register entries, B-228 to B-236, written during the phase. A wave writes its own row here when its pull request opens, and the post-merge steps move it out (§ 5 of the architecture file). **Held by `scripts/check-implementation-state.py` since 2026-08-29**: a row naming a release `main` has reached is refused, because the gesture that clears it had been missed at the close of L09, L10 and L10-bis in a row |
...
| **Next**                   | **L15 — The frame**, once L10-ter merges: § 0's rule elects it — it is the first lot in the file's order that this row does not record as landed and whose dependencies (L07, L09) it does. It sits BEFORE L11 in the file because an offline shell caches the chrome and the chrome is the engine's until L15 draws it; the whole argument is under its heading in `frontend-architecture.md` § 4. **L14 also satisfies § 0's rule** and stays last unless the operator pulls it forward (`docs/features/maquette-l10-ter/QUESTIONS.md` Q3). The order from here: L15 · L11 · L12 · L19 · L16 · L17 · L18 · L20 · L13 · L14 — the file's order, never the numbers |
