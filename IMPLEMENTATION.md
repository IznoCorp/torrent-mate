...
| **In flight** | **L02 — test anchors move to `data-*`**, Phase 0. Branch `refactor/maquette-l02`, version 0.98.12. It carries the `docs/cold-start` commit, which is not a PR of its own. **Phases 1 and 2 of 6 gated** — the instrument, then the first contracts: `screen`, `sheet`, `scrim` and `data-open` on five layers, baseline 834 → 633; phase 3 (the card family, 143 entries) is next. Findings per phase in `docs/features/maquette-l02/plan/INDEX.md` § What phase N found. Every dispatch since the end of phase 1 runs on Opus: the DeepSeek provider is out of credit |
