...
| **In flight** | **L02 — test anchors move to `data-*`**, Phase 0. Branch `refactor/maquette-l02`, version 0.98.12. It carries the `docs/cold-start` commit, which is not a PR of its own. **Phase 1 of 6 gated** — the instrument: guard, classifier, hold-count capture, an 834-entry burn-down baseline; phase 2 (the `screen` contract) is next. Eight findings in `docs/features/maquette-l02/plan/INDEX.md` § What phase 1 found |
