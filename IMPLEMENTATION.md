...
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last landed**            | **L08-bis — the correction wave between L08 and L09**, PR **#505**, merged 2026-08-26, version 0.98.43, squash `12a134ca`. **NOT A LOT**, and it did not take L09's turn: the single wave the operator arbitrated on 2026-08-25 to carry everything that had accumulated, in place of a trail of small pull requests. It closes **B-079** and **B-080** (the served identity, computed per request by `host_identity.py`, published on the document `serve.py` sends, shown in the drawer — the operator's arbitration — and worded by `lib/served-identity.ts`, which owns the case nobody published one), **B-081** (the design-note pair restored in `harness.css`), **B-082** (preflight's `[hidden]` remedy alone, replacing a twelve-name hand-maintained list, and the action button taken away while a message is up), **B-083** (L08 archived, plus a post-merge step for the gesture that slipped three waves out of eight), **B-084** (thirteen entries read out of #503's squash body — B-086 to B-098 — as CLASSES and not as names, with B-090 left `open` because it is not repaired), **B-085** (the count exists: `BUGS.md` § Guards green over what they do not read, re-measured at every wave's close as step five of § 5) and **B-099** (a test pass wrote 13 GB of real zeroes into `/tmp` and pytest kept three of them — 47 049 files before, 0 after, and the pass is a third faster; it had filled the operator's boot volume). **Two new rules, R86 and R87, 52 holds, sixteen mutations seen red.** **THE ORACLE DID NOT MOVE FOR B-081**, which is that repair's own evidence: `html.measuring` had already hidden the notes during every capture, so the judged document and the measured one became the same document without a number changing. It moved for twelve measurements only — the seven states carrying one of the four `guidance()` paragraphs, each page growing taller, which is the arbitration below. **Four adversarial reviewers read it before the merge, and what they found is the part worth reading.** A SEVENTH writer of the message state, where the wave's own seam counted six: measured, the primary action was absent for four seconds on the first tap of every session. A branch name may legally end a `<script>` element and `json.dumps` escapes nothing that stops it — held on the script BODY, because the corrupted payload parses as valid JSON. A detached head rendered as a branch called « HEAD », which is the very incident B-079 records. `.note` was not only the prototype's annotation: twenty-seven product paragraphs wore it, and the operator arbitrated an audit of all of them — twenty-three are annotations, four are guidance, including the only sentence saying a maintenance rubric DELETES. Five hand-listed corpora in `check-no-french.py` meant a file split OUT of `serve.py` inherited none of its coverage. And the mock-seed guard turned every extractor failure into a silent pass. **B-082's register entry was wrong about all five of its elements, and the correction was wrong twice more** — `#nav` is the tab bar, not the drawer, and neither remaining element is ever given the attribute at all. **B-085's table is where all of that is counted: nine instances this wave, total 26.** Both references re-recorded from `main`'s tip at `12a134ca` and verified — one line moved in each, so all 2 739 measurements and all 1 320 hold counts are byte-identical to what was recorded before the merge |
| **In flight**              | **L09 — the data layer, surface by surface**, branch `feat/maquette-l09`, version 0.98.45. Eleven surfaces take their data from the L08 mock layer through a query cache (TanStack Query, D-L09-1): server state is the query cache's (invariant 4) — seven keys the DYING ENGINE still writes into the store are named, counted and ceilinged, and no component writes one, no surface asks for data from a `useEffect` (invariant 5), and each surface's mutations carry their optimistic path and their rollback. **Each surface's share of the fixture dies with it** (D5): the engine loses 21 families and 1 814 lines (35 263 to 33 449, counted), and `seedWorld()` is down to `{lib, removedLib}`. **The unit-test runner is decided and armed** (D-L09-2): Vitest in the node environment, no jsdom and no happy-dom installed — that absence IS the browser-free proof — held by `scripts/check-maquette-unit-tests.py`. **`app/shell.tsx` split on a SUBJECT before anything was added** (D-L09-5): the router tree, the history bridge, the scroll restoration and the panel host each got their own file, content sliced by line range and never retyped. **B-090 is repaired**: the settings say the value they HOLD, `features/settings/format.ts` renders it in the interface's own words, and its test asserts against all 159 committed strings and **all 159 reproduce exactly**. Two facts the value alone cannot carry are carried by the FIXTURE rather than guessed: a `precision`, because JSON holds one number for `4` and `4.0`, and a `schedule` KIND, because a cron expression's nature was written nowhere and the six cron settings were rendering as raw cron on screen. **Three new rules — R88 (boot order), R89 (the settle), R90 (the state surfaces) — 57 holds, and every one of them seen red on purpose.** Thirty-six register entries filed during the wave, B-102 to B-137 — fifteen of them by four adversarial reviewers reading the gate AFTER it went green, including **B-108**: the oracle's own `neutralise` removed `.note` nodes React draws, raised `NotFoundError` twenty-two times over 83 states, and had recorded four states BLANK as the reference. **The two references did not move for this lot's own work**: 2 772 measurements over 84 states, no divergence, and the eight that moved for B-108's repair are heights on the four states it had blanked. 62 rules, 16 repository guards, a11y over 84 states, `make check` exit 0 |
...
| **Next**                   | **L09 — The data layer, surface by surface** (depends on L01, L05, L08 — all `LANDED`): no wave opens without its design and its plan. It also OWES THE UNIT-TEST LAYER, deliberately handed to it by L04 — the mock layer non-vacuous tests rest on is now built. The lots run strictly in sequence |
