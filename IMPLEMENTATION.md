...
| **In flight**              | **L08-bis — the correction wave between L08 and L09**, PR **#TBD**, version 0.98.43. **NOT A LOT**, and it does not take L09's turn: the single wave the operator arbitrated on 2026-08-25 to carry everything that had accumulated, in place of a trail of small pull requests. It closes **B-079** and **B-080** (the served identity, computed per request by `host_identity.py`, shown in the drawer — the operator's arbitration — and worded by `lib/served-identity.ts`, which owns the case nobody published one), **B-081** (the design-note pair restored in `harness.css`; the oracle reads NO divergence after it, which is the proof the measured document and the judged one are the same document again), **B-082** (preflight's `[hidden]` remedy alone, replacing a twelve-name hand-maintained list, and the action button taken away while a message is up), **B-083** (L08 archived, and a post-merge step for the gesture that slipped three waves out of eight), **B-084** (thirteen entries read out of #503's squash body — B-086 to B-098 — as CLASSES and not as names, with B-090 left `open` because it is not repaired) and **B-085** (the count exists: `BUGS.md` § Guards green over what they do not read, re-measured at every wave's close as step five of § 5). **Two new rules, R86 and R87, 37 holds between them, eleven mutations seen red.** `make check` no longer fails where no npm install exists: `check-mock-seeds.py` asks the extractor whether it can run and names the seven arms it skips. **B-082's register entry was wrong about three of its five elements**, and B-085's own table is where that is counted — the wave found three instances of its own subject, including one in a rule it had written that morning |
