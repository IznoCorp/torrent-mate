...
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last landed**            | **L20 — The global levers and the history**, PR **#603** merged 2026-09-15 (squash `60c6d9b1d`), version 0.98.95, cut from `feat/maquette-l20` with L13b merged in at `9025348e6`. **NINE PHASES** across **three implementer sessions**, rulings L20-1–L20-10, rules R178–R185 + R187 (R186 unused), moving the global pipeline levers — pause, resume, the automatic watcher trigger — and the passages' history — a run's own detail screen, its raw output folded by default — onto Système and Arrivées' pilot bar, and closing B-296, B-297 and B-371. **The gate on the closure head** (`69cc558b5`): `make lint` 0; full suite `TM_HARNESS_JOBS=2` — 141 rules, 26 guards, no violation; `--a11y` dark 0, light 147 under its ceiling of 149 (not lowered on this branch); hold counts `--compare` no violation (states.py, chrome.py, screen_addresses.py, back.py, url_state.py, declared_codes.py, machine.py moved; new levers.py, locks.py, queued_by_hand.py, raw_log.py, run_history.py, watch_run.py); oracle 87×34 → 113×38, no divergence. **ONE READER ROUND** (office measure 2): **2 BLOCKERS, 6 MAJORS, 5 MINORS** (`r1-C.md`) — **C1–C8 repaired in round one on the branch**, one commit per finding, each hold read RED first then green, its mutation seen to fall through `scripts/mutate.sh`; **C9–C13 (minors) filed in the steward's docs pull request**, not repaired here. **Its folder left the tree at this gesture** — `docs/features/maquette-l20/{BRIEF.md,BRIEF-L20.md,BRIEF-L20-round1.md,DESIGN.md,REPORT.md,RESUME-L20.md,RULINGS-L20.md,plan/}@60c6d9b1d`. |
| **In flight**              | **None.** L20 merged 2026-09-15; L13r runs on its branch with no pull request yet — this row names a pull request, none is open (order 15). § 0's rule elects the next from « Landed, in order »; this row says « none » rather than naming a wave nobody has started, because a row that names one is read as a wave in progress |
