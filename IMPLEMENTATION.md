...
| **Next**                   | **L20 — The global levers and the history**, since L21 merged on 2026-09-11. § 0's rule elects it: it is the first lot in the file's order that « Landed, in order » does not record as landed and whose dependencies (L15, L19, L10) it does. The order is `frontend-architecture.md`'s and is not copied here. **The `maquette-settings` micro-wave runs FIRST, by the operator's ruling of 2026-09-06** (`frontend-architecture.md` § 5, and its brief, `docs/features/maquette-settings/BRIEF.md`): it rests on L21's verb registry and takes no lot's turn, so this row names L20 and no « Between L21 and L20 » row is written until that wave lands |
