...
| **Next**                   | **L19 — the producers**, since L14 merged on 2026-09-03. § 0's rule elects it: it is the first lot in the file's order that « Landed, in order » does not record as landed and whose dependencies it does. The order from here: L19 · L20 · L16 · L17 · L18 · L13 — the file's order, never the numbers. **L19 owns B-247's producer half**, which L14 left open on purpose: the surface keeps its nodes across a store write, and the containers the dying engine fills on Découvrir are the other half of the same defect |
