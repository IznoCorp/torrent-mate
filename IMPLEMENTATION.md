...
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last landed**            | **L13a — What moves unchanged**, the first sub-lot of L13, PR **#596** merged 2026-09-14 (squash `304346145`), version 0.98.92. **NINETEEN CONVERSION PHASES** (a·1–a·19) across **thirteen implementer sessions**, rulings 1–63, moving the harness module, the ladder, the boot and the drawing out of the dying engine with no behaviour change — oracle at zero divergence, hold counts moved only where the resume names a re-aim. **The gate on the pre-review head**: full suite 126 rules + 26 guards, no violation; `--a11y` 0 violations, light 149 (at the ceiling); `--compare` no violation (R80 `residue.py`'s 19 missing with the file itself, R163 3 → 2, R72 4 → 3); `make check` **11 259 passed, 0 failed, 0 errors**. **ONE READER ROUND** (office measure 2): **2 BLOCKERS** — **A1**, the follow panel typed onto any page but the Médiathèque, for a medium the library holds and nobody follows, never got « Voir la fiche », the served owned cells or its poster, because the follow producer's `needs` carried no read for that identity; **A2**, every row of Acquisition › Suivis in list mode drew the poster as initials, because the row's descriptor copied every field of the follow but `poster` — **both repaired in L13a-bis, by a fresh session, before the merge**: R176 `cold_follow_panel.py` and R177 `follows_list_posters.py`, each seen red first and held by a `mutate.sh` proof on the repair it names. **7 MINORS filed** (B-504, B-506 to B-511): a rule with no deadline that hangs the served-copy lock instead of falling (A7); the notes control drawn with no handler on a mocks-off build (A3); a plan mutation that does not fall alone (A4); a failed typed media address that keeps an empty title (A5); a hold that never asserts the sheet it reads was open (A6); a restoration held by a crash instead of a named hold (A8); a register note that contradicted its own measurement (A9, corrected in B-503's row). **Its two sub-lot documents left the tree at this gesture** — `docs/features/maquette-l13/BRIEF-L13a.md@304346145` and `RESUME.md@304346145` — **the lot's folder stays**: L13b and L13c still read its `DESIGN.md` and `plan/`, and the folder dies only at L13c's own gesture. |
| **In flight**              | **None.** L13a merged 2026-09-14; L13b runs on its branch with no pull request yet — this row names a pull request, none is open (order 15). § 0's rule elects the next from « Landed, in order »; this row says « none » rather than naming a wave nobody has started, because a row that names one is read as a wave in progress |
