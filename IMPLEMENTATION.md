...
| **Next**                   | **L13 — The engine's residue**, by the operator's seventh measure of 2026-09-12 (`frontend-architecture.md` § 4, the re-order recorded beside the 2026-09-02 re-cut): the engine's death runs next, once the micro-waves in flight that day land, and § 0's rule elects it under the amended order — its dependencies (L07, L09, L12, L15, L19) are all in « Landed, in order ». The order is `frontend-architecture.md`'s and is not copied here. **L20 — The global levers and the history — follows L13**: its design and plan landed at #587, squash `d9221ebf9`, on 2026-09-12, and the drawing stays on `main` for the operator to read before that lot opens. The `maquette-settings` micro-wave (2026-09-06 ruling) took no lot's turn and is traced in « Between L21 and L20 » once merged |
