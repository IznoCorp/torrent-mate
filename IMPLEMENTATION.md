...
| **In flight**              | **L09 — the data layer, surface by surface**, branch `feat/maquette-l09`, version 0.98.44. Eleven surfaces take their data from the L08 mock layer through a query cache (TanStack Query, D-L09-1): server state lives in the cache and nowhere else (invariant 4), no surface asks for data from a `useEffect` (invariant 5), and each surface's mutations carry their optimistic path and their rollback. **Each surface's share of the fixture dies with it** (D5): the engine loses 21 families and 1 814 lines (35 263 to 33 449, counted), and `seedWorld()` is down to `{lib, removedLib}`. **The unit-test runner is decided and armed** (D-L09-2): Vitest in the node environment, no jsdom and no happy-dom installed — that absence IS the browser-free proof — held by `scripts/check-maquette-unit-tests.py`. **`app/shell.tsx` split on a SUBJECT before anything was added** (D-L09-5): the router tree, the history bridge, the scroll restoration and the panel host each got their own file, content sliced by line range and never retyped. **B-090 is repaired**: the settings say the value they HOLD, `features/settings/format.ts` renders it in the interface's own words, and its test asserts against all 159 committed strings — 152 reproduce exactly, the seven that do not are the ones JSON cannot carry, and the contract gains a `precision` carried by the fixture. **Three new rules — R88 (boot order), R89 (the settle), R90 (the state surfaces) — 55 holds, and every one of them seen red on purpose.** Twenty register entries filed during the wave, B-102 to B-121, including **B-108**: the oracle's own `neutralise` removed `.note` nodes React draws, raised `NotFoundError` twenty-two times over 83 states, and had recorded four states BLANK as the reference. **The two references did not move for this lot's own work**: 2 739 measurements, no divergence, and the eight that moved for B-108's repair are heights on the four states it had blanked. 62 rules, 16 repository guards, a11y over 83 states, `make check` exit 0 |
