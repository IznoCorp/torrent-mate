...
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last landed**            | **L09 — the data layer, surface by surface**, PR **#509**, merged 2026-08-28, version 0.98.45, squash `27096f31`. Eleven surfaces take their data from the L08 mock layer through a query cache (TanStack Query, D-L09-1): server state is the query cache's (invariant 4) — seven keys the DYING ENGINE still writes into the store are named, counted and ceilinged, and no component writes one, no surface asks for data from a `useEffect` (invariant 5), and each surface's mutations carry their optimistic path and their rollback. **Each surface's share of the fixture dies with it** (D5): the engine loses 21 families and 1 814 lines (35 263 to 33 449, counted), and `seedWorld()` is down to `{lib, removedLib}`. **The unit-test runner is decided and armed** (D-L09-2): Vitest in the node environment, no jsdom and no happy-dom installed — that absence IS the browser-free proof — held by `scripts/check-maquette-unit-tests.py`. **`app/shell.tsx` split on a SUBJECT before anything was added** (D-L09-5): the router tree, the history bridge, the scroll restoration and the panel host each got their own file, content sliced by line range and never retyped. **B-090 is repaired**: the settings say the value they HOLD, `features/settings/format.ts` renders it in the interface's own words, and its test asserts against all 159 committed strings and **all 159 reproduce exactly**. Two facts the value alone cannot carry are carried by the FIXTURE rather than guessed: a `precision`, because JSON holds one number for `4` and `4.0`, and a `schedule` KIND, because a cron expression's nature was written nowhere and the six cron settings were rendering as raw cron on screen. **Three new rules — R88 (boot order), R89 (the settle), R90 (the state surfaces) — 57 holds, and every one of them seen red on purpose.** Thirty-six register entries filed during the wave, B-102 to B-137 — fifteen of them by four adversarial reviewers reading the gate AFTER it went green, including **B-108**: the oracle's own `neutralise` removed `.note` nodes React draws, raised `NotFoundError` twenty-two times over 83 states, and had recorded four states BLANK as the reference. **The two references did not move for this lot's own work**: 2 772 measurements over 84 states, no divergence, and the eight that moved for B-108's repair are heights on the four states it had blanked. 62 rules, 16 repository guards, a11y over 84 states, `make check` exit 0 **The reviewers are the story of this wave**: four ran against a GREEN gate — 62 rules, 16 guards, the oracle, `make check` — and returned fifteen findings, nine of them instruments of this very lot that did not read what they claimed. **B-123**, the settings drawing a cron raw inside B-090's own repair, with a test that derived a rule the application derives nowhere and then asserted the rendering that derivation produced. **B-136**, B-090's headline figure counting quotes — 110 was `JSON.stringify`'s answer, the value is 59, and it had reached the contract and its generated types. **B-135**, three named states measuring the panel left open by the state before them, off screen, for as long as the reference has existed — found because a ninth field state moved all three by 51.6 px without touching a line any of them draws, and whose two candidate repairs were WRITTEN AND BACKED OUT because measurement showed neither changed anything. **B-085's count for L09 is 14 and the total is 40.** **Two arbitrations are left to the operator, written beside the code**: `lib/queue.ts` carrying 169 domain words against invariant 10 (and whether B-100 is worth arming now that there is something to refuse), and `engine/engine-shape.ts` exempt from the fan-in ceiling at 13 importers against 4, with the case for and against side by side. **And one gap is stated rather than reworded away**: the lot's `Done when` says the fixture literals have left the engine, and 60 families remain against 21 converted, because their surfaces are still engine-DRAWN — L13's subject. Both references re-recorded from `main`'s tip at `27096f31`: ONE line changed in each, the recorded sha, so all 2 772 measurements and all 1 381 hold counts are byte-identical to what was recorded before the merge |
| **In flight**              | **L10's third-review repairs**, version 0.98.48, PR **#513**, branch `fix/maquette-l10-round-three`. **#512 MERGED AT `47fe0c09`, WHICH IS BEFORE THEM** — so `main` carries `teardownAsked` live, no `open` listener, and register rows reading `fixed #512` for defects that are not fixed there. That is B-190's disease on `main`, and this branch is its correction. The wave's own row is below. ⟨L10 — The live relay⟩, PR **#512**, merged 2026-08-28, squash `3c66dd36`, version 0.98.47. The event stream and the cache invalidations it drives. `lib/relay.ts` is the transport and names no domain word; each feature owns its own event→key table (`features/<domain>/live.ts`); `app/live-updates.ts` composes them, one import per feature. **It is installed at BOOT, never mounted with a surface**, and that is correctness rather than placement: `staleTime: Infinity` with no focus and no reconnect refetch means a query that misses its invalidation is stale for the life of the process (B-154). **The stream is simulated at the TRANSPORT** (D-L10-3) — a fake `WebSocket` obeying `web-ui.md`'s protocol, accept-then-close `4401` included — not by a bypass that would leave the handshake, the backoff, the cursor and the ping reply walked by no proof. **§8 is the § it serves, and the header was its exact inversion**: it carried « Connecté » as a LITERAL beside a green dot, with no connection anywhere in the prototype (B-155). Five new rules — **R91** (the fan-out, measured against the query cache, both directions), **R92** (the four drawn conditions, read by their colour as much as their words), **R93** (the gap replayed, never reloaded, and never emptied), **R94** (B-140, and what a layer is), **R95** (the interface never says connected over a dead link) — plus R89 extended for the stream and R85 for the protocol. **67 rules, 1 633 holds, no count fell**; `check-live-relay.py`'s three arms in `make check` and the `--contracts` tier; `make check` exit 0 at 10 849 passed. **The oracle GREW rather than moved**: 87 states x 33 regions, 2 871 measurements, 3 states added, 0 removed, **not one of the 84 existing measurements changed** — the reference's own diff says the same thing read twice. **B-085's figure for L10 is 32 — 4 by its own mutations, 9 by a first adversarial review, 11 by a second review of the repairs, 8 by a THIRD review of those** — total **73**. Six of the last eleven sit in instruments written in response to a review, mutation-tested, and believed. The reading is not « review harder »: an instrument written by the person whose work it measures inherits that person's blind spots, whatever the discipline. **AN ADVERSARIAL REVIEW OF SEVEN READERS RAN AGAINST A GATE GREEN ON EVERY TIER**, and it is why this row is not the one that was written first. Five PRODUCTION defects, three of which made the lot's own §8 promise false: the relay reported « connected » over a dead link by three independent routes — a half-open socket with no watchdog, a hung 101 upgrade with no deadline, and an unsolicited clean close, **which is how this connection ends on every merge**. The notice announced the SESSION's start as the age of the data. A refusal was inescapable. And the map refused `DownloadStarted`/`DownloadProgressed` as « per tick » when they fire once and thrice, while mapping `ItemProgressed`, which fires ~540 times a pass. **A SECOND ADVERSARIAL REVIEW READ THE REPAIRS**, because the twelve correction commits had had no independent reader — and it found thirty-three more, six severe, **six of them in the repairs themselves**. The sharpest: the fix for « the relay says connected over a dead link » introduced a teardown flag that nothing consumes when the socket is already null (every retry from `refused` or `lost`) and that a real browser's asynchronous `close()` never lets be consumed at all — so the next unsolicited `1000`, one per deploy, was swallowed and B-160 came back through the door of its own fix. And `reset()`, taught to zero the settle counters so a desynchronised page had a way back, could drive them NEGATIVE, where `quiet()` never resolves again. **A THIRD REVIEW READ THE SECOND'S REPAIRS AND FOUND NO PRODUCT DEFECT AT ALL** — its two criticals were an ABSENT repair and a DISCARDED count. The first is the worst entry in the register: a commit describing the deletion of the `teardownAsked` flag across five paragraphs **did not touch `relay.ts`**, because a mutation's `git checkout --` had reverted the file to HEAD where the repairs were not yet committed. B-177 for the third time in one wave, and the register's own B-190 — « a false claim about a repair is worse than the defect » — broken by the commit that filed it. **The answer is a tool, not a resolution**: `scripts/mutate.sh` refuses a dirty tree and restores from the INDEX, so the failure mode is gone rather than watched; its first use immediately found that the cursor's thaw was measured by nothing. And `MockSocket.close()` now dispatches asynchronously like a real browser, which is what made the close-then-replace race unrepresentable and let one defect survive two reviews. Sixty-five register entries in all, B-152 to B-217 |
...
| **Next**                   | **L10 — The live relay** (depends on L09, landed): no wave opens without its design and its plan. The lots run strictly in sequence. **L11, L12 and L14 also satisfy § 0's rule** — L11 depends on L09, L12 on L05 and L07, L14 on L07 and L09 — and L10 is first in the file's order. **L14 is new, added 2026-08-28**: it carries the decomposition of the four surfaces over the 400-line ceiling, a debt that had been labelled L09's and that L09 landed without paying |
| **Before it**              | L07 — PR **#494**, version 0.98.37 · L06 — PR **#490**, version 0.98.32 · L03 — PR **#475**, version 0.98.18 · L02 — PR **#470**, version 0.98.13 · L01 — PR **#467**, version 0.98.10. All three archived under `docs/archive/features/`; L04 and L05 are archived beside them (L04 by #482, L05 by this pull request). L06 and L07 are archived beside them |
| **Landed, in order**       | L01 · L02 · L03 · L04 · L05 · L06 · L07 · L08 · L09, plus the correction waves L07-bis and L08-bis. **This row is the state the plan's § 0 reads** — since 2026-08-28 `frontend-architecture.md` carries the order and the dependencies and no status at all, so a lot's progress exists once and cannot go stale in a second copy |
