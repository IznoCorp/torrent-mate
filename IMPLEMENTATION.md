...
|                            |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Last landed**            | **L10 — The live relay**, PR **#512** merged 2026-08-28 (squash `3c66dd36`) and PR **#513** merged 2026-08-29 (squash `be12a697`), version 0.98.48. **TWO pull requests, and the second is the lot's sharpest lesson.** The event stream and the cache invalidations it drives: `lib/relay.ts` is transport and names no domain word, each feature owns its own event→key table, `app/live-updates.ts` composes them one import per feature. It is installed at BOOT, never with a surface — `staleTime: Infinity` with no focus and no reconnect refetch means a query that misses its invalidation is stale for the life of the process (B-154). The stream is simulated at the TRANSPORT (D-L10-3): a fake `WebSocket` obeying `web-ui.md`'s protocol, accept-then-close `4401` included, and closing ASYNCHRONOUSLY as a real browser does. §8 is the § it serves, and the header was its exact inversion — it carried « Connecté » as a LITERAL beside a green dot with no connection anywhere (B-155). **THREE ADVERSARIAL REVIEWS, THIRTEEN READERS, AND THE CURVE IS THE FINDING.** Round one: seven readers on a gate green on every tier — 5 PRODUCTION defects, three of which made the lot's own promise false, including a relay that reported « connected » over a dead link by three independent routes and a map that refused `DownloadStarted`/`DownloadProgressed` as « per tick » when they fire once and thrice while mapping `ItemProgressed`, which fires ~540 times a pass. Round two: three readers on the REPAIRS — 6 severe, **all six in the repairs**, including a fix that reintroduced the defect it repaired through a flag nothing consumed. Round three: one reader — **no product defect at all**, and its two criticals were an ABSENT repair (a commit describing it in five paragraphs and not touching the file, because a mutation's `git checkout --` had reverted it) and a count made and discarded two lines later. **The two structural answers are what end it**: `scripts/mutate.sh` refuses a dirty tree and restores from the INDEX rather than HEAD, so the failure mode that destroyed those repairs three times is gone rather than watched; and the fake's asynchronous close makes the close-then-replace race representable, which is why one defect had survived two reviews. Five new rules — **R91** (the fan-out, both directions, against the query cache), **R92** (the four drawn conditions, by their colour as much as their words), **R93** (the gap replayed, never reloaded, never emptied), **R94** (B-140, and what a layer is), **R95** (the interface never says connected over a dead link) — plus R89, R85 and R88 extended. **67 rules, 1 633 holds, no count fell**; `check-live-relay.py`'s four arms in `make check` and the `--contracts` tier; a11y 87 states; `make check` exit 0. **The oracle GREW rather than moved**: 87 states x 33 regions, 2 871 measurements, 3 states added, 0 removed, **not one of the 84 existing measurements changed** — confirmed by `--check` and by reading the reference's own diff, twice more after each squash. **B-085's figure for L10 is 32** — 4 by its own mutations, 9 by the first review, 11 by the second, 8 by the third — total **73**. Sixty-six register entries, B-152 to B-218 — the last of them found while CLOSING the lot, in this very table: B-152's shape a third time (B-218) |
| **In flight**              | **L10-bis — the correction wave**, branch `fix/maquette-l10-bis`, version 0.98.51, PR #516. **NOT A LOT, and it takes nobody's turn**: L11 is still next, behind L10-ter. It executes the twenty-two entries the operator arbitrated between 2026-08-28 and 2026-08-29 plus two findings from the steward's audit of L10, and its FIRST commit is the amendment to `BUGS.md`'s rule 3 — a closing rule written after the closures it governs measures nothing. That amendment is the wave's shape: **where no instrument reached a defect, building one was the work**. Nine new instruments — the register's own index guard with its `--next` tool, two arms of `check-markup-contracts` (an orphan typed variant, a bare painting element), the named-state table's arm, invariant 10's, the D1 arm reaching NAVIGATIONS and not only declarations, the per-pull-request tier's composition, a `stale-figure` arm, and four harness rules (R96 the add screen's footer, R97 an avatar's image, R98 the two dismiss gestures, R99 the scrollbar). Two evolutions drawn and held, E-002 and E-003, **confirmed by the operator's own finger on 2026-08-29** — the third exercise no driver can perform, since `pointercancel` is delivered by a compositor deciding it wants the gesture. **B-085 goes 73 → 93.** Eleven were counted at the wave's close, ten of them in instruments the wave was writing at the time; an adversarial review of the finished work then returned about fifty confirmed defects, and NINE MORE of that species came out of repairing them — a guard running in no CI job, a conformity hook accepting the three shapes the attribution actually arrives in, a ratchet whose ceiling was one editable integer, an arm whose vocabulary held none of the seven aliases the frame writes. **The lesson written at the close was reversed by the review**: a mutation aimed at the instrument you just wrote is not a second reader, it is the same reader with a sharper tool — it finds what you thought to break, and the nine are what nobody thought to break. **B-042 was the sharpest of them**: closed `fixed #516` with a body byte-identical to `main`, inside the wave that hardened the rule against exactly that. It has its closure now, and a sixth arm of the register guard compares every closed entry's body against the branch point |
...
| **Next**                   | **L11 — Offline and PWA** (depends on L09, landed): no wave opens without its design and its plan. The lots run strictly in sequence. **L12 and L14 also satisfy § 0's rule** — L12 depends on L05 and L07, L14 on L07 and L09 — and L11 is first in the file's order. **L14 is new, added 2026-08-28**: it carries the decomposition of the four surfaces over the 400-line ceiling, a debt that had been labelled L09's and that L09 landed without paying |
| **Before it**              | L09 — PR **#509**, version 0.98.45 · L08-bis — PR **#505**, version 0.98.43 · L08 — PR **#503**, version 0.98.42 · L07-bis — PR **#500**, version 0.98.40 · L07 — PR **#494**, version 0.98.37 · L06 — PR **#490**, version 0.98.32 · L05 — PR **#482** · L04 — PR **#478** · L03 — PR **#475**, version 0.98.18 · L02 — PR **#470**, version 0.98.13 · L01 — PR **#467**, version 0.98.10. **Every one of them is archived under `docs/archive/features/`** — a fact stated once, because the sentence it replaces was a LIST that grew a clause per wave (« all three archived; L04 and L05 are archived beside them; L06 and L07 are archived beside them ») and had stopped at L07 while three more lots landed. This row exists beside « Landed, in order » to carry what that row does not: the pull request and the version. It carries nothing else, and it names no wave as « this pull request » — that phrase sat here for weeks pointing at a PR merged long before, which is B-152's shape and the reason this row was re-read at all |
| **Landed, in order**       | L01 · L02 · L03 · L04 · L05 · L06 · L07 · L08 · L09 · L10, plus the correction waves L07-bis and L08-bis. **This row is the state the plan's § 0 reads** — since 2026-08-28 `frontend-architecture.md` carries the order and the dependencies and no status at all, so a lot's progress exists once and cannot go stale in a second copy |
...
**Next action**: **L11 — Offline and PWA** is the next lot, and nothing is open on it: no design,
no plan, no branch. A wave opens with both. L10's design and plan are ARCHIVED —
