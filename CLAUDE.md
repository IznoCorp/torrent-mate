...
- **A vocabulary SEEDED from the codebase certifies the status quo.** The dying engine's French debt (`design/src/engine/legacy.js@c0a5062ac`) lived below a banner, named as French on purpose, until the engine went: `check_french_debt` and the vocabulary's debt section died with it at L13r.
