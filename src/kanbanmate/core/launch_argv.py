...
def wrap_with_session_end(argv: list[str], issue: int, *, session_end_bin: str) -> str:
    ...
    Returns:
        The composed command line ``<quoted argv> ; <quoted session_end_bin> <issue>``.
    """
    command = " ".join(shlex.quote(part) for part in argv)
    return f"{command} ; {shlex.quote(session_end_bin)} {issue}"
