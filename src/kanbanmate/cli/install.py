...
# The PM2 app name for the webhook receiver (ingress-multiproject §8). A SECOND PM2 app alongside the
# daemon on ONE runtime root: the daemon writes, the receiver nudges.
PM2_SERVE_APP_NAME = "kanban-serve"
...
def _pm2_app_block(name: str, kanban_command: str, args: str) -> str:
    """Render one PM2 app block (the daemon or the webhook receiver) — shared boilerplate.

    Args:
        name: The PM2 app name.
        kanban_command: The console-script to exec (``kanban`` by default).
        args: The subcommand args string (``"run"`` for the daemon, ``"serve"`` for the receiver).

    Returns:
        The JavaScript object literal for one app (trailing comma included).
    """
    # ``interpreter: "none"`` is MANDATORY: ``kanban`` is a Python console-script, so PM2 must exec
    # it directly (its shebang selects the pyenv interpreter) — without this, PM2 runs it through
    # Node and crashes. ``autorestart`` restarts on death; ``max_restarts``/``restart_delay`` damp
    # crash loops (the same hardened restart policy for both apps).
    return (
        "    {\n"
        f'      name: "{name}",\n'
...
def _ecosystem_body(*, kanban_command: str = "kanban", serve: bool = False) -> str:
    """Render the PM2 ecosystem file describing the ``kanban`` daemon (+ optional receiver) apps.

    The daemon app runs ``kanban run`` (the supervisor-agnostic blocking loop, DESIGN §5) with
    ``autorestart`` so PM2 restarts it on *exit* (not on hang — the daemon's own watchdog handles
    hangs). When ``serve`` is set (an ingress=webhook install) a SECOND ``kanban-serve`` app is
    emitted alongside it (ingress-multiproject §8): the daemon writes, the receiver nudges — two PM2
    apps on ONE runtime root.
    ...
    Args:
        ...
        serve: When ``True``, also emit the ``kanban-serve`` webhook-receiver app.

    Returns:
        The JavaScript source of the ecosystem file as a string.
    """
    apps = _pm2_app_block(PM2_APP_NAME, kanban_command, "run")
    if serve:
        apps += _pm2_app_block(PM2_SERVE_APP_NAME, kanban_command, "serve")
    return (
        ...
        "// on EXIT (not on hang; the daemon has its own per-tick watchdog). When ingress=webhook a\n"
        "// second `kanban-serve` receiver app fronts it (ingress-multiproject §8).\n"
        "module.exports = {\n"
        "  apps: [\n"
        f"{apps}"
        "  ],\n"
...
def _write_ecosystem(
    ecosystem_path: Path, *, kanban_command: str = "kanban", serve: bool = False
) -> None:
    ...
    Args:
        ...
        serve: When ``True``, also emit the ``kanban-serve`` webhook-receiver app (§8).
    """
    ecosystem_path.write_text(
        _ecosystem_body(kanban_command=kanban_command, serve=serve), encoding="utf-8"
    )
...
def host_install(
    ...
    *,
    ...
    Args:
        ...
        serve: When ``True`` (an ingress=webhook install), also write + start the ``kanban-serve``
            webhook-receiver PM2 app alongside the daemon (ingress-multiproject §8).
    ...
    # Step 3: PM2 wiring (write the ecosystem file always; drive PM2 unless asked not to). When
    # ``serve`` is set the ecosystem carries a SECOND ``kanban-serve`` receiver app (§8).
    _write_ecosystem(eco_path, kanban_command=kanban_command, serve=serve)
    if run_pm2:
        ...
        if serve:
            # Start the receiver app too (a separate process; the daemon writes, the receiver nudges).
            _run_quiet(runner, ["pm2", "start", str(eco_path), "--only", PM2_SERVE_APP_NAME])
...
def host_uninstall(
    ...
    *,
    ...
    # Step 2: PM2 teardown. `pm2 delete <app>` exits non-zero when the app is absent — tolerated.
    # The receiver app (kanban-serve) is also removed (a no-op when it was never installed).
    if run_pm2:
        _run_quiet(runner, ["pm2", "delete", PM2_APP_NAME])
        _run_quiet(runner, ["pm2", "delete", PM2_SERVE_APP_NAME])
