...
export function Chip({ tone, title, children }: ChipProps): ReactElement {
  return (
    <span
      data-slot="chip"
      {...(title != null ? { title } : {})}
      className={`inline-flex items-center gap-[5px] whitespace-nowrap rounded-full px-[7px] py-[2px] text-[11px] font-semibold ${TONE_CHIP_CLASS[tone] ?? "bg-muted text-muted-foreground"}`}
    >
