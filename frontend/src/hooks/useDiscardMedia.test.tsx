...
const MOCK_DISCARD_RESPONSE = {
  ok: true,
  media_id: "abc123",
  journaled: true,
  quarantine_path: "/staging/_quarantine/abc123",
  detail: "Nettoyé",
};
...
describe("useDiscardMedia", () => {
  it("calls discardMedia and resolves on 200", async () => {
    fetchMock.mockResolvedValue(buildResponse(200, MOCK_DISCARD_RESPONSE));

    const { result } = renderHook(() => useDiscardMedia(), {
    ...
    expect(result.current.data).toEqual(MOCK_DISCARD_RESPONSE);
  });

  it("surfaces ApiError on non-OK response", async () => {
    fetchMock.mockResolvedValue(
      buildResponse(422, { detail: "Item is scrapable media" }),
    );
