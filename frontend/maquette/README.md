...
`var()` resolves), the 56 rule scripts in `harness/`, and the fact that a rendering change in the
...
The 56 scripts that measure the prototype in headless Chromium. They are committed because they
...
| `residue.py`          | R80: a typed variant and the residue rule shadowing it say the same thing — each unlayered `legacy.css` selector is paired with the `cva()` factory wearing its identity anchor and compared in the DOCUMENT, on two sibling probes, under both `prefers-reduced-motion` settings, for exactly the properties the residue declares |
