...
| `journey.py`       | R82: Back retraces the path taken (§ 16) — a page opened cold has a floor under it, the not-found surface's own way out RECORDS rather than spends the guard, switching a top-level page stacks nothing (the drawer's and the account menu's switches included), the exit guard arms at the TOP and nowhere else, a setting leaves no entry, and a Back returns to the real origin with the setting it was made under. Every hold reads `history.length` and `armedExit`, never the address alone: the destination's address is right whichever stack was built under it |
