...
decision is written beside the class that applies it. `src/styles/harness.css` is the phone frame — it is in the maquette's OWN build, because that is
...
| `factories.py`        | not a rule: the `cva()` factory reader — every typed variant's anchor, base and branches read as text, which `resolution_card.py` borrows to compare an affordance with its factory's own declaration |
