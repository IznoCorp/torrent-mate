// design/src/ecrans/resolution.tsx
...
// `fiche.tsx`, `profil.tsx` and `releases.tsx`. `data-resolve` (pick this
...
import {
  ...
  useReferentiel,
  type CarteFile,
  type DecisionAttente,
  type DecisionReglee,
} from "../donnees";

// Same helper as `fiche.tsx`'s, `profil.tsx`'s, `ajout.tsx`'s and
    ...
    <div className="card" data-nonmedia={opts.genre || "release"}>
      <div className="ctop">
        ...
        <span className="cbody">
          <span className="ctitle">{titre}</span>
...
function CarteDecision({ decision }: { decision: DecisionReglee }) {
  const {
    posterBox,
    svgIcon,
    icons,
  ...
  const regle = decision.etat != null;
  const etat = regle ? ETAT_DECISION[decision.etat] : null;
  const affiche =
    regle && decision.choix
      ? posterBox(decision.choix.t, decision.k)
      : `<span class="pfall">${svgIcon(decision.k === "movie" ? icons.film : icons.tv, 1.25)}<b>?</b></span>`;
  const identite = decision.choix
  ...
  return (
    <div className="card" data-nonmedia="decision">
      <div className="ctop">
        <span
          className="poster"
          dangerouslySetInnerHTML={{ __html: affiche }}
        />
        <span className="cbody">
          ...
          {identite ? <span className="creason">{identite}</span> : ""}
          <span className="cmeta">
...
function Candidats({ decision }: { decision: DecisionAttente }) {
  const meilleur = Math.max(...decision.c.map((candidat) => candidat.s));
    ...
    <>
      ...
      {decision.c.map((candidat) => (
        <CarteRelease
          ...
          confiance={
            exAequo > 1 && candidat.s === meilleur
              ? null
              : `${Math.round(candidat.s * 100)} %`
          }
...
export function ResolutionEcran() {
  const { dossier: brut } = useParams({ from: "/resolution/$dossier" });
  ...
  const dossier = brut.normalize("NFC");
  ...
  const decision = decisionEnAttente(dossier);
  ...
  const attente = derivedBlocked()
    .concat(derivedStuck())
    .filter(
      (bloque: CarteFile) => decisionEnAttente(bloque.t as string) != null,
    );
  const rang = decision
    ? attente.findIndex((bloque: CarteFile) => bloque.t === decision.d) + 1
    : 0;
  ...
  return (
    <section className="screen open" data-cle={`resolution:${dossier}`}>
