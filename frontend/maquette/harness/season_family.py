...
must agree with it. Four families state one, and they are read here:
  ...
  1. THE SEASON FAMILY (`window.__mocks.seasons()`, `[number, aired, owned]`), which the
...
FAMILIES = """async ([followsPath, incompletePath, seasonsPath, sheetPath]) => {
  ...
  const shows = [];
  for (const title of Object.keys(window.__mocks.seasons())) {
    ...
    shows.push({title, family: window.__mocks.seasons()[title], address,
...
async def one_season_on_both_surfaces(page, journal, errors, bare, key, season):
    ...
    family = await page.evaluate(
        "([title, season])=>(window.__mocks.seasons()[title] || []).find(([n]) => n === season) || null",
        [bare, season])
