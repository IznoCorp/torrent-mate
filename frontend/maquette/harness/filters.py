...
async def main():
  async with async_playwright() as p:
    ...
    cats = await pg.evaluate("()=>(window.__queries.getQueryCache().getAll().find(q=>q.queryKey[0]==='/api/library/categories')?.state.data||[]).map(c=>({id:c.id,l:c.label,c:c.count}))")
    ...
    for cat in cats:
        ...
        r = await pg.evaluate("""()=>({shown:document.querySelectorAll('#libitems [data-part="tile"], #libitems [data-part="card"]').length,
          ...
          coherent:[...document.querySelectorAll('#libitems [data-part="tile"]')].every(t=>{
            const o=(window.__queries.getQueryCache().getAll().filter(q=>q.queryKey[0]==='/api/library/items').sort((l,r)=>r.state.dataUpdatedAt-l.state.dataUpdatedAt)[0]?.state.data?.pages??[]).flatMap(p=>p.items).find(x=>x.title===t.querySelector('[data-part="tile/title"]').textContent); return !!o;})})""")
