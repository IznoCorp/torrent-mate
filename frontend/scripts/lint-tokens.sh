...
#   - src/styles/ps/app-surface.css     GENERATED by scripts/extract-maquette-css.py from
#                                       the maquette, and since SP5a it carries the redesign's
#                                       own token block — it is a token SOURCE, like tokens/**.
#                                       Hand-editing it is already refused by the drift guard,
#                                       so « use a DS token » has no reader here. What its
#                                       var() calls resolve to is held by
#                                       scripts/check-css-tokens.py instead.
set -euo pipefail
...
matches=$(rg -n --no-heading "$PATTERN" \
  ...
  -g '!src/styles/ps/app-surface.css' \
  src || true)
